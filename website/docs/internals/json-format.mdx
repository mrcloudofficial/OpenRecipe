...
description: >-
  OpenRecipe provides a machine-readable JSON representation of state,
  configuration and plan.
---

# JSON Output Format

-> **Note:** This format is available in OpenRecipe 0.12 and later.

When OpenRecipe plans to make changes, it prints a human-readable summary to the terminal. It can also, when run with `-out=<PATH>`, write a much more detailed binary plan file, which can later be used to apply those changes.

Since the format of plan files isn't suited for use with external tools (and likely never will be), OpenRecipe can output a machine-readable JSON representation of a plan file's changes. It can also convert state files to the same format, to simplify data loading and provide better long-term compatibility.
...
The output includes a `format_version` key, which as of OpenRecipe 1.1.0 has
...
- [Configuration Representation](#configuration-representation) — A sub-object of plan output that describes a parsed OpenRecipe configuration.
...
A plan consists of a prior state, the configuration that is being applied to that state, and the set of changes OpenRecipe plans to make to achieve that.
...
```javascript
{
  ...
  // OpenRecipe plans to use to move from the prior state to a new state
  ...
  "resource_changes": [
    ...
    // configuration are included in this list.
    {
      ...
      // instance as it was known after the previous OpenRecipe run.
      ...
      //   "tainted" in the prior state, so OpenRecipe planned to replace it.
      ...
      // - "delete_because_no_resource_config": OpenRecipe found no resource
      ...
      // - "read_because_config_unknown": For a data resource, OpenRecipe cannot
      //   read the data during the plan phase because of values in the
      //   configuration that won't be known until the apply phase.
      // - "read_because_dependency_pending": For a data resource, OpenRecipe
      ...
      //
      // If there is no special reason to note, OpenRecipe will omit this
      // property altogether.
      "action_reason": "replace_because_tainted"
    }
  ],

  // "resource_drift" is a description of the changes OpenRecipe detected
  ...
  "output_changes": {
    ...
    "foo": {
      ...
      //   ["update"]
      //   ["delete"]
      // In the OpenRecipe CLI 0.12.0 release, OpenRecipe is not yet fully able to
  ...
  // resources with postconditions, with as much information as OpenRecipe can
  ...
  "root_module": {
    "resources": [
      {
        ...
        // pass this verbatim to other OpenRecipe commands that are documented to
    ...
    "child_modules": [
      ...
      // object, with the additional "address" property shown below.
      {
        ...
        // strings and may pass verbatim to other OpenRecipe commands that are
...
Configuration is the most complicated structure in OpenRecipe, since it includes unevaluated expression nodes and other complexities.
...
```javascript
{
  ...
  // provider configurations are the one concept in OpenRecipe that can span
...
```javascript
{
  ...
  "references": [
    ...
    // OpenRecipe considers "module.foo" to be an atomic reference, not an
    // attribute access.
  ]
}
```
...
and some details may change in future OpenRecipe versions based on feedback,
even in minor releases of OpenRecipe CLI.
    ...
    "instances": [
      {
        ...
        // When a condition expression is invalid, OpenRecipe returns that as
...
even if a runtime error prevents OpenRecipe from evaluating its "count" or
...
see that OpenRecipe recognized the existence of the checks, even if it wasn't
