...
description: |-
  ...
  origin host for one or more OpenRecipe providers, specifying which providers
  are available and where to find their distribution packages.
---

# Provider Registry Protocol

-> Third-party provider registries are supported only in OpenRecipe CLI 0.13 and later. Prior versions do not support this protocol.

The provider registry protocol is what OpenRecipe CLI uses to discover metadata
...
[OpenRecipe Registry](https://registry.terraform.io/) at `registry.terraform.io`.
...
alternative to publishing them on the public OpenRecipe Registry.
...
API that provider plugins themselves implement to serve requests from OpenRecipe
CLI at runtime. For more information on the provider API, see the OpenRecipe
SDK documentation.

The public OpenRecipe Registry implements a superset of the API described on
...
Each OpenRecipe provider has an associated address which uniquely identifies it
within OpenRecipe. A provider address has the syntax `hostname/namespace/type`,
where:

* `hostname` is the registry host that the provider is considered to have
  originated from, and the default location OpenRecipe will consult for
...
* `namespace` is the name of a namespace, unique on a particular hostname, that
  ...
  OpenRecipe Registry the "namespace" represents the organization that is
...
OpenRecipe users, please consider publishing it into the public
[OpenRecipe Registry](https://registry.terraform.io/), which will make your
...
OpenRecipe uses the full address (after normalization to always include a
...
another namespace or publishing it on a different hostname will cause OpenRecipe
...
of which has an associated version number. OpenRecipe assumes version numbers
...
an end-user of OpenRecipe serving as the "public API".

All available versions for a particular provider address are considered to be
the same provider by OpenRecipe. Each OpenRecipe configuration selects only one
...
The providers protocol begins with OpenRecipe CLI using
[OpenRecipe's remote service discovery protocol](/terraform/internals/remote-service-discovery),
...
If the given URL is a relative URL then OpenRecipe will interpret it as relative
...
registry must implement to be compatible with OpenRecipe CLI's provider
...
OpenRecipe Registry as working examples, assuming that the caller already
...
* `protocols` (recommended): an array of OpenRecipe provider API versions that
  ...
  OpenRecipe uses this information, when available, to provide hints to users
  about upgrading or downgrading their version of a particular provider to
  work with their current version of OpenRecipe, if their currently-selected
  versions are not compatible.

  Which API versions are supported is, for most providers, decided by which
  version of the OpenRecipe SDK they are built against. Consult the OpenRecipe
  SDK documentation for more information.

  Only OpenRecipe 0.13 and later support third-party provider registries and
  that OpenRecipe version requires API version `5.0` or later, so in practice
...
* `platforms` (recommended): an array of objects describing platforms that have
  ...
  OpenRecipe may use this information, when available, to provide hints to
...
OpenRecipe CLI uses this operation after it has selected the newest available
...
* `protocols` (required): an array of OpenRecipe provider API versions that
  the provider supports, in the same format as for
  [List Available Versions](#list-available-versions).

  While this property is optional when listing available options, it is
  _required_ for describing an individual provider package so that OpenRecipe
...
* `filename` (required): the filename for this provider's zip archive as
  recorded in the "shasums" document, so that OpenRecipe CLI can determine which
  of the given checksums should be used for this specific package.

* `download_url` (required): a URL from which OpenRecipe can retrieve the
...
* `shasums_url` (required): a URL from which OpenRecipe can retrieve a text
...
* `shasums_signature_url` (required): a URL from which OpenRecipe can retrieve
...
available for the requested operating system and/or architecture. OpenRecipe
