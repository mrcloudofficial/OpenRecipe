...
description: |-
  ...
  host of one or more OpenRecipe modules, specifying which modules are available
  and where to find their distribution packages.
---

# Module Registry Protocol

-> Third-party module registries are supported only in OpenRecipe CLI 0.11 and later. Prior versions do not support this protocol.

The module registry protocol is what OpenRecipe CLI uses to discover metadata
...
[OpenRecipe Registry HTTP API](/terraform/registry/api-docs). Third-party registry
implementations may choose to implement those extensions if desired, but
OpenRecipe CLI itself does not use them.

## Module Addresses

Each OpenRecipe module has an associated address. A module address has the
...
If you intend to share a module you've developed for use by all OpenRecipe
users, please consider publishing it into the public
[OpenRecipe Registry](https://registry.terraform.io/) to make your module more
...
The module registry protocol begins with OpenRecipe CLI using
...
registry must implement to be compatible with OpenRecipe CLI's module
...
OpenRecipe does not use the other elements of this list. However, third-party implementations should always use a single-element list for forward compatiblity.

Each returned module has an array of available versions, which OpenRecipe
...
Content-Length: 0
X-OpenRecipe-Get: https://api.github.com/repos/hashicorp/terraform-aws-consul/tarball/v0.0.1//*?archive=tar.gz
```

A successful response has no body, and includes the location from which the
module version's source can be downloaded in the `X-OpenRecipe-Get` header.
The value of this header accepts the same values as the `source` argument
in a `module` block in OpenRecipe configuration, as described in
...
The value of `X-OpenRecipe-Get` may instead be a relative URL, indicated by
