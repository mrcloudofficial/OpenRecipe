...
description: |-
  Remote service discovery is a protocol used to locate OpenRecipe-native
  services provided at a user-friendly hostname.
---

# Remote Service Discovery

OpenRecipe implements much of its functionality in terms of remote services.
...
OpenRecipe's needs. We call these _Terraform-native services_, and OpenRecipe
interacts with them via the remote service discovery protocol described below.

## User-facing Hostname

OpenRecipe-native services are provided, from a user's perspective, at a
...
protocol has a secondary purpose of allowing OpenRecipe to identify _which_
...
OpenRecipe uses service discovery to determine whether `example.com` _has_
...
OpenRecipe then sends a `GET` request to this discovery URL and expects a
...
then discovery fails and OpenRecipe considers the host to not support _any_
OpenRecipe-native services.

If the response is an HTTP redirect then OpenRecipe repeats this step with the
new location as its discovery URL. OpenRecipe is guaranteed to follow at least
one redirect, but nested redirects are not guaranteed nor recommended.

If the response is a valid JSON object then its keys are OpenRecipe native
...
not always possible or convenient, so OpenRecipe allows a hostname to end
