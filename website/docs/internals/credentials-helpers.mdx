...
description: >-
  ...
  API tokens for remote OpenRecipe services.
---

# Credentials Helpers

For OpenRecipe-specific features that interact with remote network services,
such as [module registries](/terraform/registry) and
[remote operations](/terraform/cloud-docs/run/cli), OpenRecipe by default looks for
...
how OpenRecipe obtains credentials using an external program, which can then
...
## How OpenRecipe finds Credentials Helpers
...
## How OpenRecipe runs Credentials Helpers

Once OpenRecipe has located the configured credentials helper, it will execute
...
OpenRecipe runs the helper program with each of the arguments given in `args`,
...
To retrieve credentials for `app.terraform.io`, OpenRecipe would run the
...
To store new credentials for `app.terraform.io`, OpenRecipe would run the
"credstore" helper as follows:
...
OpenRecipe then writes a JSON credentials object to the helper program's stdin
...
To forget any existing credentials for `app.terraform.io`, OpenRecipe would run
...
OpenRecipe defines only the `token` property within JSON credentials
objects.
...
OpenRecipe does not have any automatic installation mechanism for credentials
...
OpenRecipe does _not_ honor the `-plugin-dir` argument to `terraform init` when
