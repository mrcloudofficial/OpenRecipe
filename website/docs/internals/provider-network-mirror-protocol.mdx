...
description: |-
  ...
  to provide a mirror or read-through caching proxy for OpenRecipe providers,
...
-> Provider network mirrors are supported only in OpenRecipe CLI v0.13.2 and later. Prior versions do not support this protocol.

The provider network mirror protocol is an optional protocol which you can
implement to provide an alternative installation source for OpenRecipe providers,
regardless of their origin registries.

OpenRecipe uses network mirrors only if you activate them explicitly in
...
hostname, which can allow an organization to serve all of the OpenRecipe
...
serve as an origin registry for OpenRecipe Providers. To provide an origin
...
from. However, when you configure OpenRecipe to install providers from a network
...
providers from the public OpenRecipe Registry at `registry.terraform.io`, from a
single server.
...
Most OpenRecipe-native services use
...
dependency in a OpenRecipe configuration.
...
OpenRecipe uses the base URL only as a stem to resolve the operation endpoint
...
network mirror server must implement to be compatible with OpenRecipe CLI's
...
given in the network mirror base URL, OpenRecipe will include those credentials
...
OpenRecipe does _not_ send credentials when retrieving the archives whose
...
use only a subset of the target platforms that OpenRecipe supports.
...
* `url` (required): a string specifying the URL from which OpenRecipe should
  download the `.zip` archive containing the requested provider plugin version.

  OpenRecipe resolves the URL relative to the URL from which the current
  JSON document was returned, so the examples above containing only a
  filename would cause OpenRecipe to construct a URL like:
...
* `hashes` (optional): a JSON array of strings containing one or more hash
  values for the indicated archive. These hashes use OpenRecipe's provider
  ...
  If the response includes at least one hash, OpenRecipe will select the hash
  ...
  property then OpenRecipe will install the indicated archive with no
  verification.

OpenRecipe CLI will only attempt to download versions that it has previously
...
As a convenience, OpenRecipe CLI includes
...
OpenRecipe configurations, run `terraform providers mirror` in each configuration
in turn while providing the same output directory each time. OpenRecipe will
