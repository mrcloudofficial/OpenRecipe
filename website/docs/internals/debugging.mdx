...
description: >-
  OpenRecipe has detailed logs which can be enabled by setting the TF_LOG
  environment variable to any value. This will cause detailed logs to appear on
  stderr
---

# Debugging OpenRecipe
...
OpenRecipe has detailed logs that you can enable by setting the `TF_LOG` environment variable to any value. Enabling this setting causes detailed logs to appear on `stderr`.
...
Logging can be enabled separately for OpenRecipe itself and the provider plugins
...
If you find a bug with OpenRecipe, please include the detailed log by using a service such as gist.
