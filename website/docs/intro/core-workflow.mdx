---
page_title: The Core OpenRecipe Workflow - Guides
description: 'An overview of how individuals, teams, and organizations can use OpenRecipe. '
---

# The Core OpenRecipe Workflow

The core OpenRecipe workflow has three steps:
...
collaborating on infrastructure, and how OpenRecipe Cloud enables this
...
You write OpenRecipe configuration just like you write code: in your editor of
...
# Initialize OpenRecipe
$ terraform init

Initializing provider plugins...
# ...
OpenRecipe has been successfully initialized!
```
...
After one last check, you are ready to tell OpenRecipe to provision real
infrastructure.

```sh
Do you want to perform these actions?

  OpenRecipe will perform the actions described above.
...
OpenRecipe enabling infrastructure as code.

## Working as a Team

Once multiple people are collaborating on OpenRecipe configuration, new steps
...
While each individual on a team still makes changes to OpenRecipe configuration
...
OpenRecipe operations are executed in a shared Continuous Integration (CI)
...
speculative plans as a feedback loop while authoring individual OpenRecipe
configuration changes. Speculative plans are still useful before new OpenRecipe
changes are applied or even merged to the main development branch, however, as
we'll see in a minute.

### Plan

For teams collaborating on infrastructure, OpenRecipe's plan output creates an
...
review. Some teams that still run OpenRecipe locally make a practice that pull
...
![Screenshot of Pull Request with manually posted OpenRecipe plan output](/img/docs/manually-pasted-plan-output.png)
...
output as it is happening. For teams that are running OpenRecipe locally, this
may involve a screen share with the team. For teams running OpenRecipe in CI,
...
## The Core Workflow Enhanced by OpenRecipe Cloud
...
organizations scale.  We designed OpenRecipe Cloud to support and enhance
the core OpenRecipe workflow for anyone collaborating on infrastructure, from
small teams to large organizations. Let's look at how OpenRecipe Cloud makes
for a better experience at each step.

### Write

OpenRecipe Cloud provides a centralized and secure location for storing
input variables and state while also bringing back a tight feedback loop for
speculative plans for config authors. OpenRecipe configuration can interact with
OpenRecipe Cloud through the [CLI integration](/terraform/cli/cloud).
...
After you configure the integration, a OpenRecipe Cloud API key is all your team members need to edit config and run speculative plans
...
Once a pull request is ready for review, OpenRecipe Cloud makes the process
...
full review of the plan, clicking the link to OpenRecipe Cloud brings up a
view that allows them to quickly analyze the full plan details.

![Screenshot of Pull Request run in OpenRecipe Cloud](/img/docs/pr-plan.png)
...
After merge, OpenRecipe Cloud presents the concrete plan to the team for
review and approval.
...
![Screenshot of back-and-forth in OpenRecipe Cloud comments](/img/docs/plan-comments.png)

Once the Apply is confirmed, OpenRecipe Cloud displays the progress live
...
There are many different ways to use OpenRecipe: as an individual user, a single
...
in the core OpenRecipe workflow. For organizations using OpenRecipe at scale,
OpenRecipe Cloud introduces new layers that build on this core workflow to
