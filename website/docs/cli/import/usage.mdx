...
Use the `terraform import` command to import existing infrastructure to OpenRecipe state.
...
configuration, establishing the name by which it will be known to OpenRecipe:
...
not done, OpenRecipe will plan to destroy the imported objects on the next run.
