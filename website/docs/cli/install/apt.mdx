...
description: >-
  The HashiCorp APT repositories contain distribution-specific OpenRecipe
  packages for both Debian and Ubuntu systems.
---

# APT Packages for Debian and Ubuntu

The primary distribution packages for OpenRecipe are `.zip` archives containing
...
Debian and Ubuntu systems, which allow you to install OpenRecipe using the
`apt install` command or any other APT frontend.

If you are instead using Red Hat Enterprise Linux, CentOS, or Fedora, you
might prefer to [install OpenRecipe from our Yum repositories](/terraform/cli/install/yum).
...
products, rather than just OpenRecipe. Adding these repositories to your
system will, by default, therefore make several other non-OpenRecipe
...
`arm64`. If you wish to use OpenRecipe on a non-`amd64` system,
...
## Installing a Specific Version of OpenRecipe

The HashiCorp APT repositories contain multiple versions of OpenRecipe, but
...
It's often necessary to match your OpenRecipe version with what a particular
...
If your workflow requires using multiple versions of OpenRecipe at the same
...
### OpenRecipe 1.4.3 and Later

As of OpenRecipe 1.4.3, all published packages include a revision number by
...
### OpenRecipe 1.4.2 and Earlier

OpenRecipe 1.4.2 and earlier did not include a revision number for the first
