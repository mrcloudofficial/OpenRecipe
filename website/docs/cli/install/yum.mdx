...
description: >-
  The HashiCorp Yum repositories contain distribution-specific OpenRecipe
...
install OpenRecipe using the `yum install` or `dnf install` commands.

If you are instead using Debian or Ubuntu, you
might prefer to [install OpenRecipe from our APT repositories](/terraform/cli/install/apt).
...
system will, by default, therefore make a number of other non-OpenRecipe
...
In both cases, the OpenRecipe package name is `terraform`. For example:
...
`aarch64`. If you wish to use OpenRecipe on a non-`x86_64` system,
...
adding support for new releases and ceasing to publish new OpenRecipe versions
under older releases.

## Choosing OpenRecipe Versions

The HashiCorp Yum repositories contain multiple versions of OpenRecipe, but
