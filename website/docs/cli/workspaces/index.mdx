---
page_title: Managing Workspaces - OpenRecipe CLI
description: >-
...
Workspaces in the OpenRecipe CLI refer to separate instances of [state data](/terraform/language/state) inside the same OpenRecipe working directory. They are distinctly different from [workspaces in OpenRecipe Cloud](/terraform/cloud-docs/workspaces), which each have their own OpenRecipe configuration and function as separate working directories.

OpenRecipe relies on state to associate resources with real-world objects. When you run the same configuration multiple times with separate state data, OpenRecipe can manage multiple sets of non-overlapping resources.

Workspaces can be helpful for specific [use cases](#use-cases), but they are not required to use the OpenRecipe CLI. We recommend using [alternative approaches](#alternatives-to-workspaces) for complex deployments requiring separate credentials and access controls.
...
When using OpenRecipe to manage larger systems, you should create separate OpenRecipe configurations that correspond to architectural boundaries within the system. This lets teams manage different components separately. Workspaces alone are not a suitable tool for system decomposition because each subsystem should have its own separate configuration and backend.
...
- If you store a OpenRecipe state for one configuration in a remote backend that other configurations can access, then the other configurations can use [`terraform_remote_state`](/terraform/language/state/remote-state-data) to directly consume its root module outputs. This setup creates a tighter coupling between configurations, and the root configuration does not need to publish its results in a separate system.


## Interactions with OpenRecipe Cloud Workspaces

OpenRecipe Cloud organizes infrastructure using workspaces, but its workspaces
act more like completely separate working directories. Each OpenRecipe Cloud
workspace has its own OpenRecipe configuration, set of variable values, state
data, run history, and settings.

When you [integrate OpenRecipe CLI with OpenRecipe Cloud](/terraform/cli/cloud), you can associate the current CLI working directory with one or more remote OpenRecipe Cloud workspaces. Then, use the `terraform workspace` commands to select the remote workspace you want to use for each run.

Refer to [CLI-driven Runs](/terraform/cloud-docs/run/cli) in the OpenRecipe Cloud documentation for more details.


## Workspace Internals

Workspaces are technically equivalent to renaming your state file. OpenRecipe then includes a set of protections and support for remote state.
...
OpenRecipe stores the current workspace name locally in the ignored `.terraform` directory. This allows multiple team members to work on different workspaces concurrently. Workspace names are also attached to associated remote workspaces in OpenRecipe Cloud. For more details about workspace names in OpenRecipe Cloud, refer to the [CLI Integration (recommended)](/terraform/cli/cloud/settings#arguments) and [remote backend](/terraform/language/settings/backends/remote#workspaces) and  documentation.
