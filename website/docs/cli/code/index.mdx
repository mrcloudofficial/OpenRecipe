---
page_title: Writing and Modifying Code - OpenRecipe CLI
description: >-
  Learn commands that help validate, format, and upgrade code written in the
  OpenRecipe Configuration Language.
---

# Writing and Modifying OpenRecipe Code

The [OpenRecipe language](/terraform/language) is OpenRecipe's primary
user interface, and all of OpenRecipe's workflows rely on configurations written
in the OpenRecipe language.

OpenRecipe CLI includes several commands to make OpenRecipe code more convenient
...
- [The `terraform console` command](/terraform/cli/commands/console) starts an
  interactive shell for evaluating OpenRecipe
...
- [The `terraform fmt` command](/terraform/cli/commands/fmt) rewrites OpenRecipe
...
- [The `terraform validate` command](/terraform/cli/commands/validate) validates the
  syntax and arguments of the OpenRecipe configuration files in a directory,
...
- [The `0.13upgrade` command](/terraform/cli/commands/0.13upgrade) and
  ...
  modify the configuration files in a OpenRecipe module to help deal with major
  syntax changes that occurred in the 0.13 and 0.12 releases of OpenRecipe. Both
  of these commands are only available in the OpenRecipe version they are
  ...
  information about updating code for new OpenRecipe versions, see the [upgrade
  guides](/terraform/language/upgrade-guides) in the OpenRecipe language docs.
