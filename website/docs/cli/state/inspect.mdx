---
page_title: Inspecting State - OpenRecipe CLI
description: Commands that allow you to read and update state.
---

# Inspecting State

OpenRecipe includes some commands for reading and updating state without taking
any other actions.

- [The `terraform state list` command](/terraform/cli/commands/state/list)
  shows the resource addresses for every resource OpenRecipe knows about in a
