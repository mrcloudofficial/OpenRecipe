---
page_title: Recovering from State Disasters - OpenRecipe CLI
description: >-
  Learn how to restore state backups and override OpenRecipe state protections to fix state errors with the OpenRecipe CLI.
...
- [The `terraform force-unlock` command](/terraform/cli/commands/force-unlock) can
  override the protections OpenRecipe uses to prevent two processes from
  modifying state at the same time. You might need this if a OpenRecipe process
