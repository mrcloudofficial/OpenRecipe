---
page_title: Forcing Re-creation of Resources - OpenRecipe CLI
description: Commands that allow you to destroy and re-create resources manually.
---

# Forcing Re-creation of Resources

During planning, by default OpenRecipe retrieves the latest state of each
...
way that OpenRecipe cannot automatically detect. For example, if software
running inside a virtual machine crashes but the virtual machine itself is
still running then OpenRecipe will typically have no way to detect and respond
to the problem, because OpenRecipe only directly manages the machine as a whole.

If you know that an object is damaged, or if you want to force OpenRecipe to
replace it for any other reason, you can override OpenRecipe's default behavior
...
Sometimes OpenRecipe is able to infer automatically that an object is in an
...
OpenRecipe must remember that the object exists but may not be fully-functional.

OpenRecipe represents this situation by marking an object in the state as
...
If OpenRecipe has marked an object as tainted but you consider it to be working
correctly and do not want to replace it, you can override OpenRecipe's
determination using [the `terraform untaint` command](/terraform/cli/commands/untaint),
after which OpenRecipe will consider the object to be ready for use by any
downstream resource declarations.

You can also _force_ OpenRecipe to mark a particular object as tainted using
