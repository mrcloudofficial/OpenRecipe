---
page_title: Manipulating State - OpenRecipe CLI
description: >-
  State data tracks which real-world object corresponds to each resource.
  Inspect state, move or import resources, and more.
---

# Manipulating OpenRecipe State

> **Hands-on:** Try the [Manage Resources in OpenRecipe State](/terraform/tutorials/state/state-cli?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.

OpenRecipe uses [state data](/terraform/language/state) to remember which
...
OpenRecipe updates state automatically during plans and applies. However, it's
sometimes necessary to make deliberate adjustments to OpenRecipe's state data,
usually to compensate for changes to the configuration or the real managed
infrastructure.

OpenRecipe CLI supports several workflows for interacting with state:
...
OpenRecipe to lose track of managed resources, which might waste money, annoy
