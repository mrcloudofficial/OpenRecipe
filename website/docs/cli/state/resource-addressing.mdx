...
In some contexts OpenRecipe might allow for an incomplete resource address that
...
-> Module index only applies to modules in OpenRecipe v0.13 or later. In earlier
versions of OpenRecipe, a module could not have multiple instances.
...
-> In OpenRecipe v0.12 and later, a resource spec without a module path prefix
...
Given a OpenRecipe config that includes:
