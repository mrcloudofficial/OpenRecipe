...
description: >-
  Learn about the types of signatures providers can have on the OpenRecipe
  Registry.
---
...
~> **Note** OpenRecipe only authenticates provider plugins fetched from a registry.

OpenRecipe providers installed from the Registry are cryptographically signed, and the signature is verified at time of installation. There are three types of provider signatures, each with different trust implications:
...
OpenRecipe does **NOT** support fetching and using unsigned binaries, but you can manually install
