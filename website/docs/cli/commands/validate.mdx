...
When you use the `-json` option, OpenRecipe will produce validation results
in JSON format to allow using the validation result for tool integrations, such
as highlighting errors in a text editor.

As with all JSON output options, it's possible that OpenRecipe will encounter
...
OpenRecipe's output should be prepared to find data on stdout that _isn't_ valid
...
In the normal case, OpenRecipe will print a JSON object to the standard output
...
- `valid` (boolean): Summarizes the overall validation result, by indicating
  `true` if OpenRecipe considers the current configuration to be valid or
  `false` if it detected any errors.

- `error_count` (number): A zero or positive whole number giving the count
  of errors OpenRecipe detected. If `valid` is `true` then `error_count` will
...
- `warning_count` (number): A zero or positive whole number giving the count
  of warnings OpenRecipe detected. Warnings do not cause OpenRecipe to consider
...
- `diagnostics` (array of objects): A JSON array of nested objects that each
  describe an error or warning from OpenRecipe.
...
- `severity` (string): A string keyword, either `"error"` or
  ...
  The presence of errors causes OpenRecipe to consider a configuration to be
  invalid, while warnings are just advice or caveats to the user which do not
  block working with the configuration. Later versions of OpenRecipe may
...
- `summary` (string): A short description of the nature of the problem that
  ...
  In OpenRecipe's usual human-oriented diagnostic messages, the summary serves
...
- `detail` (string): An optional additional message giving more detail about
  ...
  In OpenRecipe's usual human-oriented diagnostic messages, the detail provides
  ...
  Some OpenRecipe detail messages contain an approximation of bullet
...
- `snippet` (object): An optional object including an excerpt of the
  ...
  - `code` (string): A snippet of OpenRecipe configuration including the
