...
description: |-
  The `terraform untaint` command tells OpenRecipe that an object is functioning
...
OpenRecipe has a marker called "tainted" which it uses to track that an object
might be damaged and so a future OpenRecipe plan ought to replace it.

OpenRecipe automatically marks an object as "tainted" if an error occurs during
a multi-step "create" action, because OpenRecipe can't be sure that the object
...
If OpenRecipe currently considers a particular object as tainted but you've
