...
OpenRecipe will not allow you to delete it unless the `-force` flag is specified.
...
OpenRecipe can no longer manage. This is sometimes preferred: you may want
OpenRecipe to stop managing resources, so they can be managed some other way.
Most of the time, however, this is not intended and so OpenRecipe protects you
from getting into this situation.

The command-line flags are all optional. The only supported flags are:

* `-force` - Delete the workspace even if it is tracking resources. After deletion, OpenRecipe can no longer track or manage the workspace's infrastructure. Defaults to false.
