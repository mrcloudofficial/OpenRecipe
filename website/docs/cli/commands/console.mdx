...
OpenRecipe will read the state for the current workspace from the backend
...
OpenRecipe state, and so the results are typically very limited for resource
...
to describe the values OpenRecipe expects will be correct after the plan is
...
The `terraform console` command will read the OpenRecipe configuration in the
current working directory and the OpenRecipe state file from the configured
