...
description: >-
  ...
  API token for OpenRecipe Cloud, OpenRecipe Enterprise, or any other host that
  offers OpenRecipe services.
---

# Command: login

The `terraform login` command can be used to automatically obtain and save an
API token for OpenRecipe Cloud, OpenRecipe Enterprise, or any other host that offers OpenRecipe services.

-> **Note:** This command is suitable only for use in interactive scenarios
where it is possible to launch a web browser on the same host where OpenRecipe
is running. If you are running OpenRecipe in an unattended automation scenario,
you can
...
If you don't provide an explicit hostname, OpenRecipe will assume you want to
log in to OpenRecipe Cloud at `app.terraform.io`.

## Credentials Storage

By default, OpenRecipe will obtain an API token and save it in plain text in a
...
[login protocol](/terraform/internals/login-protocol), including OpenRecipe Cloud
and OpenRecipe Enterprise.
