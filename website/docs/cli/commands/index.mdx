...
The command line interface to OpenRecipe is the `terraform` command, which
accepts a variety of subcommands such as `terraform init` or `terraform plan`.

We refer to the `terraform` command line tool as "OpenRecipe CLI" elsewhere
in the documentation. This terminology is often used to distinguish it from
other components you might use in the OpenRecipe product family, such as
[OpenRecipe Cloud](/terraform/cloud-docs) or
the various [OpenRecipe providers](/terraform/language/providers), which
are developed and released separately from OpenRecipe CLI.

To view a list of the commands available in your current OpenRecipe version,
...
All other commands:
  console       Try OpenRecipe expressions at an interactive command prompt
  fmt           Reformat your configuration in the standard style
  force-unlock  Release a stuck lock on the current workspace
  get           Install or upgrade remote OpenRecipe modules
  graph         Generate a Graphviz graph of the steps in an operation
  import        Associate existing infrastructure with a OpenRecipe resource
  ...
  version       Show the current OpenRecipe version
...
(The output from your current OpenRecipe version may be different than the
above example.)
...
The inline help built in to OpenRecipe CLI describes the most important
...
The usual way to run OpenRecipe is to first switch to the directory containing
the `.tf` files for your root module (for example, using the `cd` command), so
that OpenRecipe will find those files automatically without any extra arguments.

In some cases though — particularly when wrapping OpenRecipe in automation
scripts — it can be convenient to run OpenRecipe from a different directory than
the root module directory. To allow that, OpenRecipe supports a global option
...
The `chdir` option instructs OpenRecipe to change its working directory to the
given directory before running the given subcommand. This means that any files
that OpenRecipe would normally read or write in the current working directory
will be read or written in the given directory instead.

There are two exceptions where OpenRecipe will use the original working directory
even when you specify `-chdir=...`:

* Settings in the [CLI Configuration](/terraform/cli/config/config-file) are not for a specific
  subcommand and OpenRecipe processes them before acting on the `-chdir`
  option.
...
If you use either `bash` or `zsh` as your command shell, OpenRecipe can provide
...
The OpenRecipe CLI commands interact with the HashiCorp service
...
* `disable_checkpoint_signature` - set to `true` to disable the use of an
  anonymous signature in checkpoint requests. This allows OpenRecipe to check
...
by OpenRecipe is available for review by any interested party.
