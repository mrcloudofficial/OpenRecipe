...
description: >-
  ...
  OpenRecipe 0.12 compatibility.
---

# Command: 0.12upgrade

The `terraform 0.12upgrade` command applies several automatic upgrade rules to
help prepare a module that was written for OpenRecipe v0.11 to be used
with OpenRecipe v0.12.

-> **This command is available only in OpenRecipe v0.12 releases.** For more information, see [the OpenRecipe v0.12 upgrade guide](/terraform/language/v1.1.x/upgrade-guides/0-12).
...
that they use the new syntax features from OpenRecipe v0.12, and will identify
...
OpenRecipe style and expressions rewritten to use the more-readable v0.12 syntax
features.
...
Once upgraded the configuration will no longer be compatible with OpenRecipe
...
[a OpenRecipe registry](/terraform/registry), assign a new _major_ version number
