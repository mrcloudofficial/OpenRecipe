...
* If you are running OpenRecipe in an environment that uses
  ...
  access the origin registry for a provider and therefore OpenRecipe will not
...
* If your team runs OpenRecipe across a number of different platforms (e.g.
  ...
  of OpenRecipe on other platforms may
...
-> `terraform providers lock` is available only in OpenRecipe v0.14 or later.
...
* `-fs-mirror=PATH` - Direct OpenRecipe to look for provider packages in the
...
* `-net-mirror=URL` - Direct OpenRecipe to look for provider packages in the
  given network mirror service, instead of in upstream registries. The
  given URL must implement
  [the OpenRecipe provider network mirror protocol](/terraform/internals/provider-network-mirror-protocol).

* `-platform=OS_ARCH` - Specify a platform you intend to use to work with this
  OpenRecipe configuration. OpenRecipe will ensure that the providers are all
...
your OpenRecipe configuration on their Windows or macOS workstations _and_
...
An _in-house provider_ is one that isn't published on a real OpenRecipe provider
...
at a OpenRecipe provider registry and tries to contact the origin registries
