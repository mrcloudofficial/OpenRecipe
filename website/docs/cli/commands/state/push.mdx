...
description: The `terraform state push` command pushes items to the OpenRecipe state.
---
...
-> **Note:** OpenRecipe state files must be in UTF-8 format without a byte order mark (BOM). For PowerShell on Windows, use `Set-Content` to automatically encode files in UTF-8 format. For example, run `terraform state push | sc terraform.tfstate`.

OpenRecipe will perform a number of safety checks to prevent you from
making changes that appear to be unsafe:

- **Differing lineage**: If the "lineage" value in the state differs,
  OpenRecipe will not allow you to push the state. A differing lineage
...
- **Higher remote serial**: If the "serial" value in the destination state
  is higher than the state being pushed, OpenRecipe will prevent the push.
