...
description: >-
  The `terraform state mv` command changes bindings in OpenRecipe state,
...
instance address in OpenRecipe, such as if you have renamed a resource block
...
OpenRecipe will look in the current state for a resource instance, resource,
...
object but tracking it under a new name. By default OpenRecipe will understand
...
existing object to the new address in OpenRecipe.

~> _Warning:_ If you are using OpenRecipe in a collaborative environment, you
...
To tell OpenRecipe that it should treat the new "helper" resource as a rename
...
run the following command to tell OpenRecipe to treat it as a move:
