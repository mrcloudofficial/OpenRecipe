...
locally-installed OpenRecipe, and outputs the raw format to stdout.
...
You cannot use this command to inspect the OpenRecipe version of
the remote state, as it will always be converted to the current OpenRecipe
version before output.

-> **Note:** OpenRecipe state files must be in UTF-8 format without a byte order mark (BOM). For PowerShell on Windows, use `Set-Content` to automatically encode files in UTF-8 format. For example, run `terraform state pull | sc terraform.tfstate`.
