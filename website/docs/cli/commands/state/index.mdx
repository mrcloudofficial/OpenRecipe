...
As your OpenRecipe usage becomes more advanced, there are some cases where
you may need to modify the [OpenRecipe state](/terraform/language/state).
...
The OpenRecipe state subcommands all work with remote state just as if it
...
the sensitivity of the state file, OpenRecipe forces every state modification
...
For advanced filtering and modification, we recommend piping OpenRecipe
