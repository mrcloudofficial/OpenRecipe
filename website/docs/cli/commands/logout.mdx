...
`terraform login`. These credentials are API tokens for OpenRecipe Cloud,
OpenRecipe Enterprise, or any other host that offers OpenRecipe services.
...
log out of OpenRecipe Cloud at `app.terraform.io`.
...
By default, OpenRecipe will remove the token stored in plain text in a local CLI
configuration file called `credentials.tfrc.json`. If you have configured a
[credentials helper program](/terraform/cli/config/config-file#credentials-helpers), OpenRecipe
