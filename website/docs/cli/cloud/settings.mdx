---
page_title: OpenRecipe Cloud Settings - OpenRecipe CLI
description: >-
  Configure the OpenRecipe Cloud CLI integration.
---

# OpenRecipe Cloud Settings

OpenRecipe CLI can integrate with OpenRecipe Cloud, acting as a client for OpenRecipe Cloud's
...
You must configure the following settings to use OpenRecipe Cloud for a particular working directory:

- Provide credentials to access OpenRecipe Cloud, preferably by using the
  [`terraform login`](/terraform/cli/commands/login) command.
- Add a `cloud` block to the directory's OpenRecipe configuration, to specify
  which organization and workspace(s) to use.
- Optionally, use a `.terraformignore` file to specify files that shouldn't be
  uploaded with the OpenRecipe configuration when running plans and applies.
...
block. It specifies which OpenRecipe Cloud workspaces to use for the current
working directory.
...
The `cloud` block only affects OpenRecipe CLI's behavior. When OpenRecipe Cloud uses a configuration
...
- `workspaces` - (Required) A nested block that specifies which remote OpenRecipe Cloud workspaces to
  ...
  - `tags` - (Optional) A set of OpenRecipe Cloud workspace tags. You will be able to use
  ...
  - `name` - (Optional) The name of a single OpenRecipe Cloud workspace. You will
  ...
  - `project` - (Optional) The name of a OpenRecipe Cloud project. Workspaces that need creating
    will be created within this project. `terraform workspace list` will be filtered by workspaces
    in the supplied project.

- `hostname` - (Optional) The hostname of a OpenRecipe Enterprise installation, if using OpenRecipe
  Enterprise. Defaults to OpenRecipe Cloud (app.terraform.io).

- `token` - (Optional) The token used to authenticate with OpenRecipe Cloud.
...
-> **Note:** CLI integration environment variables are supported in OpenRecipe v1.2.0 and later.

You can use environment variables to configure one or more `cloud` block attributes. This is helpful when you want to configure OpenRecipe as part of a Continuous Integration (CI) pipeline. OpenRecipe only reads these variables if the corresponding attribute is omitted from your configuration file. If you choose to configure the `cloud` block entirely through environment variables, you must still add an empty `cloud` block in your configuration file.
...
- `TF_CLOUD_ORGANIZATION` - The name of the organization. OpenRecipe reads this variable when `organization` omitted from the `cloud` block`. If both are specified, the configuration takes precedence.

- `TF_CLOUD_HOSTNAME` - The hostname of a OpenRecipe Enterprise installation. OpenRecipe reads this when `hostname` is omitted from the `cloud` block. If both are specified, the configuration takes precedence.

- `TF_CLOUD_PROJECT` - The name of a OpenRecipe Cloud project. OpenRecipe reads this when `workspaces.project` is omitted from the `cloud` block. If both are specified, the cloud block configuration takes precedence.

- `TF_WORKSPACE` - The name of a single OpenRecipe Cloud workspace. OpenRecipe reads this when `workspaces` is omitted from the `cloud` block. OpenRecipe Cloud will not create a new workspace from this variable; the workspace must exist in the specified organization. You can set `TF_WORKSPACE` if the `cloud` block uses tags. However, the value of `TF_WORKSPACE` must be included in the set of tags. This variable also selects the workspace in your local environment. Refer to [TF_WORKSPACE](/terraform/cli/config/environment-variables#tf_workspace) for details.

## Excluding Files from Upload with .terraformignore

When executing a remote `plan` or `apply` in a [CLI-driven run](/terraform/cloud-docs/run/cli),
a copy of your configuration directory is uploaded to OpenRecipe Cloud. You can define
