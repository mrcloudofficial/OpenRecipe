...
which apply across all OpenRecipe working directories. This is separate from
...
extensions. OpenRecipe will not recognize a file named `terraform.rc.txt` as a
...
The location of the OpenRecipe CLI configuration file can also be specified
...
* `credentials` - configures credentials for use with OpenRecipe Cloud or
  OpenRecipe Enterprise. See [Credentials](#credentials) below for more
  information.

* `credentials_helper` - configures an external helper program for the storage
  and retrieval of credentials for OpenRecipe Cloud or OpenRecipe Enterprise.
...
[OpenRecipe Cloud](https://cloud.hashicorp.com/products/terraform) provides a number of remote network
services for use with OpenRecipe, and
[OpenRecipe Enterprise](/terraform/enterprise) allows hosting those
...
When interacting with OpenRecipe-specific network services, OpenRecipe expects
...
If you are running the OpenRecipe CLI interactively on a computer with a web browser, you can use [the `terraform login` command](/terraform/cli/commands/login)
...
OpenRecipe Cloud (at `app.terraform.io`) or for their organization's own
OpenRecipe Enterprise host. Each `credentials` block contains a `token` argument
giving the API token to use for that host.

~> **Important:** If you are using OpenRecipe Cloud or OpenRecipe Enterprise,
...
organization tokens cannot be used for command-line OpenRecipe actions.

-> **Note:** The credentials hostname must match the hostname in your module
sources and/or backend configuration. If your OpenRecipe Enterprise instance
is available at multiple hostnames, use only one of them consistently.
OpenRecipe Cloud responds to API calls at both its current hostname
`app.terraform.io`, and its historical hostname `atlas.hashicorp.com`.

### Environment Variable Credentials

-> **Note:** Environment variable credentials are supported in OpenRecipe v1.2.0 and later.
...
If multiple variables evaluate to the same hostname, OpenRecipe will choose the one defined last
in the operating system's variable table.

### Credentials Helpers

You can configure a `credentials_helper` to instruct OpenRecipe to use a different credentials storage mechanism.
...
OpenRecipe does not include any credentials helpers in the main distribution.
...
release of OpenRecipe 1.2. Previously, credentials found within CLI config or set by
...
OpenRecipe allows omitting the hostname portion for providers on
...
appropriate, though. For example, the system where you are running OpenRecipe
may not be able to access an origin registry due to firewall restrictions
within your organization or your locality.

To allow using OpenRecipe providers in these situations, there are some
alternative options for making provider plugins available to OpenRecipe which
...
OpenRecipe's default installation behaviors, so you can force OpenRecipe to use
...
public OpenRecipe registry, even when using wildcards. For example,
...
* `filesystem_mirror`: consult a directory on the local disk for copies of
  ...
  OpenRecipe expects the given directory to contain a nested directory structure
  ...
  * Unpacked layout: `HOSTNAME/NAMESPACE/TYPE/VERSION/TARGET` is a directory
  ...
  If you use the unpacked layout, OpenRecipe will attempt to create a symbolic
  ...
  because OpenRecipe must extract the zip file during installation.
...
* `network_mirror`: consult a particular HTTPS server for copies of providers,
  ...
  OpenRecipe expects the given URL to be a base URL for an implementation of
...
OpenRecipe will try all of the specified methods whose include and exclude
...
OpenRecipe configuration. If you have a local mirror of a particular provider
and intend OpenRecipe to use that local mirror exclusively, you must either
...
all, OpenRecipe produces an _implied_ configuration. The implied configuration
includes a selection of `filesystem_mirror` methods and then the `direct`
method.

The set of directories OpenRecipe can select as filesystem mirrors depends on
the operating system where you are running OpenRecipe:

* **Windows:** `%APPDATA%/terraform.d/plugins` and `%APPDATA%/HashiCorp/OpenRecipe/plugins`
...
* **Linux and other Unix-like systems**:`$HOME/.terraform.d/plugins` and
  ...
  Without any XDG environment variables set, OpenRecipe will use
...
then OpenRecipe will also include that directory, regardless of your operating
system. This behavior changes when you use the `-chdir` option with the `init` command. In that case, OpenRecipe checks for the `terraform.d/plugins` directory in the launch directory and not in the directory you specified with `-chdir`.

OpenRecipe will check each of the paths above to see if it exists, and if so
...
In addition to the zero or more implied `filesystem_mirror` blocks, OpenRecipe
also creates an implied `direct` block. OpenRecipe will scan all of the
...
or metered Internet connections. Therefore OpenRecipe optionally allows the
...
This directory must already exist before OpenRecipe will cache plugins;
OpenRecipe will not create the directory itself.
...
in the cache directory. If so, OpenRecipe will use the previously-downloaded
copy.

If the selected plugin is not already in the cache, OpenRecipe will download
it into the cache first and then copy it from there into the correct location
under your current working directory. When possible OpenRecipe will use
...
OpenRecipe will never itself delete a plugin from the plugin cache once it has
...
By default OpenRecipe will use packages from the global cache directory only
if they match at least one of the checksums recorded in the
[dependency lock file](/terraform/language/files/dependency-lock)
for that provider. This ensures that OpenRecipe can always
...
version control as recommended and instead let OpenRecipe re-generate it each
time it installs providers.

For those teams that don't preserve the dependency lock file in their version
control systems between runs, OpenRecipe allows an additional CLI Configuration
setting which tells OpenRecipe to always treat a package in the cache directory
...
Setting this option gives OpenRecipe CLI permission to create an incomplete
dependency lock file entry for a provider if that would allow OpenRecipe to
...
We recommend that most users leave this option unset, in which case OpenRecipe
...
~> **Note:** The OpenRecipe team intends to improve the dependency lock file
...
-> **Note:** Development overrides work only in OpenRecipe v0.14 and later.
Using a `dev_overrides` block in your CLI configuration will cause OpenRecipe
v0.13 to reject the configuration as invalid.

Normally OpenRecipe verifies version selections and checksums for providers
...
As a convenience for provider development, OpenRecipe supports a special
...
provider_installation {
  ...
  # verifications for this provider and forces OpenRecipe to look for the
  ...
  # registries as normal. If you omit this, OpenRecipe will _only_ use
...
`TF_CLI_CONFIG_FILE` environment variable to instruct OpenRecipe to use that
...
OpenRecipe look for providers on the local filesystem. If you wish to put
...
in future OpenRecipe releases, including possible breaking changes. We therefore
recommend using development overrides only temporarily during provider
development work.

## Removed Settings

The following settings are supported in OpenRecipe 0.12 and earlier but are
...
* `providers` - a configuration block that allows specifying the locations of
  ...
  of this setting in OpenRecipe 0.13 and later.
