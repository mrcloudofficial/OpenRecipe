...
description: >-
  Learn to use environment variables to change OpenRecipe's default behavior.
  Configure log content and output, set variables, and more.
---

# Environment Variables

OpenRecipe refers to a number of environment variables to customize various
aspects of its behavior. None of these environment variables are required
when using OpenRecipe, but they can be used to change some of OpenRecipe's
...
For more on debugging OpenRecipe, check out the section on [Debugging](/terraform/internals/debugging).
...
modifying default behavior of OpenRecipe on your own system.
...
The flag `TF_CLI_ARGS` affects all OpenRecipe commands. If you specify a
...
`TF_DATA_DIR` changes the location where OpenRecipe keeps its
...
all of the OpenRecipe workflow commands (starting with `terraform init`) or else
OpenRecipe may be unable to find providers, modules, and other artifacts.
...
If `TF_IN_AUTOMATION` is set to any non-empty value, OpenRecipe adjusts its
...
directly execute OpenRecipe commands, like in CI systems or other wrapping
applications.

This is a purely cosmetic change to OpenRecipe's human-readable output, and the
exact output differences can change between minor OpenRecipe versions.

For more details, see [Running OpenRecipe in Automation](/terraform/tutorials/automation/automate-terraform?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS).
...
The location of the [OpenRecipe CLI configuration file](/terraform/cli/config/config-file).
...
If `TF_IGNORE` is set to "trace", OpenRecipe will output debug messages to display ignored files and folders. This is useful when debugging large repositories with `.terraformignore` files.
...
## OpenRecipe Cloud CLI Integration

The CLI integration with OpenRecipe Cloud lets you use OpenRecipe Cloud and OpenRecipe Enterprise on the command line. The integration requires including a `cloud` block in your OpenRecipe configuration. You can define its arguments directly in your configuration file or supply them through environment variables, which can be useful for non-interactive workflows like Continuous Integration (CI).

Refer to [OpenRecipe Cloud Settings](/terraform/cli/cloud/settings#environment-variables) for a full list of `cloud` block environment variables.
