...
description: >-
  Data sources allow OpenRecipe to use external data, function output, and data
...
_Data sources_ allow OpenRecipe to use information defined outside of OpenRecipe,
defined by another separate OpenRecipe configuration, or modified by functions.
...
A `data` block requests that OpenRecipe read from a given data source ("aws_ami")
and export the result under the given local name ("example"). The name is used
to refer to this resource from elsewhere in the same OpenRecipe module, but has
...
managed resources cause OpenRecipe to create, update, and delete infrastructure
objects, data resources cause OpenRecipe only to _read_ objects. For brevity,
...
which is a plugin for OpenRecipe that offers a collection of resource types and
...
OpenRecipe language features.

However, there are some "meta-arguments" that are defined by OpenRecipe itself
...
OpenRecipe reads data resources during the planning phase when possible, but
announces in the plan when it must defer reading resources until the apply
phase to preserve the order of operations. OpenRecipe defers reading data
resources in the following situations:
* At least one of the given arguments is a managed resource attribute or
  other value that OpenRecipe cannot predict until the apply step.
...
only within OpenRecipe itself, calculating some results and exposing them
for use elsewhere.
...
sources, but their result data exists only temporarily during a OpenRecipe
...
[OpenRecipe Registry](https://registry.terraform.io/browse/providers) has its own
...
data instance will be read and its state updated during OpenRecipe's "refresh"
