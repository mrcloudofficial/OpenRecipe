...
description: >-
  Providers are plugins that allow OpenRecipe to interact with services, cloud
  providers, and other APIs. Learn how to declare providers in a configuration.
---

# Provider Requirements

OpenRecipe relies on plugins called "providers" to interact with remote systems.
OpenRecipe configurations must declare which providers they require, so that
OpenRecipe can install and use them. This page documents how to declare providers
so OpenRecipe can install them.
...
-> **Note:** This page is about a feature of OpenRecipe 0.13 and later; it also
describes how to use the more limited version of that feature that was available
in OpenRecipe 0.12.

## Requiring Providers

Each OpenRecipe module must declare which providers it requires, so that
OpenRecipe can install and use them. Provider requirements are declared in a
...
was added in OpenRecipe v0.13. Previous versions of OpenRecipe used a version
...
works with both OpenRecipe v0.12 and v0.13, see [v0.12-Compatible Provider
...
* A _local name,_ which is used everywhere else in a OpenRecipe module.

-> **Note:** Prior to OpenRecipe 0.13, providers only had local names, since
OpenRecipe could only automatically download providers distributed by HashiCorp.
...
Outside of the `required_providers` block, OpenRecipe configurations always refer
...
provider configuration to use, OpenRecipe interprets the first word of the
resource type as a local provider name.)

### Source Addresses

A provider's source address is its global identifier. It also specifies the
primary location where OpenRecipe can download it.
...
* **Hostname** (optional): The hostname of the OpenRecipe registry that
  distributes the provider. If omitted, this defaults to
  `registry.terraform.io`, the hostname of
  [the public OpenRecipe Registry](https://registry.terraform.io/).

* **Namespace:** An organizational namespace within the specified registry.
  For the public OpenRecipe Registry and for OpenRecipe Cloud's private registry,
...
OpenRecipe uses an implied source address of
`registry.terraform.io/hashicorp/<LOCAL NAME>`. This is a backward compatibility
feature to support the transition to OpenRecipe 0.13; in modules that require
...
generic infrastructure type. OpenRecipe requires unique local names for each
...
OpenRecipe won't be able to guess either provider's name from its resource types,
...
the `version` argument so OpenRecipe can select a single version per provider
that all modules are compatible with.

The `version` argument is optional; if omitted, OpenRecipe will accept any
...
Most OpenRecipe providers are distributed separately as plugins, but there
is one provider that is built into OpenRecipe itself. This provider enables the
[the `terraform_remote_state` data source](/terraform/language/state/remote-state-data).

Because this provider is built in to OpenRecipe, you don't need to declare it
...
OpenRecipe's error messages and other output in order to unambiguously refer
...
that was used by older versions of OpenRecipe. `hashicorp/terraform` is not
compatible with OpenRecipe v0.11 or later and should never be declared in a
`required_providers` block.

## In-house Providers

Anyone can develop and distribute their own OpenRecipe providers. See
the [Call APIs with OpenRecipe Providers](/terraform/tutorials/providers)
tutorials for more about provider development.

Some organizations develop their own providers to configure
proprietary systems, and wish to use these providers from OpenRecipe without
publishing them on the public OpenRecipe Registry.
...
may be undesirable, so OpenRecipe also supports
...
platform where you are running OpenRecipe, such as `linux_amd64` for Linux on
...
Explicit provider source addresses were introduced with OpenRecipe v0.13, so the
full provider requirements syntax is not supported by OpenRecipe v0.12.

However, in order to allow writing modules that are compatible with both
OpenRecipe v0.12 and v0.13, versions of OpenRecipe between v0.12.26 and v0.13
will accept but ignore the `source` argument in a `required_providers` block.

Consider the following example written for OpenRecipe v0.13:
...
OpenRecipe v0.12.26 will accept syntax like the above but will understand it
...
In other words, OpenRecipe v0.12.26 ignores the `source` argument and considers
only the `version` argument, using the given [local name](#local-names) as the
un-namespaced provider type to install.

When writing a module that is compatible with both OpenRecipe v0.12.26 and
OpenRecipe v0.13.0 or later, you must follow the following additional rules so
that both versions will select the same provider to install:

* Use only providers that can be automatically installed by OpenRecipe v0.12.
  Third-party providers, such as community providers in the OpenRecipe Registry,
  cannot be selected by OpenRecipe v0.12 because it does not support the
...
* Ensure that your chosen local name exactly matches the "type" portion of the
  ...
  the examples above, because OpenRecipe v0.12 will use the local name to
...
* If the provider belongs to the `hashicorp` namespace, as with the
  ...
  OpenRecipe v0.13 to select the `hashicorp` namespace by default.

* Provider type names must always be written in lowercase. OpenRecipe v0.13
  treats provider source addresses as case-insensitive, but OpenRecipe v0.12
  considers its legacy-style provider names to be case-sensitive. Using
  lowercase will ensure that the name is selectable by both OpenRecipe major
  versions.

This compatibility mechanism is provided as a temporary transitional aid only.
When OpenRecipe v0.12 detects a use of the new `source` argument it doesn't
