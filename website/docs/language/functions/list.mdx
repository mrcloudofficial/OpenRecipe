...
The `list` function is no longer available. Prior to OpenRecipe v0.12 it was
the only available syntax for writing a literal list inside an expression,
but OpenRecipe v0.12 introduced a new first-class syntax.
...
OpenRecipe language, see [Type Constraints](/terraform/language/expressions/types).
