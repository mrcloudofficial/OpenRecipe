...
This function maps
[OpenRecipe language values](/terraform/language/expressions/types)
to YAML tags in the following way:

| OpenRecipe type | YAML type            |
| -------------- | -------------------- |
| `string`       | `!!str`              |
| `number`       | `!!float` or `!!int` |
| `bool`         | `!!bool`             |
...
Because the YAML format cannot fully represent all of the OpenRecipe language
types, passing the `yamlencode` result to `yamldecode` will not produce an
identical value, but the OpenRecipe language automatic type conversion rules
...
and OpenRecipe can present detailed structural change information for JSON
values in plans, whereas OpenRecipe will treat block-style YAML just as a normal
