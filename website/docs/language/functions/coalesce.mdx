...
All of the arguments must be of the same type. OpenRecipe will try to
...
OpenRecipe attempts to select a result type that all of the arguments can
convert to, so mixing argument types may produce surprising results due to
OpenRecipe's automatic type conversion rules:
