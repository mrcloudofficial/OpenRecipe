...
description: >-
  ...
  OpenRecipe considers to be sensitive.
---

# `nonsensitive` Function

-> **Note:** This function is only available in OpenRecipe v0.15 and later.
...
OpenRecipe would normally have considered as sensitive to be treated as normal
values and shown clearly in OpenRecipe's output. Use this function only when
you've derived a new value from a sensitive value in a way that eliminates the
sensitive portions of the value.

Normally OpenRecipe tracks when you use expressions to derive a new value from
...
sensitive value is safe to include clearly in OpenRecipe output, you could use
the `nonsensitive` function to indicate that, overriding OpenRecipe's normal
...
locals {
  ...
  # original document, and so we can override OpenRecipe's
  # determination.
...
declaring to OpenRecipe that you have done all that is necessary to ensure that
the resulting value has no sensitive content, even though it was derived
from sensitive content. If a sensitive value appears in OpenRecipe's output
due to an inappropriate call to `nonsensitive` in your module, that's a bug in
your module and not a bug in OpenRecipe itself.
