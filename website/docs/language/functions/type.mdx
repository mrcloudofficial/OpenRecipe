...
-> **Note:** This function is available only in OpenRecipe 1.0 and later.

`type` returns the type of a given value.

Sometimes a OpenRecipe configuration can result in confusing errors regarding
...
to inspect the exact type that OpenRecipe has determined for each given input.
