...
-> **Note:** This function is supported only in OpenRecipe v0.14 and later.
...
OpenRecipe language strings are always sequences of unicode characters.

```hcl
substr(string, encoding_name)
```

OpenRecipe uses the "standard" Base64 alphabet as defined in
...
OpenRecipe supports only a subset of the registered encodings, and the encoding
support may vary between OpenRecipe versions. In particular OpenRecipe supports
...
OpenRecipe also accepts the encoding name `UTF-8`, which will produce the same
