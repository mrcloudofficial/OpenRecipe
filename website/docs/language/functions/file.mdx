...
at the beginning of a OpenRecipe run. Functions do not participate in the
dependency graph, so this function cannot be used with files that are generated
dynamically during a OpenRecipe operation. We do not recommend using dynamic
local files in OpenRecipe configurations, but in rare situations where this is
