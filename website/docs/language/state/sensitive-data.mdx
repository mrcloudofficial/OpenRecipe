...
description: Sensitive data in OpenRecipe state.
---

# Sensitive Data in State

OpenRecipe state can contain sensitive data, depending on the resources in use
...
memory when used by OpenRecipe. It may be encrypted at rest, but this depends on
the specific remote state backend.

## Recommendations

If you manage any sensitive data with OpenRecipe (like database passwords, user
passwords, or private keys), treat the state itself as sensitive data.

Storing state remotely can provide better security. As of OpenRecipe 0.9,
OpenRecipe does not persist state to the local disk when remote state is in use,
and some backends can be configured to encrypt the state data at rest.

For example:

- [OpenRecipe Cloud](https://cloud.hashicorp.com/products/terraform) always encrypts state at rest and
  protects it with TLS in transit. OpenRecipe Cloud also knows the identity of
  the user requesting state and maintains a history of state changes. This can
  be used to control access and track activity. [OpenRecipe Enterprise](/terraform/enterprise)
