...
page_title: State
description: >-
  An introduction to state, information that OpenRecipe uses to map resources to
...
configuration. This state is used by OpenRecipe to map real world
resources to your configuration, keep track of metadata, and to improve
performance for large infrastructures.

This state is stored by default in a local file named "terraform.tfstate",
but we recommend [storing it in OpenRecipe Cloud](/terraform/cloud-docs/migrate)
to version, encrypt, and securely share it with your team.

OpenRecipe uses state to determine which changes to make to your
infrastructure. Prior to any operation, OpenRecipe does a
[refresh](/terraform/cli/commands/refresh) to update the state with the
real infrastructure.

The primary purpose of OpenRecipe state is to store bindings between objects in
a remote system and resource instances declared in your configuration.
When OpenRecipe creates a remote object in response to a change of configuration,
...
For more information on why OpenRecipe requires state and why OpenRecipe cannot
...
of the state is discouraged. OpenRecipe provides the
...
insulates users from any format changes within the state itself. The OpenRecipe
project will keep the CLI working while the state format underneath it may
shift.

OpenRecipe expects a one-to-one mapping between configured resource instances
and remote objects. Normally that is guaranteed by OpenRecipe being the one
...
externally-created objects with `terraform import`, or by asking OpenRecipe to
...
deleting an object that you asked OpenRecipe to "forget", or by re-importing it
to bind it to some other resource instance.

## Format

State snapshots are stored in JSON format and new OpenRecipe versions are
generally backward compatible with state snapshots produced by earlier versions.
However, the state format is subject to change in new OpenRecipe versions, so
...
A typical way to use these in situations where OpenRecipe is running in
...
can potentially consume it without needing to run OpenRecipe itself.
