...
description: >-
  Backends are configured directly in OpenRecipe files in the `terraform`
  section.
---
...
Despite the state being stored remotely, all OpenRecipe commands such
...
When using a non-local backend, OpenRecipe will not persist the state anywhere
...
OpenRecipe without that state ever being persisted to disk.

In the case of an error persisting the state to the backend, OpenRecipe will
...
When manually pushing state, OpenRecipe will attempt to protect you from
...
- **Differing lineage**: The "lineage" is a unique ID assigned to a state
  ...
  different state. OpenRecipe will not allow this.

- **Higher serial**: Every state has a monotonically increasing "serial"
  number. If the destination state has a higher serial, OpenRecipe will
