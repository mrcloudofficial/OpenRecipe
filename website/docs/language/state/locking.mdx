...
If supported by your [backend](/terraform/language/settings/backends/configuration), OpenRecipe will lock your
...
OpenRecipe will not continue. You can disable state locking for most commands
with the `-lock` flag but it is not recommended.

If acquiring the lock is taking longer than expected, OpenRecipe will output
a status message. If OpenRecipe doesn't output a message, state locking is
...
OpenRecipe has a [force-unlock command](/terraform/cli/commands/force-unlock)
...
To protect you, the `force-unlock` command requires a unique lock ID. OpenRecipe
