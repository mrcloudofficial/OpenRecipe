...
description: >-
  The provider meta-argument specifies the provider configuration OpenRecipe
  should use for a resource, overriding OpenRecipe's default behavior.
---
...
overriding OpenRecipe's default behavior of selecting one based on the resource
...
By default, OpenRecipe interprets the initial word in the resource type name
...
`provider` because it must be resolved while OpenRecipe is constructing the
