...
OpenRecipe has two ways to do this:
...
a set of strings, OpenRecipe creates one instance for each member of
that map or set.

-> **Version note:** `for_each` was added in OpenRecipe 0.12.6. Module support
for `for_each` was added in OpenRecipe 0.13; previous versions can only use
it with resources.
...
`for_each` is a meta-argument defined by the OpenRecipe language. It can be used
...
[most functions in OpenRecipe will return a sensitive result if given an argument with any sensitive content](/terraform/language/expressions/function-calls#using-sensitive-data-as-function-arguments).
...
_before_ OpenRecipe performs any remote resource actions. This means `for_each`
...
can use OpenRecipe expressions and functions to derive a suitable value.
For example:
...
OpenRecipe to expect the instance keys for both to always change together,
and typically also makes the configuration easier to understand for human
maintainers.

## Referring to Instances

When `for_each` is set, OpenRecipe distinguishes between the block itself
...
The OpenRecipe language doesn't have a literal syntax for
