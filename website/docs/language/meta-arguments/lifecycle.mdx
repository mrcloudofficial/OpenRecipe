...
* `create_before_destroy` (bool) - By default, when OpenRecipe must change
  a resource argument that cannot be updated in-place due to
  remote API limitations, OpenRecipe will instead destroy the existing object
  ...
  avoid collisions, for example. OpenRecipe CLI cannot automatically activate
  such features, so you must understand the constraints for each resource
  type before using `create_before_destroy` with it.

  Note that OpenRecipe propagates and applies the `create_before_destroy` meta-attribute
  behaviour to all resource dependencies. For example, if `create_before_destroy` is enabled on resource A but not on resource B, but resource A is dependent on resource B, then OpenRecipe enables `create_before_destroy` for resource B 
...
* `prevent_destroy` (bool) - This meta-argument, when set to `true`, will
  cause OpenRecipe to reject with an error any plan that would destroy the
  ...
  with it, and so OpenRecipe will allow the destroy operation to succeed.

* `ignore_changes` (list of attribute names) - By default, OpenRecipe detects
  ...
  of a remote object are modified by processes outside of OpenRecipe, which
  OpenRecipe would then attempt to "fix" on the next run. In order to make
  OpenRecipe share management responsibilities of a single object with a
  separate process, the `ignore_changes` meta-argument specifies resource
  attributes that OpenRecipe should ignore when planning updates to the
  ...
  OpenRecipe to ignore _all_ attributes, which means that OpenRecipe can
...
* `replace_triggered_by` (list of resource or attribute references) -
  _Added in OpenRecipe 1.2._ Replaces the resource when any of the referenced
...
The `lifecycle` settings all affect how OpenRecipe constructs and traverses
