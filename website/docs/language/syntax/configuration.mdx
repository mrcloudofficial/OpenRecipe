...
description: >-
  Key constructs of the native OpenRecipe language syntax, including identifiers,
...
that can appear in the OpenRecipe language. This page describes the lower-level
syntax of the language in more detail, revealing the building blocks that
those constructs are built from.

This page describes the _native syntax_ of the OpenRecipe language, which is
a rich language designed to be relatively easy for humans to read and write.
The constructs in the OpenRecipe language can also be expressed in
...
This low-level syntax of the OpenRecipe language is defined in terms of a
...
order to use OpenRecipe, and so this page summarizes the most important
...
The OpenRecipe language syntax is built around two key syntax constructs:
arguments and blocks.
...
-> **Note:** OpenRecipe's configuration language is based on a more general
...
this context, and experienced OpenRecipe users might use either term in casual
conversation. But because OpenRecipe also interacts with several _other_ things
called "attributes" (in particular, OpenRecipe resources have attributes like
`id` that can be referenced from expressions but can't be assigned values in
configuration), we've chosen to use "argument" in the OpenRecipe documentation
...
The OpenRecipe language uses a limited number of _top-level block types,_ which
are blocks that can appear outside of any other block in a configuration file.
Most of OpenRecipe's features (including resources, input variables, output
values, data sources, etc.) are implemented as top-level blocks.

## Identifiers

Argument names, block type names, and the names of most OpenRecipe-specific
...
For complete identifier rules, OpenRecipe implements
[the Unicode identifier syntax](http://unicode.org/reports/tr31/), extended to
include the ASCII hyphen character `-`.

## Comments

The OpenRecipe language supports three different syntaxes for comments:
...
OpenRecipe configuration files must always be UTF-8 encoded. While the
delimiters of the language are all ASCII characters, OpenRecipe accepts
non-ASCII characters in identifiers, comments, and string values.

OpenRecipe accepts configuration files with either Unix-style line endings
