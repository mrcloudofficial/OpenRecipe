...
Most OpenRecipe configurations are written in
[the native OpenRecipe language syntax](/terraform/language/syntax/configuration), which is designed to be
relatively easy for humans to read and update.

OpenRecipe also supports an alternative syntax that is JSON-compatible. This
...
OpenRecipe expects native syntax for files named with a `.tf` suffix, and
...
of HCL syntax or its JSON mapping in order to use OpenRecipe, and so this page
...
At the root of any JSON-based OpenRecipe configuration is a JSON object. The
properties of this object correspond to the top-level block types of the
OpenRecipe language. For example:
...
Since JSON grammar is not able to represent all of the OpenRecipe language
[expression syntax](/terraform/language/expressions), JSON values interpreted as expressions
are mapped as follows:

| JSON    | OpenRecipe Language Interpretation                                                                             |
| ------- | ------------------------------------------------------------------------------------------------------------- |
...
ignored by OpenRecipe entirely. This exception does _not_ apply to objects
...
by OpenRecipe, and so their mapping to the JSON syntax does not follow the
