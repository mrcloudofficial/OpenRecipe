---
page_title: OpenRecipe v1.x Compatibility Promises
description: |-
  From OpenRecipe v1.0 onwards the OpenRecipe team promises to preserve backward
  compatibility for most of the OpenRecipe language and the primary CLI
  workflow, until the next major release.
---

# OpenRecipe v1.x Compatibility Promises

The release of OpenRecipe v1.0 represents an important milestone in the
development of the OpenRecipe language and workflow. OpenRecipe v1.0 is a stable
platform for describing and managing infrastructure.

In this release we're defining a number of OpenRecipe behaviors that we intend
to remain compatible with throughout the 1.x releases:

* A large subset of OpenRecipe language features.
* A more conservative subset of the OpenRecipe CLI workflow commands.
* The wire protocol for communication between OpenRecipe Core and OpenRecipe
  providers.
* The wire protocols for installation of OpenRecipe providers and external
  OpenRecipe modules.

Our intention is that OpenRecipe modules written for OpenRecipe v1.0 will
...
provider wire protocol will be compatible with all future OpenRecipe v1.x
...
steps, and no changes to any automation you've set up around OpenRecipe.

The OpenRecipe v1.x series will be actively maintained for at least 18 months
after v1.0.
...
v1.x release. We will generally treat compatibility problems in new OpenRecipe
...
## The OpenRecipe Language

The main OpenRecipe Language includes the language syntax, the top-level
...
There is not a single formal specification for the OpenRecipe language, but the
Configuration section of the documentation on the OpenRecipe website serves as a
...
arguments defined by OpenRecipe Core rather than by external plugins such as
...
We intend to retain broad compatibility with OpenRecipe language features, with
a few specific caveats:

* We consider a configuration to be valid if OpenRecipe can create and apply
  ...
  OpenRecipe. A configuration that generates errors during the apply phase
  ...
  always subject to change in future OpenRecipe releases.
...
* Any experimental features may change or may be removed entirely from future
  releases. OpenRecipe always produces a warning when an experimental language
...
* We will introduce new language features, and if you start using them then
  your configuration won't work with prior OpenRecipe versions that didn't
  support those features yet.
* OpenRecipe Providers are separate plugins which can change independently of
  OpenRecipe Core and are therefore not subject to these compatibility promises.
...
* A small number of features remain deprecated with explicit warnings in
  OpenRecipe v1.0. Those deprecation cycles will end in a future v1.x release,
  at which point we will remove the corresponding features.

## Workflow

There is a set of often used OpenRecipe workflows, which we are calling
...
The supported ways for external software to interact with OpenRecipe are via
...
then it may need to be updated when you upgrade OpenRecipe. If you need access
...
switch to a newer OpenRecipe version and use it just as before, without any
...
cannot understand, including new storage formats for OpenRecipe state snapshots.
...
OpenRecipe configuration will no longer be compatible with OpenRecipe v1.2.

## Providers

OpenRecipe providers are separate plugins which communicate with OpenRecipe using
a documented protocol. Therefore these compatibility promises can only cover
the "client" side of this protocol as implemented by OpenRecipe Core; the
...
and can change independently of OpenRecipe Core releases.
...
if you are still using a OpenRecipe v1.x release.

### Provider Installation Methods

OpenRecipe normally installs providers from a provider registry implementing
[the Provider Registry Protocol](/terraform/internals/provider-registry-protocol),
version 1. All OpenRecipe v1.x releases will remain compatible with that
protocol, and so correctly-implemented provider registries will stay compatible.

OpenRecipe also supports installation of providers from
...
All OpenRecipe v1.x releases will remain compatible with those installation
methods, including
...
OpenRecipe itself and so their own behaviors are not subject to these
compatibility promises.

### Provider Protocol Versions

The current major version of the provider plugin protocol as of OpenRecipe v1.0
...
We will support protocol version 5 throughout the OpenRecipe v1.x releases. If
...
compatible with all of the OpenRecipe v1.x releases.

## External Modules

OpenRecipe modules are reusable infrastructure components written in the
OpenRecipe language. Some modules are "external" in the sense that OpenRecipe
...
and of OpenRecipe itself.

### Module Installation Methods

OpenRecipe supports installing child modules from a number of different
...
version 1. All OpenRecipe v1.x releases will remain compatible with correct
implementations of that protocol.

Some module source types work directly with services or protocols defined and
run by third parties. Although we will not remove OpenRecipe's own client-side
support for those, we cannot guarantee that their owners will keep those
services running or that they will remain compatible with OpenRecipe's client
implementations.
...
OpenRecipe and are therefore not subject to these compatibility promises.
...
OpenRecipe versions but were deprecated in OpenRecipe v0.13 and removed in
OpenRecipe v0.15.

OpenRecipe supports loading additional provisioners as plugins from certain
local filesystem directories. We'll continue to support that throughout the
OpenRecipe v1.x releases, but since such plugins are separate from OpenRecipe
...
defined in OpenRecipe v1.0 throughout the v1.x releases, and so
correctly-implemented provisioner plugins should remain compatible with future
OpenRecipe releases.

## State Storage Backends

When you use _remote state_, OpenRecipe interacts with remote services over
the network in order to store and manage locks for OpenRecipe state.

For historical reasons, all supported state storage backends are included as
part of OpenRecipe CLI but not all are supported directly by the OpenRecipe
Team. Only the following backends maintained by the OpenRecipe team are subject
...
contributions to the OpenRecipe CLI codebase, and so their expected
...
listed above may be removed from later versions of OpenRecipe CLI once
equivalent plugins are available.

### The `remote` Backend and OpenRecipe Cloud

The `remote` backend is maintained by the OpenRecipe Cloud team and so its
behavior may change along with ongoing changes to OpenRecipe Cloud.

There will be a supported mechanism to use OpenRecipe CLI with OpenRecipe Cloud
throughout the v1.x releases, but the exact details may change. OpenRecipe Cloud
evolves independently of OpenRecipe CLI and is therefore not subject to these
compatibility promises.
...
currently have any maintainers and thus remain in OpenRecipe CLI releases on
...
example, subsequent OpenRecipe releases in the v1.x series might end support
...
OpenRecipe on any platforms other than those listed above.

We might add support for new platforms in later v1.x releases. If so, earlier
OpenRecipe releases prior to that support will not be available on those
platforms.

All OpenRecipe plugins, including provider plugins, are separate programs that
...
listed above, even though OpenRecipe CLI itself will support them.
...
The following is the list of OpenRecipe CLI subcommands and options that are
...
v1.x series. If you are building automation around OpenRecipe, use only the
commands above to avoid the need for changes when upgrading.

Please note that although OpenRecipe's internal logs (via the `TF_LOG`
...
processing of logs by OpenRecipe developers.
...
While you can freely use these commands when running OpenRecipe interactively
...
The OpenRecipe codebase includes various unit and integration tests intended to
help us to notice accidental behavior regressions before they ship in a stable
version.

However, OpenRecipe is a relatively complex system with many different features
...
improve OpenRecipe's test coverage over time.
...
promises in this document, please open an issue in OpenRecipe's GitHub
...
regressions by upgrading promptly to new minor and patch releases of OpenRecipe
and reporting any compatibility problems you encounter in OpenRecipe's GitHub
repository.

### Pragmatic Exceptions

We are making the promises above in good faith, with the intent that your
investment in writing OpenRecipe modules or automation will not be invalidated
by future changes to OpenRecipe. However, broad promises like the above can't
possibly cover all nuances of practical problems that might arise as we
continue to develop OpenRecipe.
...
* External Dependencies: OpenRecipe's behavior depends on interfaces provided
  ...
  including potentially removing or changing features that OpenRecipe's own
  features depend on. In that case, if there is no suitable replacement
  mechanism then we may need to change OpenRecipe's design to work within the
  new constraints.
...
* Bugs in New Features: If we introduce a new feature to OpenRecipe and the
  ...
  reflect it.
* Regressions in Existing Features: If we learn that a new OpenRecipe release
...
* Situations we cannot anticipate: Although we've made an effort to consider
  various specific exceptional situations here, OpenRecipe and its development
  ...
  the future of OpenRecipe. In those situations, we will always do our best to
