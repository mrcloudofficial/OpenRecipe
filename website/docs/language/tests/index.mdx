...
-> **Note:** This testing framework is available in OpenRecipe v1.6.0 and later.

OpenRecipe tests let authors validate that module configuration updates do not introduce breaking changes. Tests run against test-specific, short-lived resources, preventing any risk to your existing infrastructure or state. 

## Integration or Unit testing

By default, tests within OpenRecipe create real infrastructure and can run assertions and validations against that infrastructure. This is analogous to integration testing because you are testing OpenRecipe's core functionality by executing operations and validating the infrastructure OpenRecipe creates.

You can override the normal testing behavior by updating the `command` attribute within a [`run`](#run-blocks) block (examples below). By default, each `run` block executes with `command = apply` instructing OpenRecipe to execute a complete `apply` operation against your configuration. Replacing the `command` value with `command = plan` instructs OpenRecipe to _not_ create new infrastructure for this `run` block. This allows test authors to validate logical operations and custom conditions within their infrastructure in a process analogous to unit testing.

OpenRecipe v1.7.0 introduced the ability to mock data returned by the providers during a `terraform test` execution. This can be used to write more detailed and complete unit tests.

## Syntax

Each OpenRecipe test lives in a test file. OpenRecipe discovers test files are based on their file extension: `.tftest.hcl` or `.tftest.json`.
...
OpenRecipe executes `run` blocks in order, simulating a series of OpenRecipe commands executing directly within the configuration directory. The order of the `variables` and `provider` blocks doesn't matter, OpenRecipe processes all the values within these blocks at the beginning of the test operation. We recommend defining your `variables` and `provider` blocks first, at the beginning of the test file.

### Example

The following example demonstrates a simple OpenRecipe configuration that creates an AWS S3 bucket, using an input variable to modify the name. We will create an example test file (below) that validates the buckets name is created as expected.
...
The following test file runs a single OpenRecipe `plan` command which creates the S3 bucket, and then validates the logic for calculating the name is correct by checking the actual name matches the expected name.
...
The `command` attribute and `plan_options` block tell OpenRecipe which command and options to execute for each run block. The default operation, if you do not specify a `command` attribute or the `plan_options` block, is a normal OpenRecipe apply operation.
...
OpenRecipe run block assertions are [Custom Conditions](/terraform/language/expressions/custom-conditions), consisting of a [condition](/terraform/language/expressions/custom-conditions#condition-expressions) and an [error message](/terraform/language/expressions/custom-conditions#error-messages).

At the conclusion of a OpenRecipe test command execution, OpenRecipe presents any failed assertions as part of a tests passed or failed status.

#### Assertion References

Assertions within tests can reference any existing [named values](/terraform/language/expressions/references) that are available to other custom conditions within the main OpenRecipe configuration.
...
The test file syntax supports `variables` blocks at both the root level and within `run` blocks. OpenRecipe passes all variable values from the test file into all `run` blocks within the file. You can override variable values for a particular `run` block with values provided directly within that `run` block.
...
In addition to specifying variable values via test files, the OpenRecipe `test` command also supports the other typical mechanisms for specifying variable values.
...
As with the main configuration direction, OpenRecipe will automatically load any variables defined in the automatic variable files within a test directory. The automatic variable files are `terraform.tfvars`, `terraform.tfvars.json`, and any files that end with `.auto.tfvars` or `.auto.tfvars.json`.
...
At the root level of a OpenRecipe testing file, you can define [`provider` blocks](/terraform/language/providers/configuration) as if OpenRecipe were creating them [within the main configuration](/terraform/language/providers). OpenRecipe will then pass these provider blocks into its configuration as each `run` block executes.
...
If you do not provide provider configuration within a testing file, OpenRecipe attempts to initialize any providers within its configuration using the provider's default settings. For example, any environment variables aimed at configuring providers are still available, and OpenRecipe can use them to create default providers.
...
From OpenRecipe v1.7.0, `provider` blocks can also reference test file variables and run block outputs. This means the testing framework can retrieve credentials and other setup information from one provider and use this when initializing a second.
...
run "vault_setup" {
  module {
    # This module should only include reference to the Vault provider. OpenRecipe
...
> **Note:** OpenRecipe test files only support [local](/terraform/language/modules/sources#local-paths) and [registry](/terraform/language/modules/sources#terraform-registry) modules within the `source` attribute.
...
While OpenRecipe executes a `terraform test` command, OpenRecipe maintains at least one, but possibly many, state files within memory for each test file.
...
At the conclusion of a test file, OpenRecipe attempts to destroy every resource it created during the execution of that test file. When OpenRecipe loads alternate modules, the order in which OpenRecipe destroys those objects in is important. For example, in the first [Modules](#modules) example, OpenRecipe could not destroy the resources created in the "setup" `run` block before the objects created in the "execute" `run` block, because the S3 bucket we created in the "setup" step can not be destroyed while it contains objects.

OpenRecipe destroys resources in reverse `run` block order. In the most recent [example](#modules-state), there are three state files. One for the main state, one for the `./testing/loader` module, and one for the `./testing/setup` module. The `./testing/loader` state file would be destroyed first as it was referenced most recently by the last run block. The main state file would be destroyed second as it was referenced by the "update" `run` block. The `./testing/setup` state file would then be destroyed last.
...
By default, if any [Custom Conditions](/terraform/language/expressions/custom-conditions), including `check` block assertions, fail during the execution of a OpenRecipe test file then the overall command reports the test as a failure.

However, it is a common testing paradigm to want to test failure cases. OpenRecipe supports the `expect_failures` attribute for this use case.
...
> **Note**: OpenRecipe only expects failures in the operation specified by the `command` attribute of the `run` block.
...
There are instances when OpenRecipe does not execute a custom condition during the planning stage, because that condition is relying on computed attributes that are only available after OpenRecipe creates the referenced resource. In these cases, you could use an `expect_failures` block alongside a `command = apply` attribute and value. However, in most cases we recommend only using `expect_failures` alongside `command = plan` operations.

> **Note**: Expected failures only apply to user-defined custom conditions.

Other kinds of failure _besides_ the specified expected failures in the checkable object still result in the overall test failing. For example, a variable that expects a boolean value as input fails the surrounding test if OpenRecipe provides the wrong kind of value, even if that variable is included in an `expect_failures` attribute.

The `expect_failures` attribute is included to allow authors to test their configuration and any logic defined within. A type mismatch, as in the previous example, is not something OpenRecipe authors should have to worry about testing as OpenRecipe itself will handle enforce type constraints. As such, you can only `expect_failures` in custom conditions.
