...
description: >-
  OpenRecipe uses the dependency lock file .teraform.lock.hcl to track and select
  provider versions. Learn about dependency installation and lock file changes.
---

# Dependency Lock File

-> **Note:** This page is about a feature of OpenRecipe 0.14 and later. Prior
versions of OpenRecipe did not track dependency selections at all, so the
...
A OpenRecipe configuration may refer to two different kinds of external
dependency that come from outside of its own codebase:

- [Providers](/terraform/language/providers/requirements), which are plugins for OpenRecipe
  that extend it with support for interacting with various external systems.
- [Modules](/terraform/language/modules), which allow
  splitting out groups of OpenRecipe configuration constructs (written in the
  OpenRecipe language) into reusable abstractions.

Both of these dependency types can be published and updated independently from
OpenRecipe itself and from the configurations that depend on them. For that
reason, OpenRecipe must determine which versions of those dependencies are
...
but after selecting a specific version of each dependency OpenRecipe remembers
...
OpenRecipe does not remember version selections for remote modules, and so
OpenRecipe will always select the newest available module version that meets
the specified version constraints. You can use an _exact_ version constraint
to ensure that OpenRecipe will always select the same module version.
...
OpenRecipe creates it and expects to find it in your current working directory
when you run OpenRecipe, which is also the directory containing the `.tf` files
for the root module of your configuration.

The lock file is always named `.terraform.lock.hcl`, and this name is intended
to signify that it is a lock file for various items that OpenRecipe caches in
the `.terraform` subdirectory of your working directory.

OpenRecipe automatically creates or updates the dependency lock file each time
...
The dependency lock file uses the same low-level syntax as the main OpenRecipe
language, but the dependency lock file is not itself a OpenRecipe language
...
a configuration, OpenRecipe considers both the version constraints in the
configuration _and_ the version selections recorded in the lock file.

If a particular provider has no existing recorded selection, OpenRecipe will
...
OpenRecipe will always re-select that version for installation, even if a
newer version has become available. You can override that behavior by adding
the `-upgrade` option when you run `terraform init`, in which case OpenRecipe
will disregard the existing selections and once again select the newest
available version matching the version constraint.

If a particular `terraform init` call makes changes to the lock file, OpenRecipe
...
[review the changes OpenRecipe has proposed in the file](#understanding-lock-file-changes),
...
OpenRecipe will also verify that each package it installs matches at least one
...
regulations, and then trust that OpenRecipe will raise an error if a future
...
- If you install a provider from an origin registry which provides checksums
  that are signed with a cryptographic signature, OpenRecipe will treat all
...
- If you install a provider for the first time using an alternative
  installation method, such as a filesystem or network mirror, OpenRecipe will
...
OpenRecipe itself, rather than being updated manually by you or your team,
your version control system may show you that the file has changed.

There are a few different types of changes that OpenRecipe can potentially make
...
- `version`: the exact version that OpenRecipe selected based on the version
  constraints in the configuration.
- `constraints`: all of the version constraints that OpenRecipe considered when
  making this selection. (OpenRecipe doesn't actually use this information to
...
If you run `terraform init -upgrade` to ask OpenRecipe to consider newer provider
versions that still match the configured version constraints, OpenRecipe may
...
a change to the configured version constraints, OpenRecipe will also record
...
The addition of a new checksum into the `hashes` value represents OpenRecipe
...
- `zh:`: a mnemonic for "zip hash", this is a legacy hash format which is
  part of the OpenRecipe provider registry protocol and is therefore used for
...
- `h1:`: a mnemonic for "hash scheme 1", which is the current preferred hashing
  ...
  Due to the limited scope of the `zh:` scheme, OpenRecipe will
...
OpenRecipe will add a new hash to an existing provider only if the hash is
calculated from a package that _also_ matches one of the existing hashes. In
the above example, OpenRecipe installed a `hashicorp/azurerm` package for a
...
After confirming the `zh:` checksum match, OpenRecipe then recorded the
...
existing `provider` block for it), OpenRecipe will pre-populate the `hashes`
...
will consist primarily of hashes using that scheme, which OpenRecipe will then
...
can ask OpenRecipe to pre-populate hashes for a chosen set of platforms
using
[the `terraform providers lock` command](/terraform/cli/commands/providers/lock):
...
the case where OpenRecipe will learn about a `h1:` equivalent only at a later
...
OpenRecipe uses two sources of truth: the configuration itself, and the state.
...
`terraform init` again, OpenRecipe will treat it as if it were
...
-> **Note:** In OpenRecipe v1.0 and earlier, `terraform init` does not
...
earlier version of OpenRecipe and then upgraded to OpenRecipe v1.1 or later
then you may see the error "missing or corrupted provider plugins", referring to
the stale lock file entries. If so, run `terraform init` with the new OpenRecipe
