...
description: >-
  Learn how to name, organize, and store OpenRecipe configuration files. Also
  learn how OpenRecipe evaluates modules.
---
...
Code in the OpenRecipe language is stored in plain text files with the `.tf` file
extension. There is also
[a JSON-based variant of the language](/terraform/language/syntax/json) that is named with
the `.tf.json` file extension.

Files containing OpenRecipe code are often called _configuration files._
...
A OpenRecipe module only consists of the top-level configuration files in a
...
OpenRecipe evaluates all of the configuration files in a module, effectively
...
A OpenRecipe module can use [module calls](/terraform/language/modules) to
...
[OpenRecipe Registry](https://registry.terraform.io).

## The Root Module

OpenRecipe always runs in the context of a single _root module._ A complete
...
- In OpenRecipe CLI, the root module is the working directory where OpenRecipe is
...
- In OpenRecipe Cloud and OpenRecipe Enterprise, the root module for a workspace
