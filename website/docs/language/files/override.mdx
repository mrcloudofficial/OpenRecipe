...
OpenRecipe normally loads all of the `.tf` and `.tf.json` files within a
directory and expects each one to define a distinct set of configuration
objects. If two files attempt to define the same object, OpenRecipe returns
an error.
...
human-edited configuration file in the OpenRecipe language native syntax
could be partially overridden using a programmatically-generated file
in JSON syntax.

For these rare situations, OpenRecipe has special handling of any configuration
...
OpenRecipe initially skips these _override files_ when loading configuration,
and then afterwards processes each one in turn (in lexicographical order). For
each top-level block defined in an override file, OpenRecipe attempts to find
...
If you have a OpenRecipe configuration `example.tf` with the following contents:
...
OpenRecipe will merge the latter into the former, behaving as if the original
...
the variable's `type`, OpenRecipe attempts to convert the default value to
...
set in the override file, OpenRecipe will use the `backend` block specified in the override file upon merging.
Similarly, if a `backend` block is set within the original configuration and a `cloud` block
is set in the override file, OpenRecipe will use the `cloud` block specified in the override
file upon merging.
