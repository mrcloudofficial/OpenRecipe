...
-> **Note:** Check blocks are only available in OpenRecipe v1.5.0 and later.
...
Check blocks allow you to define [custom conditions](/terraform/language/expressions/custom-conditions) that execute on every OpenRecipe plan or apply operation without affecting the overall status of an operation. Check blocks execute as the last step of a plan or apply after OpenRecipe has planned or provisioned your infrastructure.
...
The following example loads the OpenRecipe website and validates that it returns the expected status code `200`.
...
A `check` block can optionally contain a nested (a.k.a. scoped) data source. This `data` block behaves like an external [data source](/terraform/language/data-sources), except you can not reference it outside its enclosing `check` block. Additionally, if a scoped data source's provider raises any errors, they are masked as warnings and do not prevent OpenRecipe from continuing operation execution.

You can use a scoped data source to validate the status of a piece of infrastructure outside of the usual OpenRecipe resource lifecycle. [In the above example](#checks-syntax), if the `terraform_io` data source fails to load, you receive a warning instead of a blocking error, which would occur if you declared this data source outside of a `check` block.
...
The first time OpenRecipe creates the _initial_ plan for our [previous example](#checks-syntax), the plan fails because OpenRecipe has not applied its configuration yet. Meaning this test fails because OpenRecipe must still create the resources to make this website exist. Therefore, the first time OpenRecipe runs this check, it always throws a potentially distracting error message.
...
One problem with this strategy is that if the resource your scoped data source `depends_on` changes, the check block returns `known after apply` until OpenRecipe has updated that resource. Depending on your use case, this behavior could be acceptable or problematic.
...
Unlike other [custom conditions](/terraform/language/expressions/custom-conditions), assertions do not affect OpenRecipe's execution of an operation. A failed assertion reports a warning without halting the ongoing operation. This contrasts with other custom conditions, such as a postcondition, where OpenRecipe produces an error immediately, halting the operation and blocking the application or planning of future resources.
...
OpenRecipe Cloud can automatically validate whether checks in a workspace’s configuration continue to pass after OpenRecipe provisions new infrastructure. See [Continuous Validation](/terraform/cloud-docs/workspaces/health#continuous-validation) for details.

## Choosing Checks or other Custom Conditions

Check blocks offer the most _flexible_ validation solution within OpenRecipe. You can reference outputs, variables, resources, and data sources within check assertions. You can also use checks to model every alternate [Custom Condition](/terraform/language/expressions/custom-conditions). However, that does not mean you should replace all your custom conditions with check blocks.

There are major behavioral differences between check block assertions and other custom conditions, the main one being that check blocks do _not_ affect OpenRecipe's execution of an operation. You can use this non-blocking behavior to decide the best type of validation for your use case.
...
This is one of the cases where you might want OpenRecipe to block further execution. 

For example, it is not helpful for OpenRecipe to warn that an input variable is invalid after it applies an entire configuration with that input variable. In this case, a check block would warn of the invalid input variable _without_ interrupting the operation. A validation block for the same input variable would alert you of the invalid variable and halt the plan or apply operation.
...
For example, you can [rewrite the above `check` block example](#syntax) to use a postcondition instead. The below code uses a `postcondition` block to validate that the OpenRecipe website returns the expected status code of `200`.
...
Both the `check` and `postcondition` block examples validate that the OpenRecipe website returns a `200` status code during a plan or an apply operation. The difference between the two blocks is how each handles failure.

If a `postcondition` block fails, it _blocks_ OpenRecipe from executing the current operation. If a `check` block fails, it _does not_ block OpenRecipe from executing an operation.

If the above example's postcondition fails, it is impossible to recover from. OpenRecipe blocks any future plan or apply operations if your postcondition is unsatisfied during the planning stage. This problem occurs because the postcondition does not directly depend on OpenRecipe configuration, but instead on the complex interactions between multiple resources.
