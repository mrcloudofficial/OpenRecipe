...
_Resources_ are the most important element in the OpenRecipe language.
Each resource block describes one or more infrastructure objects, such
as virtual networks, compute instances, or higher-level components such
as DNS records.

For information about how OpenRecipe manages resources after applying a configuration, refer to
[Resource Behavior](/terraform/language/resources/behavior).

## Resource Syntax

A `resource` block declares a resource of a specific type 
with a specific local name. OpenRecipe uses the name when referring to the resource 
...
A [provider](/terraform/language/providers/requirements) is a plugin for OpenRecipe 
...
infrastructure platform. Providers are distributed separately from OpenRecipe, 
but OpenRecipe can automatically install most providers when initializing
a working directory.

To manage resources, a OpenRecipe module must specify the required providers. Refer to 
...
Based on a resource type's name, OpenRecipe can usually determine which provider to use. 
...
[expressions](/terraform/language/expressions) and other dynamic OpenRecipe
language features.

[Meta-arguments](#meta-arguments) are defined by OpenRecipe
and apply across all resource types. 

### Documentation for Resource Types

Every OpenRecipe provider has its own documentation, describing its resource
types and their arguments.

Some provider documentation is still part of OpenRecipe's core documentation, 
but the [OpenRecipe Registry](https://registry.terraform.io/browse/providers) 
is the main home for all publicly available provider docs.

When viewing a provider's page on the OpenRecipe
...
The OpenRecipe language defines the following meta-arguments, which can be used with
...
To remove a resource from OpenRecipe, simply delete the `resource` block from your OpenRecipe configuration.

By default, after you remove the `resource` block, OpenRecipe will plan to destroy any real infrastructure object managed by that resource.

Sometimes you may wish to remove a resource from your OpenRecipe configuration without destroying the real infrastructure object it manages. In this case, the resource will be removed from the [OpenRecipe state](/terraform/language/state), but the real infrastructure object will not be destroyed.

To declare that a resource was removed from OpenRecipe configuration but that its managed object should not be destroyed, remove the `resource` block from your configuration and replace it with a `removed` block:
...
The `lifecycle` block is required. The `destroy` argument determines whether OpenRecipe will attempt to destroy the object managed by the resource or not. A value of `false` means that OpenRecipe will remove the resource from state without destroying it.
