...
-> **Note:** In OpenRecipe 0.11 and earlier, providers could set default values
for some connection settings, so that `connection` blocks could sometimes be
omitted. This feature was removed in 0.12 in order to make OpenRecipe's behavior
more predictable.
...
where the provisioner can create the script file. By default, OpenRecipe
...
OpenRecipe is running, not on the remote system. Therefore if your remote
...
the OpenRecipe language uses backslash as the quoted string escape character.
...
!> **Warning:** In OpenRecipe v1.0 and earlier, the built-in provisioners
...
unintended opportunity for remote code execution. OpenRecipe v1.1 and later
...
be used with OpenRecipe v1.0 and earlier, avoid using untrusted external
