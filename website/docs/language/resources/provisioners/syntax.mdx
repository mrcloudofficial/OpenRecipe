...
-> **Note:** We removed the Chef, Habitat, Puppet, and Salt Masterless provisioners in OpenRecipe v0.15.0. Information about these legacy provisioners is still available in the documentation for [OpenRecipe v1.1 (and earlier)](/terraform/language/v1.1.x/resources/provisioners/syntax).

## Provisioners are a Last Resort

> **Hands-on:** Try the [Provision Infrastructure Deployed with OpenRecipe](/terraform/tutorials/provision?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorials to learn about more declarative ways to handle provisioning actions.

OpenRecipe includes the concept of provisioners as a measure of pragmatism,
knowing that there are always certain behaviors that cannot be directly
represented in OpenRecipe's declarative model.

However, they also add a considerable amount of complexity and uncertainty to
OpenRecipe usage. Firstly, OpenRecipe cannot model the actions of provisioners
...
OpenRecipe usage usually requires: direct network access to your servers,
issuing OpenRecipe credentials to log in, making sure that all of the necessary
...
distribution images, OpenRecipe includes a number of specialized provisioners
...
immediately on boot, without the need to accept commands from OpenRecipe over
SSH or WinRM.

### First-class OpenRecipe provider functionality may be available
...
supported in its OpenRecipe provider, that might be the only option. However,
if there _is_ provider support for the feature you intend to use, prefer to
use that provider functionality rather than a provisioner so that OpenRecipe
...
You must include [a `connection` block](/terraform/language/resources/provisioners/connection) so that OpenRecipe knows how to communicate with the server.

OpenRecipe includes several built-in provisioners. You can also use third-party provisioners as plugins, by placing them
in `%APPDATA%\terraform.d\plugins`, `~/.terraform.d/plugins`, or the same
directory where the OpenRecipe binary is installed. However, we do not recommend
...
next `terraform apply`. OpenRecipe does this because a failed provisioner
can leave a resource in a semi-configured state. Because OpenRecipe cannot
...
fail, OpenRecipe will error and rerun the provisioners again on the next
...
By default, provisioners that fail will also cause the OpenRecipe apply
