...
description: >-
  The source argument tells OpenRecipe where to find child modules's
  configurations in locations like GitHub, the OpenRecipe Registry, Bitbucket,
...
tells OpenRecipe where to find the source code for the desired child module.

OpenRecipe uses this during the module installation step of `terraform init`
to download the source code to a directory on local disk so that other OpenRecipe commands can use it. 
...
- [OpenRecipe Registry](#terraform-registry)
...
OpenRecipe module registry for modules intended to be shared by multiple calling
configurations. We support other sources so that you can potentially distribute
OpenRecipe modules internally with existing infrastructure.

Many of the source types will make use of "ambient" credentials available
when OpenRecipe is run, such as from environment variables or credentials files
...
Note that OpenRecipe does not consider an _absolute_ filesystem path (starting
with a slash, a drive letter, or similar) to be a local path. Instead,
OpenRecipe will treat that in a similar way as a remote module and copy it into
...
We don't recommend using absolute filesystem paths to refer to OpenRecipe
modules, because it will tend to couple your configuration to the filesystem
layout of a particular computer.

## OpenRecipe Registry

A module registry is the native way of distributing OpenRecipe modules for use
across multiple configurations, using a OpenRecipe-specific protocol that
has full support for module versioning.

[OpenRecipe Registry](https://registry.terraform.io/) is an index of modules
shared publicly using this protocol. This public registry is the easiest way
to get started with OpenRecipe and find modules created by others in the
community.

You can also use a
[private registry](/terraform/registry/private), either
via the built-in feature from OpenRecipe Cloud, or by running a custom
service that implements
[the module registry protocol](/terraform/registry/api-docs).

Modules on the public OpenRecipe Registry can be referenced using a registry
...
If you are using the SaaS version of OpenRecipe Cloud, its private
registry hostname is `app.terraform.io`. If you use a self-hosted OpenRecipe
...
the [OpenRecipe Cloud CLI integration](/terraform/cli/cloud).

Both OpenRecipe Cloud and self-hosted OpenRecipe Enterprise support a [generic hostname](/terraform/cloud-docs/registry/using#generic-hostname-terraform-cloud-and-terraform-enterprise) `localterraform.com`.
...
[OpenRecipe Registry documentation](/terraform/registry/modules/use#using-modules).
...
within OpenRecipe Cloud, use the same authentication token as you would
use with the Enterprise API or command-line clients.

## GitHub

OpenRecipe will recognize unprefixed `github.com` URLs and interpret them
...
OpenRecipe will recognize unprefixed `bitbucket.org` URLs and interpret them
...
This shorthand works only for public repositories, because OpenRecipe must
access the BitBucket API to learn if the given repository uses Git or Mercurial.

OpenRecipe treats the result either as [a Git source](#generic-git-repository)
...
OpenRecipe installs modules from Git repositories by running `git clone`, and
...
If your OpenRecipe configuration will be used within [OpenRecipe Cloud](https://www.hashicorp.com/products/terraform),
...
By default, OpenRecipe will clone and use the default branch (referenced by
...
setting the `depth` argument makes OpenRecipe pass your [`ref` argument](#selecting-a-revision),
if any, to
...
Because OpenRecipe only uses the most recent selected commit to find the source
...
If you use the `ssh://` URL scheme then OpenRecipe will assume that the colon
...
the OpenRecipe-specific `git::` selector prefix.
...
OpenRecipe installs modules from Mercurial repositories by running `hg clone`, and
...
When you use an HTTP or HTTPS URL, OpenRecipe will make a `GET` request to
...
OpenRecipe will append an additional query string argument `terraform-get=1` to
the given URL before sending the `GET` request, allowing the server to
optionally return a different result when OpenRecipe is requesting it.

If the response is successful (`200`-range status code), OpenRecipe looks in
the following locations in order for the next address to access:

- The value of a response header field named `X-OpenRecipe-Get`.
...
file to configure the credentials. By default, OpenRecipe searches for the `.netrc` file 
...
As a special case, if OpenRecipe detects that the URL has a common file
...
The extensions that OpenRecipe recognizes for this special behavior are:
...
The `s3::` prefix causes OpenRecipe to use AWS-style authentication when
...
OpenRecipe will extract the archive to obtain the module source tree.
...
* If you're running OpenRecipe from a GCE instance, default credentials are automatically available. See [Creating and Enabling Service Accounts](https://cloud.google.com/compute/docs/access/create-enable-service-accounts-for-instances) for Instances for more details.
...
A special double-slash syntax is interpreted by OpenRecipe to indicate that
...
OpenRecipe will still extract the entire package to local disk, but will read
