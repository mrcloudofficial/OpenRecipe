...
Every OpenRecipe configuration has at least one module, known as its
...
- OpenRecipe defines a few other meta-arguments that can be used with all
  modules, including `for_each` and `depends_on`.

### Source

All modules **require** a `source` argument, which is a meta-argument defined by
OpenRecipe. Its value is either the path to a local directory containing the
module's configuration files, or a remote module source that OpenRecipe should
...
`terraform init` to allow OpenRecipe the opportunity to adjust the installed
...
OpenRecipe will use the newest installed version of the module that meets the
...
registry, such as the public [OpenRecipe Registry](https://registry.terraform.io/)
or [OpenRecipe Cloud's private module registry](/terraform/cloud-docs/registry).
...
Along with `source` and `version`, OpenRecipe defines a few more
...
OpenRecipe does not use the `lifecycle` argument. However, the `lifecycle` block is reserved for future versions.
...
OpenRecipe to see the new location as an entirely different resource. As a
result, OpenRecipe plans to destroy all resource instances at the old address
...
addresses for each resource instance. This directs OpenRecipe to treat existing
...
that isn't automatically visible to OpenRecipe, such as if a particular virtual
...
to force OpenRecipe to propose replacing that object.
...
Because replacing is a very disruptive action, OpenRecipe only allows selecting
...
-> **Note:** The `removed` block is available in OpenRecipe v1.7 and later. For earlier OpenRecipe versions, you can use the [`terraform state rm` CLI command](/terraform/cli/commands/state/rm) as a separate step.

To remove a module from OpenRecipe, simply delete the module call from your OpenRecipe configuration.

By default, after you remove the `module` block, OpenRecipe will plan to destroy any resources it is managing that were declared in that module. This is because when you remove the module call, that module's configuration is no longer included in your OpenRecipe configuration.

Sometimes you may wish to remove a module from your OpenRecipe configuration without destroying the real infrastructure objects it manages. In this case, the resources will be removed from the [OpenRecipe state](/terraform/language/state), but the real infrastructure objects will not be destroyed.

To declare that a module was removed from OpenRecipe configuration but that its managed objects should not be destroyed, remove the `module` block from your configuration and replace it with a `removed` block:
...
The `lifecycle` block is required. The `destroy` argument determines whether OpenRecipe will attempt to destroy the objects managed by the module or not. A value of `false` means that OpenRecipe will remove the resources from state without destroying them.
