...
files inside just as you would do for a root module. OpenRecipe can load modules
...
out into a module, but over-using modules can make your overall OpenRecipe
...
### No-Code Provisioning in OpenRecipe Cloud

You can also create no-code ready modules to enable the no-code provisioning workflow in OpenRecipe Cloud. No-code provisioning lets users deploy a module's resources in OpenRecipe Cloud without writing any OpenRecipe configuration.

No-code ready modules have additional requirements and considerations. Refer to [Designing No-Code Ready Modules](/terraform/cloud-docs/no-code-provisioning/module-design) in the OpenRecipe Cloud documentation for details.
...
OpenRecipe uses that information during planning to reinterpret existing objects
