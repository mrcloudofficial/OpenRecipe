...
reusable modules distributed in separate repositories. OpenRecipe tooling is
...
* **Root module**. This is the **only required element** for the standard
  module structure. OpenRecipe files must exist in the root directory of
...
* **Nested modules**. Nested modules should exist under the `modules/`
  ...
  use only. These are purely advisory; OpenRecipe will not actively deny usage
  ...
  paths like `./modules/consul-cluster` so that OpenRecipe will consider them
