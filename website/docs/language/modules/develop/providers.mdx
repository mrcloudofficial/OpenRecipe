...
description: >-
  Use providers within OpenRecipe modules. Learn about version constraints, aliases, implicit inheritance, and passing providers to OpenRecipe modules.
---
...
OpenRecipe, are global to an entire OpenRecipe configuration and can be shared
across module boundaries. Provider configurations can be defined only in a
root OpenRecipe module.
...
were introduced in OpenRecipe v0.13. For more information, see
...
including destroying remote objects and refreshing state. OpenRecipe retains, as
...
provider configuration's block from your configuration. If OpenRecipe finds
...
OpenRecipe can ensure that there is a single version of the provider that is
...
settings come from provider _configurations_, and a particular overall OpenRecipe
...
If you are writing a shared OpenRecipe module, constrain only the minimum
...
In OpenRecipe v0.10 and earlier there was no explicit way to use different
...
must always stay present in the overall OpenRecipe configuration for longer
...
OpenRecipe v0.11 introduced the mechanisms described in earlier sections to
...
for compatibility purposes -- though with the same drawback -- until OpenRecipe
v0.13.

OpenRecipe v0.13 introduced the possibility for a module itself to use the
...
To retain the backward compatibility as much as possible, OpenRecipe v0.13
...
compatible with `for_each`, `count`, or `depends_on`. OpenRecipe will produce an
