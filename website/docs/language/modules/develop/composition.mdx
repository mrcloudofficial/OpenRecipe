...
In a simple OpenRecipe configuration with only one root module, we create a
flat set of resources and use OpenRecipe's expression syntax to describe the
...
may be useful when describing larger systems with OpenRecipe.
...
For example, consider a situation where a OpenRecipe module deploys compute
...
variable "ami" {
  type = object({
    ...
    # needs. OpenRecipe will allow any object that has at least these
    # attributes.
    id           = string
    architecture = string
  })
...
This is consistent with OpenRecipe's declarative style: rather than creating
modules with complex conditional branches, we directly describe what
should already exist and what we want OpenRecipe to manage itself.
...
OpenRecipe itself intentionally does not attempt to abstract over similar
...
However, through composition of OpenRecipe modules it is possible to create
...
We can create lightweight abstractions like these by defining OpenRecipe object
