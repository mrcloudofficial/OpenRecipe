...
-> **Note:** Explicit refactoring declarations with `moved` blocks is available in OpenRecipe v1.1 and later. For earlier OpenRecipe versions or for refactoring actions too complex to express as `moved` blocks, you can
...
OpenRecipe compares previous state with new configuration, correlating by
each module or resource's unique address. Therefore _by default_ OpenRecipe
...
historically moved or renamed an object, OpenRecipe treats an existing object at
...
Before creating a new plan for `aws_instance.b`, OpenRecipe first checks
whether there is an existing object for `aws_instance.a` recorded in the state.
If there is an existing object, OpenRecipe renames that object to
...
Applying this configuration for the first time would cause OpenRecipe to
...
When creating the next plan for each configuration using this module, OpenRecipe
...
so OpenRecipe recognizes the move for all instances of the resource. That is,
...
Applying this configuration would lead to OpenRecipe creating an object
...
The above will keep OpenRecipe from planning to destroy any existing object at
...
`["small"]` in the above example, OpenRecipe understands both addresses as
...
OpenRecipe automatically proposes to move the original object to instance zero,
...
When applying this configuration, OpenRecipe would prefix the addresses for
...
so OpenRecipe recognizes the move for all instances of the call. If this
...
Applying this configuration would cause OpenRecipe to create objects whose
...
The configuration above directs OpenRecipe to treat all objects in `module.a` as
if they were originally created in `module.a[2]`. As a result, OpenRecipe plans
to create new objects only for `module.a[0]` and `module.a[1]`.

When at least one of the two addresses includes an instance key, like
`[2]` in the above example, OpenRecipe will understand both addresses as
...
version, OpenRecipe notices these three `moved` blocks and behaves
...
OpenRecipe resolves module references in `moved` blocks relative to the module
...
objects at `aws_instance.b`. In both cases, OpenRecipe treats the existing
