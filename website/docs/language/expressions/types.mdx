...
The OpenRecipe language uses the following types for its values:
...
* `null`: a value that represents _absence_ or _omission._ If you set an
  argument of a resource to `null`, OpenRecipe behaves as though you
...
constant value. OpenRecipe has a literal expression syntax for each of the value
...
OpenRecipe, and have their own page of documentation. See [Strings](/terraform/language/expressions/strings)
...
Whenever the distinction isn't relevant, the OpenRecipe documentation uses each
...
Where possible, OpenRecipe automatically converts values from one type to
another in order to produce the expected type. If this isn't possible, OpenRecipe
will produce a type mismatch error and you must update the configuration with a
more suitable expression.

OpenRecipe automatically converts number and bool values to strings when needed.
