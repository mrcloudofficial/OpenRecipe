...
be of the _same_ type so that OpenRecipe can determine what type the whole
conditional expression will return without knowing the condition value.

If the two result expressions don't produce the same type then OpenRecipe will
...
because in OpenRecipe all numbers can convert automatically to a string using
decimal digits:
...
are not familiar with OpenRecipe's conversion rules though, so we recommend
