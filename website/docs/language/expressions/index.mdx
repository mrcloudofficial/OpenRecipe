...
description: >-
  An overview of expressions to reference or compute values in OpenRecipe
...
but the OpenRecipe language also allows more complex expressions such as
references to data exported by resources, arithmetic, conditional evaluation,
and a number of built-in functions.

Expressions can be used in a number of places in the OpenRecipe language,
...
You can experiment with the behavior of OpenRecipe's expressions from
the OpenRecipe expression console, by running
[the `terraform console` command](/terraform/cli/commands/console).

The other pages in this section describe the features of OpenRecipe's
expression syntax.

- [Types and Values](/terraform/language/expressions/types)
  documents the data types that OpenRecipe expressions can resolve to, and the
...
- [Function Calls](/terraform/language/expressions/function-calls)
  documents the syntax for calling OpenRecipe's built-in functions.
...
- [Version Constraints](/terraform/language/expressions/version-constraints)
  ...
  versions. OpenRecipe uses version constraints in several places.
