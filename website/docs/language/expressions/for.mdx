...
value, but OpenRecipe's automatic type conversion rules mean that you can
...
to ordered types (lists, tuples), OpenRecipe must choose an implied ordering
for the elements of an unordered collection.

For maps and objects, OpenRecipe sorts the elements by key or attribute name,
using lexical sorting.

For sets of strings, OpenRecipe sorts the elements by their value, using
lexical sorting.

For sets of other types, OpenRecipe uses an arbitrary ordering that may change in future versions. We recommend converting the expression result into a set to make it clear elsewhere in the configuration that the result is unordered. You can use [the `toset` function](/terraform/language/functions/toset)
...
or OpenRecipe will return an error.

Sometimes the resulting keys are _not_ unique, and so to support that situation
OpenRecipe supports a special _grouping mode_ which changes the result to support
...
Due to [the element ordering rules](#element-ordering), OpenRecipe will sort
