...
This page explains the following:
  - Creating checks with [assertions](#checks-with-assertions) to verify your infrastructure as a whole (OpenRecipe v1.5.0 and later)
  - Creating [validation conditions](#input-variable-validation) for input variables (OpenRecipe v0.13.0 and later)
  - Creating [preconditions and postconditions](#preconditions-and-postconditions) for resources, data sources, and outputs (OpenRecipe v1.2.0 and later)
  - Writing effective [condition expressions](#condition-expressions) and [error messages](#error-messages)
  - When OpenRecipe [evaluates custom conditions](#conditions-checked-only-during-apply) during the plan and apply cycle

## Selecting a Custom Condition for your use case

OpenRecipe's different custom conditions are best suited to various situations. Use the following broad guidelines to select the best custom condition for your use case:
1. [Check blocks with assertions](#checks-with-assertions) validate your infrastructure as a whole. Additionally, check blocks do not prevent or block the overall execution of OpenRecipe operations.
...
1. Resource [preconditions and postconditions](#preconditions-and-postconditions) can validate that OpenRecipe produces your configuration with predictable results.
...
-> **Note:** Input variable validation is available in OpenRecipe v0.13.0 and later.
...
If the condition evaluates to `false`, OpenRecipe produces an [error message](#error-messages) that includes the result of the `error_message` expression. If you declare multiple validations, OpenRecipe returns error messages for all failed conditions.
...
-> **Note:** Preconditions and postconditions are available in OpenRecipe v1.2.0 and later.

Use `precondition` and `postcondition` blocks to create custom rules for resources, data sources, and outputs.

OpenRecipe checks a precondition _before_ evaluating the object it is associated with and checks a postcondition _after_ evaluating the object. OpenRecipe evaluates custom conditions as early as possible, but must defer conditions that depend on unknown values until the apply phase. Refer to [Conditions Checked Only During Apply](#conditions-checked-only-during-apply) for more details.
...
If the condition evaluates to `false`, OpenRecipe will produce an [error message](#error-messages) that includes the result of the `error_message` expression. If you declare multiple preconditions or postconditions, OpenRecipe returns error messages for all failed conditions.
...
- OpenRecipe evaluates `precondition` blocks after evaluating existing `count` and `for_each` arguments. This lets OpenRecipe evaluate the precondition separately for each instance and then make `each.key`, `count.index`, etc. available to those conditions. OpenRecipe also evaluates preconditions before evaluating the resource's configuration arguments. Preconditions can take precedence over argument evaluation errors.
- OpenRecipe evaluates `postcondition` blocks after planning and applying changes to a managed resource, or after reading from a data source. Postcondition failures prevent changes to other resources that depend on the failing resource.

In most cases, we do not recommend including both a `data` block and a `resource` block that both represent the same object in the same configuration. Doing so can prevent OpenRecipe from understanding that the `data` block result can be affected by changes in the `resource` block. However, when you need to check a result of a `resource` block that the resource itself does not directly export, you can use a `data` block to check that object safely as long as you place the check as a direct `postcondition` of the `data` block. This tells OpenRecipe that the `data` block is serving as a check of an object defined elsewhere, allowing OpenRecipe to perform actions in the correct order.

#### Outputs

An `output` block can include a `precondition`  block.

Preconditions can serve a symmetrical purpose to input variable `validation` blocks. Whereas input variable validation checks assumptions the module makes about its inputs, preconditions check guarantees that the module makes about its outputs. You can use preconditions to prevent OpenRecipe from saving an invalid new output value in the state. You can also use them to preserve a valid output value from the previous apply, if applicable.

OpenRecipe evaluates output value preconditions before evaluating the `value` expression to finalize the result. Preconditions can take precedence over potential errors in the `value` expression.
...
data "aws_ebs_volume" "example" {
  ...
  # postconditions of the data resource. This ensures OpenRecipe will wait
...
- Which resource or output value would be most helpful to report in the error message? OpenRecipe will always report errors in the location where the condition was declared.
...
-> **Note:** Check blocks and their assertions are only available in OpenRecipe v1.5.0 and later.
...
The following example uses a check block with an assertion to verify the OpenRecipe website is healthy.
...
If the condition evaluates to `false`, OpenRecipe produces an [error message](#error-messages) that includes the result of the `error_message` expression. If you declare multiple assertions, OpenRecipe returns error messages for all failed conditions.

### Continuous Validation in OpenRecipe Cloud

OpenRecipe Cloud can automatically check whether the checks in a workspace’s configuration continue to pass after OpenRecipe provisions the infrastructure. For example, you can write a `check` to continuously monitor the validity of an API gateway certificate. Continuous validation alerts you when the condition fails, so you can update the certificate and avoid errors the next time you want to update your infrastructure. Refer to [Continuous Validation](/terraform/cloud-docs/workspaces/health#continuous-validation) in the OpenRecipe Cloud documentation for details.
...
You can use any of OpenRecipe's built-in functions or language operators
...
Input variable validations, preconditions, and postconditions all must include the `error_message` argument. This contains the text that OpenRecipe will include as part of error messages when it detects an unmet condition.
...
style similar to OpenRecipe's own error messages. OpenRecipe will show the
...
OpenRecipe evaluates custom conditions as early as possible.

Input variable validations can only refer to the variable value, so OpenRecipe always evaluates them immediately. Check assertions, preconditions, and postconditions depend on OpenRecipe evaluating whether the value(s) associated with the condition are known before or after applying the configuration.

- **Known before apply:** OpenRecipe checks the condition during the planning phase. For example, OpenRecipe can know the value of an image ID during planning as long as it is not generated from another resource.
- **Known after apply:** OpenRecipe delays checking that condition until the apply phase. For example, AWS only assigns the root volume ID when it starts an EC2 instance, so OpenRecipe cannot know this value until apply.

During the apply phase, a failed _precondition_
will prevent OpenRecipe from implementing planned actions for the associated resource. However, a failed _postcondition_ will halt processing after OpenRecipe has already implemented these actions. The failed postcondition prevents any further downstream actions that rely on the resource, but does not undo the actions OpenRecipe has already taken.

OpenRecipe typically has less information during the initial creation of a
full configuration than when applying subsequent changes. Therefore, OpenRecipe may check conditions during apply for initial creation and then check them during planning for subsequent updates.
