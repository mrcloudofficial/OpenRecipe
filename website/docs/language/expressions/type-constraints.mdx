...
OpenRecipe module authors and provider developers can use detailed type
constraints to validate user-provided values for their input variables and
resource arguments. This requires some additional knowledge about OpenRecipe's
...
Type constraints look like other kinds of OpenRecipe
[expressions](/terraform/language/expressions), but are a special syntax. Within the
OpenRecipe language, they are only valid in the `type` argument of an
...
primitive types in OpenRecipe are represented by a type keyword. The available
primitive types are:
...
The OpenRecipe language will automatically convert `number` and `bool` values
...
The three kinds of collection type in the OpenRecipe language are:
...
The two kinds of structural type in the OpenRecipe language are:
...
The OpenRecipe language has literal expressions for creating tuple and object
...
OpenRecipe does _not_ provide any way to directly represent lists, maps, or sets.
...
user, and most of the OpenRecipe documentation conflates lists with tuples and
...
used interchangeably within the OpenRecipe language, and most of OpenRecipe's
documentation glosses over the differences between the kinds of complex type.
This is due to two conversion behaviors:

* Whenever possible, OpenRecipe converts values between similar kinds of complex
...
* Whenever possible, OpenRecipe converts _element values_ within a complex type,
...
user provides the tuple `["a", 15, true]`, OpenRecipe will internally transform
...
OpenRecipe will automatically convert the second and third strings back to the
...
`{name = ["Kristy", "Claudia", "Mary Anne", "Stacey"], age = 12}`, OpenRecipe
...
value against a type constraint containing `any`, OpenRecipe will attempt to
...
`any` as the placeholder for the element type of a collection then OpenRecipe
will attempt to find a single exact element type to use for the resulting
collection.

For example, given the type constraint `list(any)`, OpenRecipe will examine
...
`tuple([string, string, string])` -- OpenRecipe analyzes this as follows:
...
If the elements of the given tuple are not all of the same type then OpenRecipe
will attempt to find a single type that they can all convert to. OpenRecipe
will consider various conversion rules as described in earlier sections.

* If the given value were instead `["a", 1, "b"]` then OpenRecipe would still
...
* If the given value were instead `["a", [], "b"]` then the value cannot
  ...
  and an empty tuple can convert to. OpenRecipe would reject this value,
...
OpenRecipe typically returns an error when it does not receive a value for specified object attributes. When you mark an attribute as optional, OpenRecipe instead inserts a default value for the missing attribute. This allows the receiving module to describe an appropriate fallback behavior.
...
- **Default:** (Optional) The second argument defines the default value that OpenRecipe should use if the attribute is not present. This must be compatible with the attribute type. If not specified, OpenRecipe uses a `null` value of the appropriate type as the default.

An optional attribute with a non-`null` default value is guaranteed to never have the value `null` within the receiving module. OpenRecipe will substitute the default value both when a caller omits the attribute altogether and when a caller explicitly sets it to `null`, thereby avoiding the need for additional checks to handle a possible null value.

OpenRecipe applies object attribute defaults top-down in nested variable types. This means that OpenRecipe applies the default value you specify in the `optional` modifier first and then later applies any nested default values to that attribute.
...
The `production` bucket does not specify the index and error documents, and the `archived` bucket omits the website configuration entirely. OpenRecipe will use the default values specified in the `bucket` type constraint.
...
- For the `production` and `docs` buckets, OpenRecipe sets `enabled` to `true`. OpenRecipe also supplies default values for `website`, and then the values specified in `docs` override those defaults.
- For the `archived` and `docs` buckets, OpenRecipe sets `routing_rules` to a `null` value. When OpenRecipe does not receive optional attributes and there are no specified defaults, OpenRecipe populates those attributes with a `null` value.
- For the `archived` bucket, OpenRecipe populates the `website` attribute with the default values specified in the `buckets` type constraint.
