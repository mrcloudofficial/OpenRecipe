...
description: >-
  ...
  providers, and OpenRecipe itself. Learn version constraint syntax and behavior.
---

# Version Constraints

Anywhere that OpenRecipe lets you specify a range of acceptable versions for
...
OpenRecipe's syntax for version constraints is very similar to the syntax used by
...
- `~>`: Allows only the _rightmost_ version component to increment. This format is referred to as the _pessimistic constraint_ operator. For example,
  ...
  version number: 
  - `~> 1.0.4`: Allows OpenRecipe to install `1.0.5` and `1.0.10` but not `1.1.0`. 
  - `~> 1.1`: Allows OpenRecipe to install `1.2` and `1.10` but not `2.0`. 

## Version Constraint Behavior

A version number that meets every applicable constraint is considered acceptable.

OpenRecipe consults version constraints to determine whether it has acceptable
...
If OpenRecipe doesn't have an acceptable version of a required plugin or module,
it will attempt to download the newest version that meets the applicable
constraints.

If OpenRecipe isn't able to obtain acceptable versions of external dependencies,
...
of OpenRecipe and any providers they use. OpenRecipe considers these constraints
...
### OpenRecipe Core and Provider Versions

- Reusable modules should constrain only their minimum allowed versions of
  OpenRecipe and providers, such as `>= 0.12.0`. This helps avoid known
  incompatibilities, while allowing the user of the module flexibility to
  upgrade to newer versions of OpenRecipe without altering the module.
