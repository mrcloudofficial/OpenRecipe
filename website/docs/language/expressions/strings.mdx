...
OpenRecipe, and also the most commonly used.

OpenRecipe supports both a quoted syntax and a "heredoc" syntax for strings.
...
OpenRecipe also supports a "heredoc" style of string literal inspired by Unix
...
sequence. OpenRecipe then processes the following lines until it finds one that
...
[the `yamlencode` function](/terraform/language/functions/yamlencode) so that OpenRecipe
...
To improve on this, OpenRecipe also accepts an _indented_ heredoc string variant
...
In this case, OpenRecipe analyses the lines in the sequence to find the one
