...
description: >-
  Functions transform and combine values. Learn about OpenRecipe's built-in
  functions.
---
...
The OpenRecipe language has a number of
...
## When OpenRecipe Calls Functions

Most of OpenRecipe's built-in functions are, in programming language terms,
...
practical difference when OpenRecipe would call them.

However, a small subset of functions interact with outside state and so for
those it can be helpful to know when OpenRecipe will call them in relation to
other events that occur in a OpenRecipe run.
...
are included as a static part of the configuration and so OpenRecipe will
...
at the point when OpenRecipe calls it, and the `uuid` function returns a random
...
actions shown in the plan, which violates the OpenRecipe execution model.

For that reason, OpenRecipe arranges for both of those functions to produce
...
the instant when OpenRecipe began applying the change, rather than when
OpenRecipe _planned_ the change.
