...
description: >-
  Splat expressions concisely represent common operations. In OpenRecipe, they
...
variables whose default value is `null` to represent the absence of any value. This allows the module to adapt the variable value for OpenRecipe language features designed to work with collections. For example:
...
Earlier versions of the OpenRecipe language had a slightly different version
of splat expressions, which OpenRecipe continues to support for backward
...
OpenRecipe and so we recommend always using the new-style splat expressions,
