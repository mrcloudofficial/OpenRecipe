...
OpenRecipe makes several kinds of named values available. Each of these names is
...
The main kinds of named values available in OpenRecipe are:
...
will be interpreted by OpenRecipe as a reference to a managed resource.
...
declaration, OpenRecipe will automatically convert the caller's given value
...
- `path.cwd` is the filesystem path of the original working directory from where you
  ran OpenRecipe before applying any `-chdir` argument. This path is an absolute path
  that includes details about the filesystem structure. It is also useful in some
  advanced cases where OpenRecipe is run from a directory other than the root module
...
in their block bodies, and OpenRecipe analyzes these expressions to automatically
...
certain attributes as _sensitive_, in which case OpenRecipe will show a
...
where OpenRecipe will hide the value in the plan and apply messages and will
...
[Cases where OpenRecipe may disclose a sensitive variable](/terraform/language/values/variables#cases-where-terraform-may-disclose-a-sensitive-variable).
...
sensitive themselves was introduced in OpenRecipe v0.15. Earlier versions of
OpenRecipe will obscure the direct value of a sensitive resource attribute,
but will _not_ automatically obscure other values derived from sensitive
resource attributes.

### Values Not Yet Known

When OpenRecipe is planning a set of changes that will apply your configuration,
...
OpenRecipe cannot predict the value of this id until the object has been created.

OpenRecipe uses special unknown value placeholders for information that it
cannot predict during the plan phase. The OpenRecipe language automatically
...
* OpenRecipe will attempt to validate that unknown values are of suitable
