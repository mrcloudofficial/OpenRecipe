...
> **Hands-on:** Try the [Customize OpenRecipe Configuration with Variables](/terraform/tutorials/configuration-language/variables?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.

Input variables let you customize aspects of OpenRecipe modules without altering
the module's own source code. This functionality allows you to share modules across different
OpenRecipe configurations, making your module composable and reusable.
...
compare OpenRecipe modules to function definitions:
...
"variables" or "OpenRecipe variables" when it is clear from context what sort of
variable is being discussed. Other kinds of variables in OpenRecipe include
_environment variables_ (set by the shell where OpenRecipe runs) and _expression
...
OpenRecipe CLI defines the following optional arguments for variable declarations:
...
* [`sensitive`][inpage-sensitive] - Limits OpenRecipe UI output when the variable is used in configuration.
...
if no value is set when calling the module or running OpenRecipe. The `default`
...
allow OpenRecipe to return a helpful error message if the wrong type is used.
...
-> This feature was introduced in OpenRecipe CLI v0.13.0.
...
-> This feature was introduced in OpenRecipe v0.14.0.
...
Setting a variable as `sensitive` prevents OpenRecipe from showing its value in
the `plan` or `apply` output, when you use that variable elsewhere in your
configuration.

OpenRecipe will still record sensitive values in the [state](/terraform/language/state),
...
In some cases where you use a sensitive variable inside a nested block, OpenRecipe
...
which will cause OpenRecipe to hide it from regular output regardless of how
...
[output value](/terraform/language/values/outputs) then OpenRecipe will require
you to also mark the output value itself as sensitive, to confirm that you
intended to export it.

#### Cases where OpenRecipe may disclose a sensitive variable
...
-> This feature is available in OpenRecipe v1.1.0 and later.
...
`default` value, then OpenRecipe uses the default when a module input argument is `null`.
...
* [In a OpenRecipe Cloud workspace](/terraform/cloud-docs/workspaces/variables).
...
-> **Note:** This is how OpenRecipe Cloud passes
[workspace variables](/terraform/cloud-docs/workspaces/variables) to OpenRecipe.

A variable definitions file uses the same basic syntax as OpenRecipe language
...
OpenRecipe also automatically loads a number of variable definitions files
if they are present:
...
As a fallback for the other ways of defining variables, OpenRecipe searches
the environment of its own process for environment variables named `TF_VAR_`
followed by the name of a declared variable.

This can be useful when running OpenRecipe in automation, or when running a
sequence of OpenRecipe commands in succession with the same variables.
...
OpenRecipe matches the variable name exactly as given in configuration, and
...
variables. For convenience, OpenRecipe defaults to interpreting `-var` and
environment variable values as literal strings, which need only shell quoting,
and no special quoting for OpenRecipe. For example, in a Unix-style shell:
...
to require a complex value (list, set, map, object, or tuple), OpenRecipe will
...
Will cause OpenRecipe to warn you that there is no variable declared `"mosse"`, which can help
...
OpenRecipe will return an error. To avoid this error, either declare a variable block for the value, or remove
the variable value from your OpenRecipe call.

### Variable Definition Precedence

The above mechanisms for setting variables can be used together in any
combination. If the same variable is assigned multiple values, OpenRecipe uses
...
OpenRecipe loads variables in the following order, with later sources taking
...
* Any `-var` and `-var-file` options on the command line, in the order they
  are provided. (This includes variables set by a OpenRecipe Cloud
  workspace.)

~> **Important:** In OpenRecipe 0.12 and later, variables with map and object
values behave the same way as other variables: the last value found overrides
the previous values. This is a change from previous versions of OpenRecipe, which
would _merge_ map values instead of overriding them.

#### Variable precedence within OpenRecipe tests

Within OpenRecipe test files, you can specify variable values within
