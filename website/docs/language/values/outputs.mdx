...
description: Output values are the return values of a OpenRecipe module.
---

# Output Values

Output values make information about your infrastructure available on the
command line, and can expose information for other OpenRecipe configurations to
use. Output values are similar to return values in programming languages.

> **Hands-on:** Try the [Output Data From
> OpenRecipe](/terraform/tutorials/configuration-language/outputs)
> tutorial.
...
Resource instances managed by OpenRecipe each export attributes whose values
...
-> **Note:** Outputs are only rendered when OpenRecipe applies your plan. Running
...
OpenRecipe will hide values marked as sensitive in the messages from
...
-> **Note:** In OpenRecipe versions prior to OpenRecipe 0.14, setting an output
value in the root module as sensitive would prevent OpenRecipe from showing its
...
child modules, the dependencies of that output value allow OpenRecipe to
...
OpenRecipe analyzes the `value` expression for an output value and automatically
