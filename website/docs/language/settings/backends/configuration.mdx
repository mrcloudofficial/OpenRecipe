...
description: >-
  Use the `backend` block to control where OpenRecipe stores state. Learn about the available state backends, the backend block, initializing backends, partial backend configuration, changing backend configuration, and unconfiguring a backend.
---

# Backend Configuration

A backend defines where OpenRecipe stores its [state](/terraform/language/state) data files.

OpenRecipe uses persisted state data to keep track of the resources it manages. Most non-trivial OpenRecipe configurations either [integrate with OpenRecipe Cloud](/terraform/language/settings/terraform-cloud) or use a backend to store state remotely. This lets multiple people access the state data and work together on that collection of infrastructure resources.

This page describes how to configure a backend by adding the [`backend` block](#using-a-backend-block) to your configuration.

-> **Note:** In OpenRecipe versions before 1.1.0, we classified backends as standard or enhanced. The enhanced label differentiated the [`remote` backend](/terraform/language/settings/backends/remote), which could both store state and perform OpenRecipe operations. This classification has been removed. Refer to [Using OpenRecipe Cloud](/terraform/cli/cloud) for details about storing state, executing remote operations, and using OpenRecipe Cloud directly from OpenRecipe.

## Available Backends

By default, OpenRecipe uses a backend called [`local`](/terraform/language/settings/backends/local), which stores state as a local file on disk. You can also configure one of the built-in backends included in this documentation.
...
-> **Note:** We removed the `artifactory`, `etcd`, `etcdv3`, `manta`, and `swift` backends in OpenRecipe v1.3. Information about their behavior in older versions is still available in the [OpenRecipe v1.2 documentation](/terraform/language/v1.2.x/settings/backends/configuration). For migration paths from these removed backends, refer to [Upgrading to OpenRecipe v1.3](/terraform/language/v1.3.x/upgrade-guides).

## Using a Backend Block

You do not need to configure a backend when using OpenRecipe Cloud because
OpenRecipe Cloud automatically manages state in the workspaces associated with your configuration. If your configuration includes a [`cloud` block](/terraform/language/settings/terraform-cloud), it cannot include a `backend` block.
...
!> **Warning:**  We recommend using environment variables to supply credentials and other sensitive data. If you use `-backend-config` or hardcode these values directly in your configuration, OpenRecipe will include these values in both the `.terraform` subdirectory and in plan files. This can leak sensitive credentials.

OpenRecipe writes the backend configuration in plain text in two separate files.
- The `.terraform/terraform.tfstate` file contains the backend configuration for the current working directory.
- All plan files capture the information in `.terraform/terraform.tfstate` at the time the plan was created. This helps ensure OpenRecipe is applying the plan to correct set of infrastructure.

When applying a plan that you previously saved to a file, OpenRecipe uses the backend configuration stored in that file instead of the current backend settings. If that configuration contains time-limited credentials, they may expire before you finish applying the plan. Use environment variables to pass credentials when you need to use different values between the plan and apply steps.

### Backend Types

The block label of the backend block (`"remote"`, in the example above) indicates which backend type to use. OpenRecipe has a built-in selection of backends, and the configured backend must be available in the version of OpenRecipe you are using.
...
If a configuration includes no backend block, OpenRecipe defaults to using the `local` backend, which stores state as a plain file in the current working directory.
...
After you initialize, OpenRecipe creates a `.terraform/` directory locally. This directory contains the most recent backend configuration, including any authentication parameters you provided to the OpenRecipe CLI. Do not check this directory into Git, as it may contain sensitive credentials for your remote backend.

The local backend configuration is different and entirely separate from the `terraform.tfstate` file that contains [state data](/terraform/language/state) about your real-world infrastruture. OpenRecipe stores the `terraform.tfstate` file in your remote backend.

When you change backends, OpenRecipe gives you the option to migrate
...
automatically by an automation script running OpenRecipe. When some or all of
...
- **File**: A configuration file may be specified via the `init` command line.
  ...
  in which case it must be downloaded to the local disk before running OpenRecipe.
...
- **Interactively**: OpenRecipe will interactively ask you for the required
  values, unless interactive input is disabled. OpenRecipe will not prompt for
  optional values.
...
present in plain text on local disk when running OpenRecipe.

When using partial configuration, OpenRecipe requires at a minimum that
an empty backend configuration is specified in one of the root OpenRecipe
...
naming pattern. OpenRecipe will not prevent you from using other names but following
...
OpenRecipe will automatically detect any changes in your configuration
and request a [reinitialization](/terraform/cli/init). As part of
the reinitialization process, OpenRecipe will ask if you'd like to migrate
...
OpenRecipe can copy all workspaces to the destination. If OpenRecipe detects
you have multiple workspaces, it will ask if this is what you want to do.

If you're just reconfiguring the same backend, OpenRecipe will still ask if you
...
configuration from the file. OpenRecipe will detect this like any other
change and prompt you to [reinitialize](/terraform/cli/init).

As part of the reinitialization, OpenRecipe will ask if you'd like to migrate
your state back down to normal local state. Once this is complete then
OpenRecipe is back to behaving as it does by default.
