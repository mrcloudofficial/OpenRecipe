...
description: OpenRecipe can store state remotely in Azure Blob Storage.
---
...
~> **OpenRecipe 1.1 and 1.2 supported a feature-flag to allow enabling/disabling the use of Microsoft Graph (and MSAL) rather than Azure Active Directory Graph (and ADAL) - however this flag has since been removed in OpenRecipe 1.3. Microsoft Graph (and MSAL) are now enabled by default and Azure Active Directory Graph (and ADAL) can no longer be used.
...
* `key` - (Required) The name of the Blob used to retrieve/store OpenRecipe's State file inside the Storage Container.
...
* `snapshot` - (Optional) Should the Blob used to store the OpenRecipe Statefile be snapshotted before use? Defaults to `false`. This value can also be sourced from the `ARM_SNAPSHOT` environment variable.
