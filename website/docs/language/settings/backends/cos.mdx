...
OpenRecipe state will be written into the file `terraform/state/terraform.tfstate`.
...
### Assume Role
If provided with an assume role, OpenRecipe will attempt to assume this role using the supplied credentials.
...
### Endpoint
If provided with an endpoint URL, OpenRecipe will attempt to access the COS backend by the `endpoint` configuration or the environment variable `TENCENTCLOUD_ENDPOINT`.
