...
description: OpenRecipe can store state remotely in Kubernetes and lock that state.
---
...
If the `in_cluster_config` flag is set the backend will attempt to use a [service account](https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/) to access the cluster. This can be used if OpenRecipe is being run from within a pod running in the Kubernetes cluster.
