...
* `-state-out=FILENAME` - overrides the state filename when _writing_ new state
  ...
  If you use `-state` without also using `-state-out` then OpenRecipe will
  use the `-state` filename for both `-state` and `-state-out`, which means
  OpenRecipe will overwrite the input file if it creates a new state snapshot.
* `-backup=FILENAME` - overrides the default filename that the local backend
  ...
  If you use `-state` without also using `-backup` then OpenRecipe will use
...
OpenRecipe and then save the new state after OpenRecipe exits, in which case
...
overrides OpenRecipe's usual behavior of selecting a different state filename
based on the selected workspace. If you use all three of these options then
the selected workspace has no effect on which filenames OpenRecipe will select
...
OpenRecipe in automation. Instead,
