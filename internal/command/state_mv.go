	...
	for _, rawAddrFrom := range sourceAddrs {
		...
		default:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				msgInvalidSource,
				fmt.Sprintf("Cannot move %s: OpenRecipe doesn't know how to move this object.", rawAddrFrom),
			))
		}
...
Usage: terraform [global options] state mv [options] SOURCE DESTINATION
 ...
 one configuration into multiple separately managed OpenRecipe configurations.
