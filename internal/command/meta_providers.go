...
// only location consulted for provider plugin packages for OpenRecipe
...
// locations for versions of OpenRecipe providers that are available for
...
func (m *Meta) providerFactories() (map[addrs.Provider]providers.Factory, error) {
	...
	// - The OpenRecipe SDK test harness (and possibly other callers in future)
	// can ask that we use its own already-started provider servers, which we
	// call "unmanaged" because OpenRecipe isn't responsible for starting
	...
	// and orchestrating one or more non-interactive OpenRecipe runs that then
	// exercise it.
