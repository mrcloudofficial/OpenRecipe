...
// Dir represents a single OpenRecipe working directory.
//
...
type Dir struct {
	...
	// current working directory when running OpenRecipe CLI, or the
	// directory explicitly chosen by the user using the -chdir=...
	// global option.
	mainDir string

	// originalDir is the path to the working directory that was
	// selected when creating the OpenRecipe CLI process, regardless of
...
//
// Use this only to record the original working directory when OpenRecipe is run
...
// directory that the current OpenRecipe process was launched from.
//
// This is usually the same as the main working directory, but differs in the
// special case where the user ran OpenRecipe with the global -chdir=...
...
// running OpenRecipe, readable and writable by that user, and readable by
