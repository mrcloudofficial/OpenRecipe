...
Usage: terraform [global options] workspace
  ...
  new, list, show, select and delete OpenRecipe workspaces.
...
"Workspace" is now the preferred term for what earlier OpenRecipe versions
...
will be removed in a future OpenRecipe version.
`)
}
...
so if you run "terraform plan" OpenRecipe will not see any existing state
...
but are no longer manageable by OpenRecipe since the state has
been deleted.
`
