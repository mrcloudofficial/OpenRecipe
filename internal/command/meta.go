...
type Meta struct {
	...
	// working directory of the OpenRecipe process, though this can take on
	...
	//
	// The intended use-case of this flag is when OpenRecipe is running in
	...
	// checksums inside to only be sufficient for the computer where OpenRecipe
	...
	// predating OpenRecipe, which OpenRecipe should use but not worry about the
	// lifecycle of.
	//
	// This is essentially a more extreme version of ProviderDevOverrides where
	// OpenRecipe doesn't even worry about how the provider server gets launched,
	// just trusting that someone else did it before running OpenRecipe.
	UnmanagedProviders map[addrs.Provider]*plugin.ReattachConfig

	// AllowExperimentalFeatures controls whether a command that embeds this
	// Meta is permitted to make use of experimental OpenRecipe features.
	//
	...
	// state even if the remote and local OpenRecipe versions don't match.
...
// hashicorp/cli rather than by OpenRecipe. Use this only in situations
...
// contextOpts returns the options to use to initialize a OpenRecipe
...
// the error when the configured OpenRecipe version on the remote workspace
// does not match the local OpenRecipe version.
func (m *Meta) ignoreRemoteVersionFlagSet(n string) *flag.FlagSet {
	f := m.defaultFlagSet(n)

	f.BoolVar(&m.ignoreRemoteVersion, "ignore-remote-version", false, "continue even if remote and local OpenRecipe versions are incompatible")
...
func (m *Meta) showDiagnostics(vals ...interface{}) {
	...
	if m.compactWarnings {
		...
		if useCompact {
			...
			msg = "\n" + msg + "\nTo see the full warning notes, run OpenRecipe without -compact-warnings.\n"
			m.Ui.Warn(msg)
			return
		}
	}
...
// to set the name of the OpenRecipe workspace, overriding the workspace chosen
