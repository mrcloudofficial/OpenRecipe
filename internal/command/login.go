...
func (c *LoginCommand) Run(args []string) int {
	...
	case *disco.ErrVersionNotSupported:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
			"Host does not support OpenRecipe login",
			fmt.Sprintf("The given hostname %q allows creating OpenRecipe authorization tokens, but requires a newer version of OpenRecipe CLI to do so.", dispHostname),
		))
	default:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
	...
	if clientConfig == nil {
		...
		case *disco.ErrServiceNotProvided:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Host does not support OpenRecipe tokens API",
				fmt.Sprintf("The given hostname %q does not support creating OpenRecipe authorization tokens.", dispHostname),
			))
		case *disco.ErrVersionNotSupported:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
	...
	// Prefer OpenRecipe login if available
	if clientConfig != nil {
		...
		case clientConfig.SupportedGrantTypes.Has(disco.OAuthOwnerPasswordGrant) && hostname == svchost.Hostname("app.terraform.io"):
			// The password grant type is allowed only for OpenRecipe Cloud SaaS.
		...
		default:
			tokenDiags = tokenDiags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Host does not support OpenRecipe login",
				fmt.Sprintf("The given hostname %q does not allow any OAuth grant types that are supported by this version of OpenRecipe.", dispHostname),
			))
		}
		if oauthToken != nil {
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("The given host returned an API token, but OpenRecipe failed to save it: %s.", err),
		))
	}
	...
	c.Ui.Output("\n---------------------------------------------------------------------------------\n")
	if hostname == "app.terraform.io" { // OpenRecipe Cloud
		var motd struct {
	...
	if tfeservice != nil { // OpenRecipe Enterprise
		c.outputDefaultTFELoginSuccess(dispHostname)
	} else {
		c.Ui.Output(
			fmt.Sprintf(
				c.Colorize().Color(strings.TrimSpace(`
[green][bold]Success![reset] [bold]OpenRecipe has obtained and saved an API token.[reset]

The new API token will be used for any future OpenRecipe command that must make
...
func (c *LoginCommand) outputDefaultTFELoginSuccess(dispHostname string) {
	c.Ui.Output(
		fmt.Sprintf(
			c.Colorize().Color(strings.TrimSpace(`
[green][bold]Success![reset] [bold]Logged in to OpenRecipe Enterprise (%s)[reset]
`)),
			dispHostname,
		) + "\n",
	)
...
[green][bold]Success![reset] [bold]Logged in to OpenRecipe Cloud[reset]
` + "\n")))
}

func (c *LoginCommand) logMOTDError(err error) {
	log.Printf("[TRACE] login: An error occurred attempting to fetch a message of the day for OpenRecipe Cloud: %s", err)
}
...
func (c *LoginCommand) interactiveGetTokenByCode(hostname svchost.Hostname, credsCtx *loginCredentialsContext, clientConfig *disco.OAuthClient) (*oauth2.Token, tfdiags.Diagnostics) {
	...
	if c.BrowserLauncher != nil {
		...
		if err == nil {
			c.Ui.Output(fmt.Sprintf("OpenRecipe must now open a web browser to the login page for %s.\n", hostname.ForDisplay()))
	...
	c.Ui.Output("OpenRecipe will now wait for the host to signal that login was successful.\n")
...
func (c *LoginCommand) interactiveGetTokenByPassword(hostname svchost.Hostname, credsCtx *loginCredentialsContext, clientConfig *disco.OAuthClient) (*oauth2.Token, tfdiags.Diagnostics) {
	...
	c.Ui.Output("\n---------------------------------------------------------------------------------\n")
	c.Ui.Output("OpenRecipe must temporarily use your password to request an API token.\nThis password will NOT be saved locally.\n")
	...
	if err != nil {
		...
		// for a OpenRecipe end-user audience, so once we have more experience
...
func (c *LoginCommand) interactiveGetTokenByUI(hostname svchost.Hostname, credsCtx *loginCredentialsContext, service *url.URL) (svcauth.HostCredentialsToken, tfdiags.Diagnostics) {
	...
	if c.BrowserLauncher != nil {
		...
		if err == nil {
			c.Ui.Output(fmt.Sprintf("OpenRecipe must now open a web browser to the tokens page for %s.\n", hostname.ForDisplay()))
			c.Ui.Output(fmt.Sprintf("If a browser does not open this automatically, open the following URL to proceed:\n    %s\n", tokensURL.String()))
		} else {
	...
	if credsCtx != nil {
		...
		case cliconfig.CredentialsViaHelper:
			c.Ui.Output(fmt.Sprintf("OpenRecipe will store the token in the configured %q credentials helper\nfor use by subsequent commands.\n", credsCtx.HelperType))
		case cliconfig.CredentialsInPrimaryFile, cliconfig.CredentialsNotAvailable:
			c.Ui.Output(fmt.Sprintf("OpenRecipe will store the token in plain text in the following file\nfor use by subsequent commands:\n    %s\n", credsCtx.LocalFilename))
		}
	}
...
func (c *LoginCommand) interactiveContextConsent(hostname svchost.Hostname, grantType disco.OAuthGrantType, credsCtx *loginCredentialsContext) (bool, tfdiags.Diagnostics) {
	...
	c.Ui.Output(fmt.Sprintf("OpenRecipe will request an API token for %s using %s.\n", hostname.ForDisplay(), mechanism))
	...
	if credsCtx != nil {
		switch credsCtx.Location {
		case cliconfig.CredentialsViaHelper:
			c.Ui.Output(fmt.Sprintf("If login is successful, OpenRecipe will store the token in the configured\n%q credentials helper for use by subsequent commands.\n", credsCtx.HelperType))
		case cliconfig.CredentialsInPrimaryFile, cliconfig.CredentialsNotAvailable:
			c.Ui.Output(fmt.Sprintf("If login is successful, OpenRecipe will store the token in plain text in\nthe following file for use by subsequent commands:\n    %s\n", credsCtx.LocalFilename))
		}
	}
...
<p>The login server has returned an authentication code to OpenRecipe.</p>
