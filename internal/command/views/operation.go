...
type OperationHuman struct {
	...
	// run a follow-up command, perhaps because OpenRecipe is running in
...
Note: You didn't use the -out option to save this plan, so OpenRecipe can't guarantee to take exactly these actions if you run "terraform apply" now.
`
...
OpenRecipe has generated configuration and written it to %s. Please review the configuration and edit it as necessary before adding it to version control.
`
