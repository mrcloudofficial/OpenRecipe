		...
		if run.Config.Command == configs.ApplyTestCommand {
			...
			if err != nil {
				run.Diagnostics = run.Diagnostics.Append(tfdiags.Sourceless(
					...
					fmt.Sprintf("OpenRecipe could not marshal the state for display: %v", err)))
			} else {
			...
			if err != nil {
				run.Diagnostics = run.Diagnostics.Append(tfdiags.Sourceless(
					...
					fmt.Sprintf("OpenRecipe could not marshal the plan for display: %v", err)))
			} else {
...
func (t *TestHuman) DestroySummary(diags tfdiags.Diagnostics, run *moduletest.Run, file *moduletest.File, state *states.State) {
	...
	if diags.HasErrors() {
		t.view.streams.Eprint(format.WordWrap(fmt.Sprintf("OpenRecipe encountered an error destroying resources created while executing %s.\n", identifier), t.view.errorColumns()))
	}
...
func (t *TestJSON) DestroySummary(diags tfdiags.Diagnostics, run *moduletest.Run, file *moduletest.File, state *states.State) {
	if state.HasManagedResourceInstanceObjects() {
		...
		if run != nil {
			t.view.log.Error(
				fmt.Sprintf("OpenRecipe left some resources in state after executing %s/%s, they need to be cleaned up manually.", file.Name, run.Name),
			...
			t.view.log.Error(
				fmt.Sprintf("OpenRecipe left some resources in state after executing %s, they need to be cleaned up manually.", file.Name),
				"type", json.MessageTestCleanup,
				json.MessageTestCleanup, cleanup,
				"@testfile", file.Name)
		}
...
func (t *TestJSON) FatalInterruptSummary(run *moduletest.Run, file *moduletest.File, existingStates map[*moduletest.Run]*states.State, created []*plans.ResourceInstanceChangeSrc) {
	...
	if run != nil {
		t.view.log.Error(
			"OpenRecipe was interrupted during test execution, and may not have performed the expected cleanup operations.",
