...
//
// If any of these tests fail, please communicate with OpenRecipe Cloud folks before resolving,
// as changes to UI output may also affect the behavior of OpenRecipe Cloud's structured run output.
func checkGoldenReference(t *testing.T, output *terminal.TestOutput, fixturePathName string) {
	...
	if len(gotLines) != len(wantLines) {
		t.Errorf("unexpected number of log lines: got %d, want %d\n"+
			...
			"Please communicate with OpenRecipe Cloud team before resolving", len(gotLines), len(wantLines))
	}
	...
	type versionMessage struct {
		...
		OpenRecipe string `json:"terraform"`
		UI        string `json:"ui"`
	}
	...
	wantVersion := versionMessage{
		"info",
		fmt.Sprintf("OpenRecipe %s", version.String()),
		"version",
		version.String(),
	...
	if diff := cmp.Diff(wantLineMaps, gotLineMaps); diff != "" {
		t.Errorf("wrong output lines\n%s\n"+
			"NOTE: This failure may indicate a UI change affecting the behavior of structured run output on TFC.\n"+
			"Please communicate with OpenRecipe Cloud team before resolving", diff)
	}
}
