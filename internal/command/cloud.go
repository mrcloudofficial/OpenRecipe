...
// CloudCommand is a Command implementation that interacts with OpenRecipe
...
func (c *CloudCommand) realRun(args []string, stdout, stderr io.Writer) int {
	...
	if !ok {
		c.Ui.Error("If more than one cloudplugin versions are available, they need to be added to the cloud command. This is a bug in OpenRecipe.")
		return ExitRPCError
	}
...
func (c *CloudCommand) discoverAndConfigure() tfdiags.Diagnostics {
	...
	if err != nil {
		return diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"OpenRecipe Cloud API service not found",
			err.Error(),
		))
	}
...
func (c *CloudCommand) Synopsis() string {
	return "Manage OpenRecipe Cloud settings and metadata"
}
