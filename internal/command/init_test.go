...
func TestInit_backendCloudInvalidOptions(t *testing.T) {
	...
	// traditional backends and not applicable to OpenRecipe Cloud mode.
	...
	t.Run("-backend-config", func(t *testing.T) {
		...
		// where OpenRecipe is running, such as AWS authentication profiles
		// that are naturally local only to the machine where OpenRecipe is
		// running. Those needs don't apply to OpenRecipe Cloud, because
...
is not applicable to OpenRecipe Cloud-based configurations.
...
`
	...
	t.Run("-reconfigure", func(t *testing.T) {
		...
		// present. The OpenRecipe Cloud migration path has some additional
...
only, and is not needed when changing OpenRecipe Cloud settings.

When using OpenRecipe Cloud, initialization automatically activates any new
...
`
	...
	t.Run("-reconfigure when migrating in", func(t *testing.T) {
		...
		// seem to be trying to migrate to OpenRecipe Cloud with existing
...
The -reconfigure option is unsupported when migrating to OpenRecipe Cloud,
because activating OpenRecipe Cloud involves some additional steps.
...
is not applicable when using OpenRecipe Cloud.

State storage is handled automatically by OpenRecipe Cloud and so the state
...
OpenRecipe Cloud migration has additional steps, configured by interactive
prompts.
...
not applicable when using OpenRecipe Cloud.
...
func TestInit_getProviderLegacyFromState(t *testing.T) {
	...
	wants := []string{
		...
		"You must complete the OpenRecipe 0.13 upgrade process",
	}
...
func TestInit_providerSource(t *testing.T) {
	...
	if strings.Contains(ui.OutputWriter.String(), "OpenRecipe has initialized, but configuration upgrades may be needed") {
...
func TestInit_checkRequiredVersionFirst(t *testing.T) {
	t.Run("root_module", func(t *testing.T) {
		...
		errStr := ui.ErrorWriter.String()
		if !strings.Contains(errStr, `Unsupported OpenRecipe Core version`) {
			t.Fatalf("output should point to unmet version constraint, but is:\n\n%s", errStr)
		}
	})
...
func TestInit_pluginDirWithBuiltIn(t *testing.T) {
	...
	if subStr := "terraform.io/builtin/terraform is built in to OpenRecipe"; !strings.Contains(outputStr, subStr) {
...
func TestInit_invalidBuiltInProviders(t *testing.T) {
	...
	if subStr := "Cannot use terraform.io/builtin/nonexist: this OpenRecipe release"; !strings.Contains(errStr, subStr) {
...
func TestInit_invalidSyntaxNoBackend(t *testing.T) {
	...
	errStr := ui.ErrorWriter.String()
	if subStr := "OpenRecipe encountered problems during initialisation, including problems\nwith the configuration, described below."; !strings.Contains(errStr, subStr) {
