...
// check if there is a new version of OpenRecipe.
...
Usage: terraform [global options] version [options]
  ...
  Displays the version of OpenRecipe and all installed plugins
...
func (c *VersionCommand) Run(args []string) int {
	...
	fmt.Fprintf(&versionString, "OpenRecipe v%s", c.Version)
	...
	if jsonOutput {
		...
		if outdated {
			c.Ui.Output(fmt.Sprintf(
				"\nYour version of OpenRecipe is out of date! The latest version\n"+
...
func (c *VersionCommand) Synopsis() string {
	return "Show the current OpenRecipe version"
}
