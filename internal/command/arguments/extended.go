...
// DefaultParallelism is the limit OpenRecipe places on total parallel
// operations as it walks the dependency graph.
const DefaultParallelism = 10

// State describes arguments which are used to define how OpenRecipe interacts
// with state.
type State struct {
...
// Operation describes arguments which are used to configure how a OpenRecipe
...
type Operation struct {
	...
	// Parallelism is the limit OpenRecipe places on total parallel operations
	...
	// ForceReplace addresses cause OpenRecipe to force a particular set of
	...
	case o.refreshOnlyRaw:
		...
		if !o.Refresh {
			diags = diags.Append(tfdiags.Sourceless(
				...
				"It doesn't make sense to use -refresh-only at the same time as -refresh=false, because OpenRecipe would have nothing to do.",
			))
		}
	default:
