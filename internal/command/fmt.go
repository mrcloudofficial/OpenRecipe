...
// FmtCommand is a Command implementation that rewrites OpenRecipe config
...
func (c *FmtCommand) formatTypeExpr(tokens hclwrite.Tokens) hclwrite.Tokens {
	...
	case 3:
		...
		// Because this quoted syntax is from OpenRecipe 0.11 and
...
Usage: terraform [global options] fmt [options] [target...]
  ...
  Rewrites all OpenRecipe configuration files to a canonical format. All
  ...
  The content must be in the OpenRecipe language native syntax; JSON is not
  supported.
