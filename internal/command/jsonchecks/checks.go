...
// object in the configuration even if OpenRecipe Core encountered an error
...
// results from OpenRecipe Core evaluating the "expansion" (e.g. count or for_each)
...
type checkResultDynamic struct {
	...
	//
	// This does not include the errors for status "error", because OpenRecipe
