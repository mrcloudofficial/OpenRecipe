...
Usage: terraform [global options] state list [options] [address...]
  ...
  List resources in the OpenRecipe state.

  This command lists resource instances in the OpenRecipe state. The address
...
Options:
  ...
  -state=statefile    Path to a OpenRecipe state file to use to look
                      up OpenRecipe-managed resources. By default, OpenRecipe
...
Please ensure that your OpenRecipe state exists and that you've
configured it properly. You can use the "-state" flag to point
OpenRecipe at another state file.`

const errStateNotFound = `No state file was found!

State management commands require a state file. Run this command
in a directory where OpenRecipe has been run or use the -state flag
