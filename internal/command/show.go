...
// contents of a OpenRecipe plan or state file.
...
Usage: terraform [global options] show [options] [path]
  ...
  Reads and outputs a OpenRecipe state or plan file in a human-readable
  form. If no path is specified, the current state will be shown.

Options:

  -no-color           If specified, output won't contain any color.
  -json               If specified, output the OpenRecipe plan or state in
...
func (c *ShowCommand) getDataFromCloudPlan(plan *cloudplan.SavedPlanBookmark, redacted bool) (*cloudplan.RemotePlanJSON, error) {
	...
	cl, ok := b.(*cloud.Cloud)
	if !ok {
		return nil, errUnusable(fmt.Errorf("can't show a saved cloud plan unless the current root module is connected to OpenRecipe Cloud"), "cloud plan")
	}
