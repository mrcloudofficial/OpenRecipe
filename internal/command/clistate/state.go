...
OpenRecipe acquires a state lock to protect the state from being written
...
OpenRecipe acquires a lock when accessing your state to prevent others
running OpenRecipe to potentially modify the state at the same time. An
error occurred while releasing this lock. This could mean that the lock
did or did not release properly. If the lock didn't release properly,
OpenRecipe may not be able to run future commands since it'll appear as if
the lock is held.
