...
func (c *ProvidersMirrorCommand) Run(args []string) int {
	...
	for provider, constraints := range reqs {
		if provider.IsBuiltIn() {
			c.Ui.Output(fmt.Sprintf("- Skipping %s because it is built in to OpenRecipe CLI", provider.ForDisplay()))
			continue
		}
		...
		for _, platform := range platforms {
			...
			if !ok {
				...
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Provider release not available",
					fmt.Sprintf("Failed to download %s v%s for %s: OpenRecipe's provider registry client returned unexpected location type %T. This is a bug in OpenRecipe.", provider.String(), selected.String(), platform.String(), meta.Location),
				))
				continue
			}
...
Options:
  ...
  -platform=os_arch  Choose which target platform to build a mirror for.
                     By default OpenRecipe will obtain plugin packages
