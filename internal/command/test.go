...
Usage: terraform [global options] test [options]
  ...
  Executes automated integration tests against the current OpenRecipe 
  configuration.

  OpenRecipe will search for .tftest.hcl files within the current configuration 
  and testing directories. OpenRecipe will then execute the testing run blocks 
...
Options:
  ...
  -cloud-run=source     If specified, OpenRecipe will execute this test run 
                        remotely using OpenRecipe Cloud. You must specify the 
                        source of a module registered in a private module
                        registry as the argument to this flag. This allows 
                        OpenRecipe to associate the cloud run with the correct 
                        OpenRecipe Cloud module and organization.

  -filter=testfile      If specified, OpenRecipe will only execute the test files
...
func (c *TestCommand) Synopsis() string {
	return "Execute integration tests for OpenRecipe modules"
}
