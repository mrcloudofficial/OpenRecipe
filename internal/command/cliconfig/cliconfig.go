...
// variables. The CLI config is not the same thing as a OpenRecipe configuration
// written in the OpenRecipe language; the logic for those lives in the top-level
// directory "configs".
package cliconfig
...
// Config is the structure of the configuration for the OpenRecipe CLI.
//
// This is not the configuration for OpenRecipe itself. That is in the
// "config" package.
type Config struct {
	...
	//
	// This is likely to become a silent no-op in future OpenRecipe versions but
	// is here in recognition of the fact that the dependency lock file is not
	// yet a good fit for all OpenRecipe workflows and folks in that category
...
// ConfigDir returns the configuration directory for OpenRecipe.
...
func LoadConfig() (*Config, tfdiags.Diagnostics) {
	...
	// files because we're doing something special, like running OpenRecipe
