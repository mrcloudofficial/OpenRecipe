...
func collectCredentialsFromEnv() map[svchost.Hostname]string {
	...
	for _, ev := range os.Environ() {
		...
		// that is what the OpenRecipe credentials helper protocol will
		// use in its requests.
		//
		// Using ForDisplay first here makes this more liberal than OpenRecipe
		// itself would usually be in that it will tolerate pre-punycoded
		// hostnames that OpenRecipe normally rejects in other contexts in order
