...
func (p *plan) marshalPlanVariables(vars map[string]plans.DynamicValue, decls map[string]*configs.Variable) error {
	...
	// In OpenRecipe v1.1 and earlier we had some confusion about which subsystem
	// of OpenRecipe was the one responsible for substituting in default values
	...
	// during the OpenRecipe Core graph walk.
	//
	// For OpenRecipe v1.2 and later we rationalized that by having the OpenRecipe
