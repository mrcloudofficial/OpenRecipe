...
func (c *ImportCommand) Run(args []string) int {
	...
	cmdFlags.BoolVar(&c.ignoreRemoteVersion, "ignore-remote-version", false, "continue even if remote and local OpenRecipe versions are incompatible")
	...
	if !c.dirIsConfigPath(configPath) {
		diags = diags.Append(&hcl.Diagnostic{
			...
			Summary:  "No OpenRecipe configuration files",
			Detail: fmt.Sprintf(
				"The directory %s does not contain any OpenRecipe configuration files (.tf or .tf.json). To specify a different configuration directory, use the -config=\"...\" command line option.",
				configPath,
			),
		})
		c.showDiagnostics(diags)
	...
	// import something that OpenRecipe will want to immediately destroy on
	...
	// Check remote OpenRecipe version is compatible
...
Usage: terraform [global options] import [options] ADDR ID
  ...
  Import existing infrastructure into your OpenRecipe state.

  This will find and import the specified resource into your OpenRecipe
  state, allowing existing infrastructure to come under OpenRecipe
  management without having to be initially created by OpenRecipe.
...
Options:
  ...
  -config=path            Path to a directory of OpenRecipe configuration files
  ...
  -var 'foo=bar'          Set a variable in the OpenRecipe configuration. This
                          flag can be set multiple times. This is only useful
                          with the "-config" flag.

  -var-file=foo           Set variables in the OpenRecipe configuration from
...
func (c *ImportCommand) Synopsis() string {
	return "Associate existing infrastructure with a OpenRecipe resource"
}
...
your OpenRecipe state and will henceforth be managed by OpenRecipe.
`
