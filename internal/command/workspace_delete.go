...
func (c *WorkspaceDeleteCommand) Run(args []string) int {
	...
	if hasResources && !force {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf(
				"Workspace %q is currently tracking the following resource instances:%s\n\nDeleting this workspace would cause OpenRecipe to lose track of any associated remote objects, which would then require you to delete them manually outside of OpenRecipe. You should destroy these objects with OpenRecipe before deleting the workspace.\n\nIf you want to delete this workspace anyway, and have OpenRecipe forget about these managed objects, use the -force option to disable this safety check.",
...
Usage: terraform [global options] workspace delete [OPTIONS] NAME
  ...
  Delete a OpenRecipe workspace


Options:

  -force             Remove a workspace even if it is managing resources.
                     OpenRecipe can no longer track or manage the workspace's
                     infrastructure.
