...
func (c *ProvidersLockCommand) Run(args []string) int {
	...
	// ever be inadvertently executed by other OpenRecipe commands -- and then
	...
	for _, platform := range platforms {
			...
			FetchPackageBegin: func(provider addrs.Provider, version getproviders.Version, loc getproviders.PackageLocation) {
				...
				if prevVersion, exists := selectedVersions[provider]; exists && version != prevVersion {
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf(
							"The version constraint for %s selected inconsistent versions for different platforms, which is unexpected.\n\nThe upstream registry may have changed its available versions during OpenRecipe's work. If so, re-running this command may produce a successful result.",
							provider,
						),
					))
				}
		...
		if err != nil {
			diags = diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf("OpenRecipe failed to fetch the requested providers for %s in order to calculate their checksums: %s.", platform, err),
			))
			break
		}
	...
	if madeAnyChange {
		c.Ui.Output(c.Colorize().Color("\n[bold][green]Success![reset] [bold]OpenRecipe has updated the lock file.[reset]"))
		c.Ui.Output("\nReview the changes in .terraform.lock.hcl and then commit to your\nversion control system to retain the new checksums.\n")
	} else {
		c.Ui.Output(c.Colorize().Color("\n[bold][green]Success![reset] [bold]OpenRecipe has validated the lock file and found no need for changes.[reset]"))
	}
	return 0
}
...
Options:
  ...
  -fs-mirror=dir     Consult the given filesystem mirror directory instead
                     ...
                     of valid checksums will be limited only to what OpenRecipe
  ...
  -platform=os_arch  Choose a target platform to request package checksums
                     ...
                     By default OpenRecipe will request package checksums
