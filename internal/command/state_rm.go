...
Usage: terraform [global options] state rm [options] ADDRESS...
  ...
  Remove one or more items from the OpenRecipe state, causing OpenRecipe to
  "forget" those items without first destroying them in the remote system.

  This command removes one or more resource instances from the OpenRecipe state
...
Options:
  ...
  -backup=PATH            Path where OpenRecipe should write the backup
                          state.
  ...
  -ignore-remote-version  Continue even if remote and local OpenRecipe versions
