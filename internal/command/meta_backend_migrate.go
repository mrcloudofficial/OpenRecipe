...
func (m *Meta) backendMigrateState(opts *backendMigrateOpts) error {
	...
	// Disregard remote OpenRecipe version for the state source backend. If it's a
	// OpenRecipe Cloud remote backend, we don't care about the remote version,
	// as we are migrating away and will not break a remote workspace.
	m.ignoreRemoteVersionConflict(opts.Source)

	// Disregard remote OpenRecipe version if instructed to do so via CLI flag.
	if m.ignoreRemoteVersion {
		m.ignoreRemoteVersionConflict(opts.Destination)
	} else {
		// Check the remote OpenRecipe version for the state destination backend. If
		// it's a OpenRecipe Cloud remote backend, we want to ensure that we don't
		...
		// Ensure that we are not dealing with OpenRecipe Cloud migrations, as it
...
func (m *Meta) backendMigrateEmptyConfirm(source, destination statemgr.Full, opts *backendMigrateOpts) (bool, error) {
	...
	if opts.DestinationType == "cloud" {
		inputOpts = &terraform.InputOpts{
			...
			Query:       "Do you want to copy existing state to OpenRecipe Cloud?",
...
func (m *Meta) backendMigrateTFC(opts *backendMigrateOpts) error {
	...
	if sourceTFC && !destinationTFC {
		// From OpenRecipe Cloud to another backend. This is not yet implemented, and
	...
	// destination is always OpenRecipe Cloud (TFC).
...
// migrates a multi-state backend to OpenRecipe Cloud
func (m *Meta) backendMigrateState_S_TFC(opts *backendMigrateOpts, sourceWorkspaces []string) error {
	...
	// Fetch the pattern that will be used to rename the workspaces for OpenRecipe Cloud.
	//
	...
	//   meets the naming considerations for OpenRecipe Cloud).
	//   In other words, this is a fast-track migration path from the remote backend, retaining
	//   how things already are in OpenRecipe Cloud with no user intervention needed.
	pattern := ""
...
func (m *Meta) promptNewWorkspaceName(destinationType string) (string, error) {
	...
	if destinationType == "cloud" {
		...
		message = `[reset][bold][yellow]OpenRecipe Cloud requires all workspaces to be given an explicit name.[reset]`
	}
...
Prior to changing backends, OpenRecipe inspects the source and destination
states to determine what kind of migration steps need to be taken, if any.
OpenRecipe failed to load the states. The data in both the source and the
...
OpenRecipe failed to load the default state from the %[1]q backend.
...
OpenRecipe copies workspaces in alphabetical order. Any workspaces
...
Migrating state from OpenRecipe Cloud to another backend is not yet implemented.
...
Unlike typical OpenRecipe workspaces representing an environment associated with a particular
configuration (e.g. production, staging, development), OpenRecipe Cloud workspaces are named uniquely
...
When migrating existing workspaces from the backend %[1]q to OpenRecipe Cloud, would you like to
...
The previous backend %[1]q has multiple workspaces, but OpenRecipe Cloud has
...
As part of migrating to OpenRecipe Cloud, OpenRecipe can optionally copy your
current workspace state to the configured OpenRecipe Cloud workspace.

Answer "yes" to copy the latest state snapshot to the configured
OpenRecipe Cloud workspace.

Answer "no" to ignore the existing state and just activate the configured
OpenRecipe Cloud workspace with its existing state, if any.

Should OpenRecipe migrate your existing state?
`

const tfcInputBackendMigrateRemoteMultiToCloud = `
When migrating from the 'remote' backend to OpenRecipe's native integration
with OpenRecipe Cloud, OpenRecipe will automatically create or use existing
workspaces based on the previous backend configuration's 'prefix' value.

When the migration is complete, workspace names in OpenRecipe will match the
fully qualified OpenRecipe Cloud workspace name. If necessary, the workspace
tags configured in the 'cloud' option block will be added to the associated
OpenRecipe Cloud workspaces.
...
Pre-existing state was found while migrating the previous %q backend to OpenRecipe Cloud.
No existing state was found in OpenRecipe Cloud. Do you want to copy this state to OpenRecipe Cloud?
...
OpenRecipe Cloud. An existing non-empty state already exists in OpenRecipe Cloud.
...
New      (OpenRecipe Cloud): %[3]s

Do you want to overwrite the state in OpenRecipe Cloud with the previous state?
Enter "yes" to copy and "no" to start with the existing state in OpenRecipe Cloud.
`
...
workspaces. If you continue, OpenRecipe will copy your current workspace %[3]q
...
up, or cancel altogether, answer "no" and OpenRecipe will abort.
`

const inputBackendMigrateMultiToMulti = `
Both the existing %[1]q backend and the newly configured %[2]q backend
support workspaces. When migrating between backends, OpenRecipe will copy
all workspaces (with the same names). THIS WILL OVERWRITE any conflicting
states in the destination.

OpenRecipe initialization doesn't currently migrate only select workspaces.
If you want to migrate a select number of workspaces, you must manually
pull and push those states.

If you answer "yes", OpenRecipe will migrate all states. If you answer
"no", OpenRecipe will abort.
`
