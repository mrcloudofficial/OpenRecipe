...
// ApplyCommand is a Command implementation that applies a OpenRecipe
...
func (c *ApplyCommand) Run(rawArgs []string) int {
	...
	// OpenRecipe operations. At the moment, this value is used both to
	...
	// rendered already in a remote OpenRecipe process.
	...
	if lp, ok := planFile.Local(); ok {
		...
		if plan.Backend.Config == nil {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Failed to read plan from plan file",
				"The given plan file does not have a valid backend configuration. This is a bug in the OpenRecipe command that generated this plan file.",
			))
			return nil, diags
		}
...
Usage: terraform [global options] apply [options] [PLAN]
  ...
  Creates or updates infrastructure according to OpenRecipe configuration
  files in the current directory.

  By default, OpenRecipe will generate a new plan and present it for your
  approval before taking any action. You can optionally provide a plan
  file created by a previous call to "terraform plan", in which case
  OpenRecipe will take the actions described in that plan without any
  confirmation prompt.
...
Options:
  ...
  -compact-warnings      If OpenRecipe produces any warnings that are not
                         accompanied by errors, show them in a more compact
                         form that includes only the summary messages.

  -destroy               Destroy OpenRecipe-managed infrastructure.
...
Usage: terraform [global options] destroy [options]
  ...
  Destroy OpenRecipe-managed infrastructure.
