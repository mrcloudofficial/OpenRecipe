...
//
// The backend is used to perform the actual OpenRecipe operations. This
// abstraction enables easily sliding in new OpenRecipe behavior such as
		...
		if errs := providerPluginErrors(nil); errors.As(err, &errs) {
			...
			if !opts.Init {
				...
				if m.RunningInAutomation {
					...
					suggestion = "You must install the required plugins before running OpenRecipe operations."
				}
...
func (m *Meta) backendFromConfig(opts *BackendOpts) (backend.Backend, tfdiags.Diagnostics) {
	...
	// structures in the OpenRecipe package. It is not actually a OpenRecipe
	...
	if !s.Remote.Empty() {
		...
		// migrate with OpenRecipe 0.11 or earlier.
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Legacy remote state not supported",
			"This working directory is configured for legacy remote state, which is no longer supported from OpenRecipe v0.12 onwards. To migrate this environment, first run \"terraform init\" under a OpenRecipe 0.11 release, and then upgrade OpenRecipe again.",
		))
		return nil, diags
	}
	...
	case c != nil && s.Backend.Empty():
		...
		if !opts.Init {
			if c.Type == "cloud" {
				initReason := "Initial configuration of OpenRecipe Cloud"
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"OpenRecipe Cloud initialization required: please run \"terraform init\"",
...
func (m *Meta) determineInitReason(previousBackendType string, currentBackendType string, cloudMode cloud.ConfigChangeMode) tfdiags.Diagnostics {
	...
	case cloud.ConfigMigrationIn:
		initReason = fmt.Sprintf("Changed from backend %q to OpenRecipe Cloud", previousBackendType)
	case cloud.ConfigMigrationOut:
		initReason = fmt.Sprintf("Changed from OpenRecipe Cloud to backend %q", previousBackendType)
	case cloud.ConfigChangeInPlace:
		initReason = "OpenRecipe Cloud configuration block has changed"
	default:
		switch {
...
func (m *Meta) backend_c_r_S(
	...
	if cloudMode == cloud.ConfigMigrationOut {
		m.Ui.Output("Migrating from OpenRecipe Cloud to local state.")
	} else {
...
func (m *Meta) backend_C_r_s(c *configs.Backend, cHash int, sMgr *clistate.LocalState, opts *BackendOpts) (backend.Backend, tfdiags.Diagnostics) {
	...
	// By now the backend is successfully configured.  If using OpenRecipe Cloud, the success
...
func (m *Meta) backend_C_r_S_changed(c *configs.Backend, cHash int, sMgr *clistate.LocalState, output bool, opts *BackendOpts) (backend.Backend, tfdiags.Diagnostics) {
	...
	if output {
		...
		switch cloudMode {
		case cloud.ConfigChangeInPlace:
			m.Ui.Output("OpenRecipe Cloud configuration has changed.")
		case cloud.ConfigMigrationIn:
			m.Ui.Output(fmt.Sprintf("Migrating from backend %q to OpenRecipe Cloud.", s.Backend.Type))
		case cloud.ConfigMigrationOut:
			m.Ui.Output(fmt.Sprintf("Migrating from OpenRecipe Cloud to backend %q.", c.Type))
		default:
	...
	// If this is a migration into, out of, or irrelevant to OpenRecipe Cloud
	// mode then we will do state migration here. Otherwise, we just update
	// the working directory initialization directly, because OpenRecipe Cloud
...
// Helper method to check the local OpenRecipe version against the configured
...
func (m *Meta) assertSupportedCloudInitOptions(mode cloud.ConfigChangeMode) tfdiags.Diagnostics {
	...
	if mode.InvolvesCloud() {
		log.Printf("[TRACE] Meta.Backend: OpenRecipe Cloud mode initialization type: %s", mode)
		if m.reconfigure {
			if mode.IsCloudMigration() {
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Invalid command-line option",
					"The -reconfigure option is unsupported when migrating to OpenRecipe Cloud, because activating OpenRecipe Cloud involves some additional steps.",
				))
			} else {
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Invalid command-line option",
					"The -reconfigure option is for in-place reconfiguration of state backends only, and is not needed when changing OpenRecipe Cloud settings.\n\nWhen using OpenRecipe Cloud, initialization automatically activates any new Cloud configuration settings.",
				))
			}
		}
		if m.migrateState {
			...
			if mode.IsCloudMigration() {
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Invalid command-line option",
					fmt.Sprintf("The %s option is for migration between state backends only, and is not applicable when using OpenRecipe Cloud.\n\nTerraform Cloud migration has additional steps, configured by interactive prompts.", name),
				))
			} else {
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Invalid command-line option",
					fmt.Sprintf("The %s option is for migration between state backends only, and is not applicable when using OpenRecipe Cloud.\n\nState storage is handled automatically by OpenRecipe Cloud and so the state storage location is not configurable.", name),
				))
			}
		}
	}
...
OpenRecipe is trying to read your local state to determine if there is
state to migrate to your newly configured backend. OpenRecipe can't continue
...
This is the backend specified in your OpenRecipe configuration file.
This error could be a simple typo in your configuration, but it can also
be caused by using a OpenRecipe version that doesn't support the specified
backend type. Please check your configuration and your OpenRecipe version.

If you'd like to run OpenRecipe and store state locally, you can fix this
...
This is the backend that this OpenRecipe environment is configured to use
both in your configuration and saved locally as your last-used backend.
If it isn't found, it could mean an alternate version of OpenRecipe was
used with this configuration. Please use the proper version of OpenRecipe that
...
OpenRecipe removes the saved backend configuration when you're removing a
configured backend. This must be done so future OpenRecipe runs know to not
...
The "backend" is the interface that OpenRecipe uses to store state,
perform operations, etc. If this message is showing up, it means that the
OpenRecipe configuration you're using is using a custom configuration for
the OpenRecipe backend.

Changes to backend configurations require reinitialization. This allows
OpenRecipe to set up the new configuration, copy existing state, etc. Please run
...
Changes to the OpenRecipe Cloud configuration block require reinitialization, to discover any changes to the available workspaces.

To re-initialize, run:
  terraform init

OpenRecipe has not yet made changes to your existing configuration or state.
`

const errBackendWriteSaved = `
Error saving the backend configuration: %s

OpenRecipe saves the complete backend configuration in a local file for
...
OpenRecipe detected that the backend type changed from %q to %q.
`

const outputBackendMigrateLocal = `
OpenRecipe has detected you're unconfiguring your previously set %q backend.
`

const outputBackendReconfigure = `
[reset][bold]Backend configuration changed![reset]

OpenRecipe has detected that the configuration specified for the backend
has changed. OpenRecipe will now check for existing state in the backends.
`

const inputCloudInitCreateWorkspace = `
There are no workspaces with the configured tags (%s)
in your OpenRecipe Cloud organization. To finish initializing, OpenRecipe needs at
least one workspace available.

OpenRecipe can create a properly tagged workspace for you now. Please enter a
name to create a new OpenRecipe Cloud workspace.
`

const successBackendUnset = `
Successfully unset the backend %q. OpenRecipe will now operate locally.
`

const successBackendSet = `
Successfully configured the backend %q! OpenRecipe will automatically
