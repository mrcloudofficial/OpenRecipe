...
func TestTest_Runs(t *testing.T) {
	tcs := map[string]struct {
		...
		"destroy_fail": {
			expectedOut:           "1 passed, 0 failed.",
			expectedErr:           `OpenRecipe left the following resources in state`,
			code:                  1,
...
func TestTest_DoubleInterrupt(t *testing.T) {
	...
	cleanupMessage := `OpenRecipe was interrupted while executing main.tftest.hcl, and may not have
...
OpenRecipe specifically suggests to use it as part of an error message.
...
or when OpenRecipe specifically suggests to use it as part of an error
message.
