...
func (c *UnlockCommand) Run(args []string) int {
	...
	if !force {
		...
		desc := "OpenRecipe will remove the lock on the remote state.\n" +
			"This will allow local OpenRecipe commands to modify this state, even though it\n" +
...
[reset][bold][green]OpenRecipe state has been successfully unlocked![reset][green]

The state has been unlocked, and OpenRecipe commands should now be able to
