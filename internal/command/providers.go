...
func (c *ProvidersCommand) Run(args []string) int {
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("OpenRecipe encountered an unexpected error while verifying that the given configuration directory is valid: %s.", err),
		))
	...
	if empty {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("The directory %s contains no OpenRecipe configuration files.", absPath),
		))
		c.showDiagnostics(diags)
		return 1
	}
