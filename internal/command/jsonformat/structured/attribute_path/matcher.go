...
func (p *PathMatcher) GetChildWithIndex(index int) Matcher {
	...
	for _, path := range p.Paths {
		...
		// OpenRecipe actually allows user to provide strings into indexes as
		...
		//    - test_resource.resource.list["0"].attribute
		//
		// Note, that OpenRecipe will raise a validation error if the string
		...
		case string:
			...
			if err != nil {
				panic(fmt.Errorf("found invalid type within path (%v:%T), the validation shouldn't have allowed this to happen; this is a bug in OpenRecipe, please report it", val, val))
			}
			if int(f) == index {
				child.Paths = append(child.Paths, path[1:])
			}
