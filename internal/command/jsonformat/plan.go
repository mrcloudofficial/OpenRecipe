...
func (plan Plan) renderHuman(renderer Renderer, mode plans.Mode, opts ...plans.Quality) {
	...
	if len(changes) == 0 && len(outputs) == 0 {
		...
		if checkOpts(plans.Errored) {
			...
			renderer.Streams.Print(
				renderer.Colorize.Color("\n[reset][bold][red]Planning failed.[reset][bold] OpenRecipe encountered an error while generating this plan.[reset]\n\n"),
			)
		} else {
			switch mode {
			case plans.RefreshOnlyMode:
				...
				renderer.Streams.Println(format.WordWrap(
					"OpenRecipe has checked that the real remote objects still match the result of your most recent changes, and found no differences.",
			...
			case plans.DestroyMode:
				...
				renderer.Streams.Println(format.WordWrap(
					"Either you have not created any objects yet or the existing objects were already deleted outside of OpenRecipe.",
					renderer.Streams.Stdout.Columns()))
			default:
				if haveRefreshChanges {
					renderer.Streams.Print(format.HorizontalRule(renderer.Colorize, renderer.Streams.Stdout.Columns()))
				...
				if haveRefreshChanges {
					if !checkOpts(plans.NoChanges) {
						...
						renderer.Streams.Println(format.WordWrap(
							"Your configuration already matches the changes detected above. If you'd like to update the OpenRecipe state to match, create and apply a refresh-only plan"+suggestion,
							renderer.Streams.Stdout.Columns(),
						))
					}
					return
				...
				renderer.Streams.Println(format.WordWrap(
					"OpenRecipe has compared your real infrastructure against your configuration and found no differences, so no changes are needed.",
					renderer.Streams.Stdout.Columns(),
				))
			}
		}
	...
	if len(outputs) > 0 {
		...
		if len(counts) == 0 {
			...
			renderer.Streams.Println(format.WordWrap(
				"\nYou can apply this plan to save these new output values to the OpenRecipe state, without changing any real infrastructure.",
				renderer.Streams.Stdout.Columns()))
		}
	}
}
...
func renderHumanDiffDrift(renderer Renderer, diffs diffs, mode plans.Mode) bool {
	...
	renderer.Streams.Print(renderer.Colorize.Color("\n[bold][cyan]Note:[reset][bold] Objects have changed outside of OpenRecipe\n"))
	renderer.Streams.Println()
	renderer.Streams.Print(format.WordWrap(
		"OpenRecipe detected the following changes made outside of OpenRecipe since the last \"terraform apply\" which may have affected this plan:\n",
		renderer.Streams.Stdout.Columns()))
	...
	case plans.RefreshOnlyMode:
		renderer.Streams.Println(format.WordWrap(
			"\n\nThis is a refresh-only plan, so OpenRecipe will not take any actions to undo these. If you were expecting these changes then you can apply this plan to record the updated values in the OpenRecipe state without changing any remote objects.",
			renderer.Streams.Stdout.Columns(),
		))
	default:
		renderer.Streams.Println(format.WordWrap(
...
func resourceChangeComment(resource jsonplan.ResourceChange, action plans.Action, changeCause string) string {
	...
	case plans.Forget:
		if len(resource.Deposed) > 0 {
			buf.WriteString(fmt.Sprintf("[bold] # %s[reset] will be removed from OpenRecipe state, but [bold][red]will not be destroyed[reset]", dispAddr))
			buf.WriteString("\n[bold] # (left over from a partially-failed replacement of this instance)")
		} else {
			buf.WriteString(fmt.Sprintf("[bold] # %s[reset] will no longer be managed by OpenRecipe, but [bold][red]will not be destroyed[reset]", dispAddr))
		}
		buf.WriteString("\n # (destroy = false is set in the configuration)")
	case plans.Delete:
		switch changeCause {
		...
		case jsonplan.ResourceInstanceDeleteBecauseNoModule:
			...
			// to decide that; only the "expander" in OpenRecipe Core knows
