...
type Installer struct {
	...
	// namespace, which we use for providers that are built in to OpenRecipe
	...
	// considered implemented, but that OpenRecipe does not manage the
...
// to OpenRecipe running on another operating system or CPU architecture.
...
// built in to OpenRecipe and thus do not need a separate installation step.
//
...
// indicated by the passed addrs.Providers as unmanaged. OpenRecipe does not
// need to control the lifecycle of these providers, and they are assumed to be
// running already when OpenRecipe is started. Because these are essentially
// processes, not binaries, OpenRecipe will not do any work to ensure presence
		...
		if provider.IsBuiltIn() {
			...
			if valid {
				if len(versionConstraints) == 0 {
					...
					// that is built in to the current OpenRecipe release.
					err = fmt.Errorf("built-in providers do not support explicit version constraints")
				}
			} else {
				err = fmt.Errorf("this OpenRecipe release has no built-in provider named %q", provider.Type)
			}
			if err != nil {
				errs[provider] = err
...
NeedProvider:
	...
	for provider, version := range need {
		...
		if i.globalCacheDir != nil {
			...
			if cached := i.globalCacheDir.ProviderVersion(provider, version); cached != nil {
				...
				if acceptablePackage {
					...
					if new == nil {
						err := fmt.Errorf("after linking %s from provider cache at %s it is still not detected in the target directory; this is a bug in OpenRecipe", provider, i.globalCacheDir.baseDir)
						errs[provider] = err
						if cb := evts.LinkFromCacheFailure; cb != nil {
							cb(provider, version, err)
						}
		...
		if new == nil {
			err := fmt.Errorf("after installing %s it is still not detected in %s; this is a bug in OpenRecipe", provider, installTo.BasePath())
			errs[provider] = err
			if cb := evts.FetchPackageFailure; cb != nil {
				cb(provider, version, err)
			}
		...
		if linkTo != nil {
			...
			if new == nil {
				err := fmt.Errorf("after installing %s it is still not detected in %s; this is a bug in OpenRecipe", provider, linkTo.BasePath())
				errs[provider] = err
				if cb := evts.FetchPackageFailure; cb != nil {
					cb(provider, version, err)
				}
