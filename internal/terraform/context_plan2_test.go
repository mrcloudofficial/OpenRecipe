...
func TestContext2Plan_removedDuringRefresh(t *testing.T) {
	...
	// state has been deleted outside of OpenRecipe, which we should detect
...
func TestContext2Plan_resourceChecksInExpandedModule(t *testing.T) {
	...
	//
	// In earlier versions of OpenRecipe we did that expansion as two levels
...
`})
	...
	// OpenRecipe that made it invalid, although technically in that scenario
...
func TestContext2Plan_managedResourceChecksOtherManagedResourceChange(t *testing.T) {
	...
	// This doesn't work because OpenRecipe can't tell that the data resource
...
locals {
	...
	# cause OpenRecipe to defer the data resource to the apply phase due to
	...
	# so OpenRecipe can't "see" that the data resource result might be affected
...
func TestContext2Plan_movedResourceCollision(t *testing.T) {
	...
	wantDiags := tfdiags.Diagnostics{
		tfdiags.Sourceless(
			...
			`OpenRecipe tried to adjust resource instance addresses in the prior state based on change information recorded in the configuration, but some adjustments did not succeed due to existing objects already at the intended addresses:
  - test_object.a[0] could not move to test_object.a

OpenRecipe has planned to destroy these objects. If OpenRecipe's proposed changes aren't appropriate, you must first resolve the conflicts using the "terraform state" subcommands and then create a new plan.`,
		),
	}.ForRPC()
...
func TestContext2Plan_movedResourceCollisionDestroy(t *testing.T) {
	...
	wantDiags := tfdiags.Diagnostics{
		tfdiags.Sourceless(
			...
			// because it says "OpenRecipe has planned to destroy these objects"
...
`,
	...
	for _, diag := range diags {
		...
		if desc.Summary == "Module output value precondition failed" {
			...
			if got, want := desc.Detail, "The error expression used to explain this condition refers to sensitive values, so OpenRecipe will not display the resulting message."; !strings.Contains(got, want) {
				t.Errorf("unexpected detail\ngot: %s\nwant to contain %q", got, want)
			}
		} else {
			t.Errorf("unexpected summary\ngot: %s", desc.Summary)
...
`,
	...
	wantDiags := tfdiags.Diagnostics{
		tfdiags.Sourceless(
			...
			`If you apply this plan, OpenRecipe will discard its tracking information for the following objects, but it will not delete them:
 - test_object.a

After applying this plan, OpenRecipe will no longer manage these objects. You will need to import them into OpenRecipe to manage them again.`,
		),
	}.ForRPC()
	if diff := cmp.Diff(wantDiags, gotDiags); diff != "" {
		t.Errorf("wrong diagnostics\n%s", diff)
