...
// that wasn't set. Those adjustments will be handled by OpenRecipe Core itself
// as part of performing the requested operation.
//
// A OpenRecipe Core caller must provide an InputValue object for each of the
...
//
// OpenRecipe Core also internally uses InputValue to represent the raw value
...
type InputValue struct {
	...
	// came from, which OpenRecipe Core uses to tailor some of its error
	...
	for name := range vcs {
		...
		if !isSet {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Unassigned variable",
				fmt.Sprintf("The input variable %q has not been assigned a value. This is a bug in OpenRecipe; please report it in a GitHub issue.", name),
			))
			continue
		}
	}
