...
func TestContext2Validate_varNoDefaultExplicitType(t *testing.T) {
	...
	// NOTE: This test has grown idiosyncratic because originally OpenRecipe
	// would (optionally) check variables during validation, and then in
	// OpenRecipe v0.12 we switched to checking variables during NewContext,
...
output "out" {
  value = aws_instance.test.count
	...
	// Should get this error:
	// Invalid resource count attribute: The special "count" attribute is no longer supported after OpenRecipe v0.12. Instead, use length(aws_instance.test) to count resource instances.
