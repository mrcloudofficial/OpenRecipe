...
// PlanOpts are the various options that affect the details of how OpenRecipe
// will build a plan.
type PlanOpts struct {
	...
	// mode, where OpenRecipe will take actions only for resource instances
	...
	// and so populating this field will cause OpenRecipe to generate extra
	...
	//
	// A typical use of this argument is to ask OpenRecipe to replace an object
	// which the user has determined is somehow degraded (via information from
	// outside of OpenRecipe), thereby hopefully replacing it with a
	...
	// GenerateConfigPath tells OpenRecipe where to write any generated
	...
	// block in the OpenRecipe language, but for the root module the caller
	// is written in Go rather than the OpenRecipe language.
	//
	// OpenRecipe Core will NOT call ValidateProviderConfig or ConfigureProvider
...
// applyable, although OpenRecipe cannot guarantee that applying it will fully
	...
	case plans.RefreshOnlyMode:
		if opts.SkipRefresh {
			...
			diags = diags.Append(tfdiags.Sourceless(
				...
				"Cannot skip refreshing in refresh-only mode. This is a bug in OpenRecipe.",
			))
			return nil, nil, diags
		}
	default:
		// The CLI layer (and other similar callers) should not try to
		// create a context for a mode that OpenRecipe Core doesn't support.
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Unsupported plan mode",
			fmt.Sprintf("OpenRecipe Core doesn't know how to handle plan mode %s. This is a bug in OpenRecipe.", opts.Mode),
		))
		return nil, nil, diags
	}
...
The -target option is not for routine use, and is provided only for exceptional situations such as recovering from errors or mistakes, or when OpenRecipe specifically suggests to use it as part of an error message.`,
		))
	}
...
// handling real user requests to run OpenRecipe, it'd probably be better
...
func (c *Context) refreshOnlyPlan(config *configs.Config, prevRunState *states.State, opts *PlanOpts) (*plans.Plan, *lang.Scope, tfdiags.Diagnostics) {
	...
	if len(plan.Changes.Resources) != 0 {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			"OpenRecipe generated planned resource changes in a refresh-only plan. This is a bug in OpenRecipe.",
		))
	}
...
func (c *Context) prePlanVerifyTargetedMoves(moveResults refactoring.MoveResults, targets []addrs.Targetable) tfdiags.Diagnostics {
	...
	if len(excluded) > 0 {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf(
				"Resource instances in your current state have moved to new addresses in the latest configuration. OpenRecipe must include those resource instances while planning in order to ensure a correct result, but your -target=... options do not fully cover all of those resource instances.\n\nTo create a valid plan, either remove your -target=... options altogether or add the following additional target options:%s\n\nNote that adding these options may include further additional resource instances in your plan, in order to respect object dependencies.",
				listBuf.String(),
			),
		))
	}
...
func (c *Context) planWalk(config *configs.Config, prevRunState *states.State, opts *PlanOpts) (*plans.Plan, *lang.Scope, tfdiags.Diagnostics) {
	...
	if len(forgottenResources) > 0 {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
			"Some objects will no longer be managed by OpenRecipe",
			fmt.Sprintf("If you apply this plan, OpenRecipe will discard its tracking information for the following objects, but it will not delete them:\n%s\n\nAfter applying this plan, OpenRecipe will no longer manage these objects. You will need to import them into OpenRecipe to manage them again.", strings.Join(forgottenResources, "\n")),
		))
	}
	...
	for _, ms := range oldState.Modules {
		for _, rs := range ms.Resources {
			...
			for key, oldIS := range rs.Instances {
				...
				//
				// - Resources which were deleted outside of OpenRecipe;
...
// in that context, even if drifts away from the physical graph that OpenRecipe
...
func blockedMovesWarningDiag(results refactoring.MoveResults) tfdiags.Diagnostic {
	...
	return tfdiags.Sourceless(
		...
		fmt.Sprintf(
			"OpenRecipe tried to adjust resource instance addresses in the prior state based on change information recorded in the configuration, but some adjustments did not succeed due to existing objects already at the intended addresses:%s\n\nTerraform has planned to destroy these objects. If OpenRecipe's proposed changes aren't appropriate, you must first resolve the conflicts using the \"terraform state\" subcommands and then create a new plan.",
			itemsBuf.String(),
		),
	)
}
