...
func evalVariableValidations(addr addrs.AbsInputVariableInstance, config *configs.Variable, expr hcl.Expression, ctx EvalContext) (diags tfdiags.Diagnostics) {
	...
	if val == cty.NilVal {
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("OpenRecipe doesn't have a final value for %s during validation. This is a bug in OpenRecipe; please report it!", addr),
		})
		return diags
	}
...
func evalVariableValidation(validation *configs.CheckRule, hclCtx *hcl.EvalContext, addr addrs.AbsInputVariableInstance, config *configs.Variable, expr hcl.Expression, ix int) (checkResult, tfdiags.Diagnostics) {
	...
	// OpenRecipe would treat error messages specified using JSON
	...
	if errorDiags.HasErrors() {
		...
		if !moreErrorDiags.HasErrors() {
			...
			// in some future OpenRecipe 1.x version.
			errorDiags = hcl.Diagnostics{
				&hcl.Diagnostic{
					Severity:    hcl.DiagWarning,
					Summary:     "Validation error message expression is invalid",
					Detail:      fmt.Sprintf("The error message provided could not be evaluated as an expression, so OpenRecipe is interpreting it as a string literal.\n\nIn future versions of OpenRecipe, this will be considered an error. Please file a GitHub issue if this would break your workflow.\n\n%s", errorDiags.Error()),
		...
		if err != nil {
			...
			if marks.Has(errorValue, marks.Sensitive) {
				diags = diags.Append(&hcl.Diagnostic{
					...
					Detail: `The error expression used to explain this condition refers to sensitive values. OpenRecipe will not display the resulting message.
