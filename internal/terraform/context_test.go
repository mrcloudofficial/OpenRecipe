...
`
	...
	// operations we typically fail more deeply in OpenRecipe Core, with
		...
		t.Run(testName, func(t *testing.T) {
			...
			wantDiags = wantDiags.Append(
				tfdiags.Sourceless(
					...
					"This configuration requires built-in provider terraform.io/builtin/nonexist, but that provider isn't available in this OpenRecipe version.",
				),
				tfdiags.Sourceless(
					tfdiags.Error,
					"Missing required provider",
				...
				tfdiags.Sourceless(
					...
					`This configuration requires provisioner plugin "nonexist", which isn't available. If you're intending to use an external provisioner plugin, you must install it manually into one of the plugin search directories before running OpenRecipe.`,
				),
			)
