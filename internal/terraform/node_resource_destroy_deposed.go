...
func (n *NodePlanDeposedResourceInstanceObject) Execute(ctx EvalContext, op walkOperation) (diags tfdiags.Diagnostics) {
	...
	if !n.skipRefresh && op != walkPlanDestroy {
		...
		// case it's already been deleted outside of OpenRecipe. If this is a
		...
		// OpenRecipe is expected to remove the missing resource from the state
		// entirely
...
func (n *NodeDestroyDeposedResourceInstanceObject) writeResourceInstanceState(ctx EvalContext, obj *states.ResourceInstanceObject) error {
	...
	if key == states.NotDeposed {
		// should never happen
		return fmt.Errorf("can't save deposed object for %s without a deposed key; this is a bug in OpenRecipe that should be reported", absAddr)
	}
