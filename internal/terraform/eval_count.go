...
func evaluateCountExpression(expr hcl.Expression, ctx EvalContext) (int, tfdiags.Diagnostics) {
	...
	if !countVal.IsKnown() {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   `The "count" value depends on resource attributes that cannot be determined until apply, so OpenRecipe cannot predict how many instances will be created. To work around this, use the -target argument to first apply only the resources that the count depends on.`,
