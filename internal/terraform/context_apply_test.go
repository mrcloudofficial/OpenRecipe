...
func TestContext2Apply_stop(t *testing.T) {
	...
	if got, want := atomic.LoadUint32(&applyStopped), uint32(1); got != want {
		...
		// to inadvertently stop OpenRecipe before it reaches te apply phase,
...
// across several different OpenRecipe subsystems. This is here because
// historically there were quirky differences in handling across different
// parts of OpenRecipe and so here we want to assert the expected behavior and
...
func TestContext2Apply_multiVarMissingState(t *testing.T) {
	...
	// Before the relevant bug was fixed, OpenRecipe would panic during apply.
	if _, diags := ctx.Apply(plan, m, nil); diags.HasErrors() {
...
func TestContext2Apply_errorDestroy(t *testing.T) {
	...
	p.PlanResourceChangeFn = func(req providers.PlanResourceChangeRequest) providers.PlanResourceChangeResponse {
		// Should actually be called for this test, because OpenRecipe Core
...
func TestContext2Apply_errorCreateInvalidNew(t *testing.T) {
	...
	p.ApplyResourceChangeFn = func(req providers.ApplyResourceChangeRequest) providers.ApplyResourceChangeResponse {
		...
		// because we want to test that OpenRecipe ignores the inconsistency
	...
	if got, want := len(diags), 1; got != want {
		// There should be no additional diagnostics generated by OpenRecipe's own eval logic,
...
func TestContext2Apply_errorUpdateNullNew(t *testing.T) {
	...
	p.ApplyResourceChangeFn = func(req providers.ApplyResourceChangeRequest) providers.ApplyResourceChangeResponse {
		...
		// test that OpenRecipe retains the prior state, rather than treating
...
func TestContext2Apply_vars(t *testing.T) {
	...
	variables := InputValues{
		...
		"bar": &InputValue{
			...
			// has a declared default, which OpenRecipe Core will use instead.
			Value:      cty.NilVal,
			SourceType: ValueFromCaller,
		},
...
func TestContext2Apply_scaleInMultivarRef(t *testing.T) {
	...
	assertNoErrors(t, diags)
	{
		...
		// This test was originally written with OpenRecipe v0.11 and earlier
