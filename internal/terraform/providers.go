...
func checkExternalProviders(rootCfg *configs.Config, got map[addrs.RootProviderConfig]providers.Interface) tfdiags.Diagnostics {
	...
	// we can't be precise because OpenRecipe permits implicit default provider
	// configurations.)
...
// and pre-configured.
//
// This is a kinda-hacky way to deal with the fact that OpenRecipe Core
...
// be pre-configured before passing them to OpenRecipe Core.
