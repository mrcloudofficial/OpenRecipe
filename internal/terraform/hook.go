...
// callback. This allows you to modify the behavior of OpenRecipe at runtime.
...
const (
	...
	// and the action that OpenRecipe was about to take is cancelled.
	HookActionHalt
)

// Hook is the interface that must be implemented to hook into various
// parts of OpenRecipe, allowing you to inspect or change behavior at runtime.
//
// There are MANY hook points into OpenRecipe. If you only want to implement
...
type Hook interface {
	...
	// Stopping is called if an external signal requests that OpenRecipe
	// gracefully abort an operation in progress.
	//
	// This notification might suggest that the user wants OpenRecipe to exit
	// ASAP and in that case it's possible that if OpenRecipe runs for too much
	...
	//
	// This hook cannot control whether OpenRecipe continues, because the
