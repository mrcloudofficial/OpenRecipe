...
type ContextOpts struct {
	...
	//
	// The preloaded schemas do not need to be exhaustive. OpenRecipe will
	...
	// been passed to OpenRecipe Core using this field.
...
// into OpenRecipe in various ways. This must be provided by the Context
// initializer.
type ContextMeta struct {
	Env string // Env is the state environment

	// OriginalWorkingDir is the working directory where the OpenRecipe CLI
...
// Context represents all the context that OpenRecipe needs in order to
	...
	go func() {
...

		{
			...
			for _, p := range ps {
				...
				// advisory: OpenRecipe will exit once the graph node completes.
				p.Stop()
			}
		}
...
//
// Currently this function checks whether the current OpenRecipe CLI version
...
func (c *Context) checkConfigDependencies(config *configs.Config) tfdiags.Diagnostics {
	...
	// This checks the OpenRecipe CLI version constraints specified in all of
	// the modules.
		...
		if !c.plugins.HasProvider(providerAddr) {
			...
			if !providerAddr.IsBuiltIn() {
				...
				diags = diags.Append(tfdiags.Sourceless(
					...
					"Missing required provider",
					fmt.Sprintf(
						"This configuration requires built-in provider %s, but that provider isn't available in this OpenRecipe version.",
						providerAddr,
					),
				))
			}
			...
			for _, pc := range rc.Managed.Provisioners {
				if !c.plugins.HasProvisioner(pc.Type) {
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf(
							"This configuration requires provisioner plugin %q, which isn't available. If you're intending to use an external provisioner plugin, you must install it manually into one of the plugin search directories before running OpenRecipe.",
							pc.Type,
						),
					))
				}
