...
func (d *evaluationStateData) GetInputVariable(addr addrs.InputVariable, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	// that are disabled, etc. OpenRecipe's static validation leans towards
...
func (d *evaluationStateData) GetResource(addr addrs.Resource, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	if schema == nil {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("No schema is available for %s in %s. This is a bug in OpenRecipe and should be reported.", addr, config.Provider),
			Subject:  rng.ToHCL().Ptr(),
		})
		return cty.DynamicVal, diags
	}
		...
		if is.Current.Status == states.ObjectPlanned {
			if change == nil {
				...
				diags = diags.Append(&hcl.Diagnostic{
					...
					Detail:   fmt.Sprintf("Instance %s is marked as having a change pending but that change is not recorded in the plan. This is a bug in OpenRecipe; please report it.", instAddr),
...
func (d *evaluationStateData) GetTerraformAttr(addr addrs.TerraformAttr, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	case "env":
		// Prior to OpenRecipe 0.12 there was an attribute "env", which was
...
func (d *evaluationStateData) GetOutput(addr addrs.OutputValue, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	if output == nil {
		...
		// Then we did get a value but OpenRecipe itself thought it was NilVal
...
func (d *evaluationStateData) GetCheckBlock(addr addrs.Check, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	diags = diags.Append(&hcl.Diagnostic{
		...
		Detail:   "The \"check\" object can only be referenced from an \"expect_failures\" attribute within a OpenRecipe testing \"run\" block.",
		Subject:  rng.ToHCL().Ptr(),
	})
