...
// ApplyOpts are options that affect the details of how OpenRecipe will apply a
...
Note that the -target option is not suitable for routine use, and is provided only for exceptional situations such as recovering from errors or mistakes, or when OpenRecipe specifically suggests to use it as part of an error message.`,
		))
	}
...
func (c *Context) applyGraph(plan *plans.Plan, config *configs.Config, opts *ApplyOpts, validate bool) (*Graph, walkOperation, tfdiags.Diagnostics) {
	...
	// which case OpenRecipe will once again use the default value from the
