...
func (c *ComponentInstance) CheckInputVariableValues(ctx context.Context, phase EvalPhase) (cty.Value, tfdiags.Diagnostics) {
	...
	for _, path := range stackconfigtypes.ProviderInstancePathsInValue(v) {
		err := path.NewErrorf("cannot send provider configuration reference to OpenRecipe module input variable")
		diags = diags.Append(&hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "Invalid inputs for component",
			Detail: fmt.Sprintf(
...
// main OpenRecipe modules runtime expects.
	...
	for _, inCalleeAddr := range neededConfigs {
		...
		if !v.IsKnown() {
			...
			diags = diags.Append(&hcl.Diagnostic{
				...
				Summary:  errSummary,
				Detail: fmt.Sprintf(
					"This expression depends on values that won't be known until the apply phase, so OpenRecipe cannot determine which provider configuration to use while planning changes for %s.",
					c.Addr().String(),
				),
				Subject: result.Expression.Range().Ptr(),
			})
...
func (c *ComponentInstance) CheckModuleTreePlan(ctx context.Context) (*plans.Plan, tfdiags.Diagnostics) {
	...
	return doOnceWithDiags(
		...
		func(ctx context.Context) (*plans.Plan, tfdiags.Diagnostics) {
			...
			// This is our main bridge from the stacks language into the main OpenRecipe
			...
			if err != nil {
				...
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Failed to instantiate OpenRecipe modules runtime",
					fmt.Sprintf("Could not load the main OpenRecipe language runtime: %s.\n\nThis is a bug in OpenRecipe; please report it!", err),
				))
				return nil, diags
			}
...
func (c *ComponentInstance) ApplyModuleTreePlan(ctx context.Context, plan *plans.Plan) (*ComponentInstanceApplyResult, tfdiags.Diagnostics) {
	...
	// the goal of producing a traditional OpenRecipe CLI-style saved plan
...
func (c *ComponentInstance) CheckApplyResult(ctx context.Context) (*ComponentInstanceApplyResult, tfdiags.Diagnostics) {
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("OpenRecipe needs the result from applying changes to %s, but that apply was apparently not scheduled to run. This is a bug in OpenRecipe.", c.Addr()),
		))
	}
...
func (c *ComponentInstance) ResultValue(ctx context.Context, phase EvalPhase) cty.Value {
	...
	case PlanPhase:
		...
		// parts of OpenRecipe use UIMode == DestroyMode in particular to deal
...
// arguments are suitable, and then asking the main OpenRecipe language runtime
		...
		for _, rsrcChange := range corePlan.Changes.Resources {
			...
			if err != nil {
				diags = diags.Append(tfdiags.Sourceless(
					...
					fmt.Sprintf(
						"Failed to retrieve the schema for %s from provider %s: %s. This is a bug in OpenRecipe.",
			...
			changes = append(changes, &stackplan.PlannedChangeResourceInstancePlanned{
				...
				// OpenRecipe Core's heuristics. Only the external plan
				// description needs that info, to populate the
				// "changes outside of OpenRecipe" part of the plan UI;
			...
			if os != nil {
				...
				if rs == nil {
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf(
							"There is a state for %s specifically, but somehow no state for its containing resource %s. This is a bug in OpenRecipe.",
							rioAddr, rAddr,
						),
					))
					continue
				...
				if err != nil {
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf(
							"Failed to retrieve the schema for %s from provider %s: %s. This is a bug in OpenRecipe.\n\nThe new state for this object cannot be saved. If this object was only just created, you may need to delete it manually in the target system to reconcile with the OpenRecipe state before trying again.",
...
func (c *ComponentInstance) resourceTypeSchema(ctx context.Context, providerTypeAddr addrs.Provider, mode addrs.ResourceMode, typ string) (*configschema.Block, error) {
	...
	// robustness but any error here suggests a bug in OpenRecipe.
