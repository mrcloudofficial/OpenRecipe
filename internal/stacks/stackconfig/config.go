...
type Config struct {
	...
	//
	// This is also the source bundle that any OpenRecipe modules used by
...
func resolveFinalSourceAddr(base sourceaddrs.FinalSource, rel sourceaddrs.Source, versionConstraints constraints.IntersectionSpec, sources *sourcebundle.Bundle) (sourceaddrs.FinalSource, error) {
	...
	default:
		...
		return nil, fmt.Errorf("cannot resolve final source address for %T (this is a bug in OpenRecipe)", rel)
	}
}
