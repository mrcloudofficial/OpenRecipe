...
// resources are the most important object in a OpenRecipe module: each one
// refers to a OpenRecipe module that describes the infrastructure that the
...
type Component struct {
	...
	// data type vs. the main OpenRecipe language's treatment of provider
...
func decodeComponentBlock(block *hcl.Block) (*Component, tfdiags.Diagnostics) {
	...
	if attr, ok := content.Attributes["providers"]; ok {
		...
		// it's populating an inflexible part of OpenRecipe Core's input.
		// This argument, if present, must always be an object constructor
		// whose attributes are OpenRecipe Core-style provider configuration
