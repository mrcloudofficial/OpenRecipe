...
// keys are to be considered opaque by anything other than OpenRecipe Core.
//
...
//     OpenRecipe Core will raise an error during state decoding if it encounters
...
//     key type is "preserved". When OpenRecipe Core encounters an unrecognized
...
//     If the key type is "preserved" then OpenRecipe Core will just ignore it
...
// forward-compatibility by allowing an older version of OpenRecipe Core to,
// when it's safe to do so, silently discard or preserve objects that were
// presumably added by a later version of OpenRecipe. When we add new key types
...
// object could cause an older version of OpenRecipe Core to misinterpret
