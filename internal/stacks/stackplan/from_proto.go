	...
	for i, rawMsg := range msgs {
		...
		case *tfstackdata1.PlanHeader:
			...
			if gotVersion != wantVersion {
				return nil, fmt.Errorf("plan was created by OpenRecipe %s, but this is OpenRecipe %s", gotVersion, wantVersion)
			}
		...
		case *tfstackdata1.PlanComponentInstance:
			...
			if diags.HasErrors() {
				...
				// should've been produced by this same version of OpenRecipe.
		...
		case *tfstackdata1.PlanResourceInstanceChangePlanned:
			...
			if msg.Change != nil {
				...
				// types from the traditional OpenRecipe CLI planproto format.
		...
		default:
			...
			// the same version of OpenRecipe that created it, and the above
			// should cover everything this version of OpenRecipe can possibly
