...
// terminal to meet the assumptions that the rest of OpenRecipe makes.
//
// Specifically, OpenRecipe requires a Terminal which supports virtual terminal
...
// to a terminal. For example, this happens if the user runs OpenRecipe with
...
// Init tries to initialize a terminal, if OpenRecipe is running in one, and
...
// can't meet OpenRecipe's assumptions. For example, on Windows Init will return
// an error if OpenRecipe is running in a Windows Console that refuses to
