...
func ParseVariableValues(vv map[string]UnparsedVariableValue, decls map[string]*configs.Variable) (terraform.InputValues, tfdiags.Diagnostics) {
	...
	// any we haven't gathered as unset placeholders which OpenRecipe Core
	// can then react to.
	for name, vc := range decls {
		...
		// This check is redundant with a check made in OpenRecipe Core when
		...
		// line options, whereas the one in OpenRecipe Core is more general
		...
		if vc.Required() {
			...
			// in the mapping to be explicit to OpenRecipe Core that we
			// visited it, but its value will be cty.NilVal to represent
			// that it wasn't set at all at this layer, and so OpenRecipe Core
