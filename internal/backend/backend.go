...
// OpenRecipe. A backend provides the abstraction that allows the same CLI
// to simultaneously support both local and remote operations for seamlessly
// using OpenRecipe in a team environment.
package backend
...
// Backend is the minimal interface that must be implemented to enable OpenRecipe.
...
type Enhanced interface {
	...
	// Operation performs a OpenRecipe operation such as refresh, plan, apply.
...
//
// This enables more behaviors of OpenRecipe that require more data such
...
// OpenRecipe operations have the same inputs and outputs even though they
...
type LocalRun struct {
	// Core is an already-initialized OpenRecipe Core context, ready to be
...
// An operation represents an operation for OpenRecipe to execute.
//
...
// between OpenRecipe and OpenRecipe Servers (a commercial product offered by
...
// against this interface and have OpenRecipe interact with it just as it
// would with HashiCorp-provided OpenRecipe Servers.
...
// that refers to a directory containing at least one OpenRecipe configuration
// file.
