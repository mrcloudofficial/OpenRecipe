...
func (b *Remote) Configure(obj cty.Value) tfdiags.Diagnostics {
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Failed to create the OpenRecipe Enterprise client",
			fmt.Sprintf(
				`The "remote" backend encountered an unexpected error while creating the `+
					`OpenRecipe Enterprise client: %s.`, err,
			),
		))
		return diags
	}
...
func (b *Remote) checkConstraints(c *disco.Constraints) tfdiags.Diagnostics {
	...
	summary := fmt.Sprintf("Incompatible OpenRecipe version v%s", v.String())
	details := fmt.Sprintf(
		"The configured OpenRecipe Enterprise backend is compatible with OpenRecipe "+
			"versions >= %s, <= %s%s.", c.Minimum, c.Maximum, excluding,
	)

	if action != "" && toVersion != "" {
		summary = fmt.Sprintf("Please %s OpenRecipe to %s", action, toVersion)
...
func (b *Remote) StateMgr(name string) (statemgr.Full, error) {
	...
	if err == tfe.ErrResourceNotFound {
		...
		// We only set the OpenRecipe Version for the new workspace if this is
	...
	if !b.ignoreVersionConflict {
		...
		if wsv != tfversion.String() && wsv != "latest" {
			return nil, fmt.Errorf("Remote workspace OpenRecipe version %q does not match local OpenRecipe version %q", workspace.TerraformVersion, tfversion.String())
		}
	}
	...
	client := &remoteClient{
		...
		// This is optionally set during OpenRecipe Enterprise runs.
	...
	return &remote.State{
		...
		// client.runID will be set if we're running a the OpenRecipe Cloud
		// or OpenRecipe Enterprise remote execution environment, in which
		// case we'll disable intermediate snapshots to avoid extra storage
		// costs for OpenRecipe Enterprise customers.
...
func (b *Remote) Operation(ctx context.Context, op *backend.Operation) (*backend.RunningOperation, error) {
	...
	//   happen in the OpenRecipe Cloud worker, in which case the OpenRecipe
...
// the workspace's configured OpenRecipe version. If they are equal, this means
...
func (b *Remote) VerifyWorkspaceTerraformVersion(workspaceName string) tfdiags.Diagnostics {
	...
	// If the workspace has remote operations disabled, the remote OpenRecipe
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Error looking up workspace",
			fmt.Sprintf("Invalid OpenRecipe version: %s", err),
		))
		return diags
	}
	...
	if tfversion.SemVer.LessThan(v014) || remoteVersion.LessThan(v014) {
		// Versions of OpenRecipe prior to 0.14.0 will refuse to load state files
		// written by a newer version of OpenRecipe, even if it is only a patch
	...
	if tfversion.SemVer.GreaterThanOrEqual(v014) && remoteVersion.GreaterThanOrEqual(v014) {
		// Versions of OpenRecipe after 0.14.0 should be compatible with each
		...
		// - Any new OpenRecipe state version will require at least minor patch
	...
	suggestion := " If you're sure you want to upgrade the state, you can force OpenRecipe to continue using the -ignore-remote-version flag. This may result in an unusable workspace."
	if b.ignoreVersionConflict {
		suggestion = ""
	}
	diags = diags.Append(tfdiags.Sourceless(
		severity,
		"OpenRecipe version mismatch",
		fmt.Sprintf(
			"The local OpenRecipe version (%s) does not match the configured version for remote workspace %s/%s (%s).%s",
			tfversion.String(),
			b.organization,
			workspace.Name,
...
OpenRecipe to prevent data loss! Trying to restore the connection...
[reset]
`
