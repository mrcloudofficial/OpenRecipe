...
func TestRemote_versionConstraints(t *testing.T) {
	cases := map[string]struct {
		...
		"version too old": {
			...
			version: "0.0.1",
			result:  "upgrade OpenRecipe to >= 0.1.0",
		},
		"version too new": {
			config: cty.ObjectVal(map[string]cty.Value{
				"hostname":     cty.NullVal(cty.String),
			...
			version: "10.0.1",
			result:  "downgrade OpenRecipe to <= 10.0.0",
		},
	}
...
func TestRemote_checkConstraints(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"version too old": {
			...
			version: "0.10.1",
			result:  "upgrade OpenRecipe to >= 0.11.0",
		},
		"version too new": {
			constraints: &disco.Constraints{
				Minimum: "0.11.0",
			...
			version: "0.12.0",
			result:  "downgrade OpenRecipe to <= 0.11.11",
		},
		"version excluded - ordered": {
			...
			version: "0.11.7",
			result:  "upgrade OpenRecipe to > 0.11.8",
		},
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("local %s, remote %s", tc.local, tc.remote), func(t *testing.T) {
			...
			if tc.wantErr {
				...
				if got := diags.Err().Error(); !strings.Contains(got, "OpenRecipe version mismatch") {
...
func TestRemote_VerifyWorkspaceTerraformVersion_workspaceErrors(t *testing.T) {
	...
	if got := diags.Err().Error(); !strings.Contains(got, "Error looking up workspace: Invalid OpenRecipe version") {
		t.Fatalf("unexpected error: %s", got)
	}
}
...
func TestRemote_VerifyWorkspaceTerraformVersion_ignoreFlagSet(t *testing.T) {
	...
	if got, want := diags[0].Description().Summary, "OpenRecipe version mismatch"; got != want {
		t.Errorf("wrong summary: got %s, want %s", got, want)
	}
	wantDetail := "The local OpenRecipe version (0.14.0) does not match the configured version for remote workspace hashicorp/prod (0.13.5)."
