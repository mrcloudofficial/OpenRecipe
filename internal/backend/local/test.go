...
type TestSuiteRunner struct {
	...
	// StoppedCtx and CancelledCtx allow in progress OpenRecipe operations to
		...
		if run.Config.ConfigUnderTest != nil {
			...
			if key == MainStateIdentifier {
				...
				run.Diagnostics = run.Diagnostics.Append(&hcl.Diagnostic{
					...
					Detail:   fmt.Sprintf("The source for the selected module evaluated to %s which should not be possible. This is a bug in OpenRecipe - please report it!", key),
		...
		if runner.Suite.Verbose {
			...
			if diags.HasErrors() {
				...
				diags = diags.Append(tfdiags.Sourceless(
					...
					fmt.Sprintf("OpenRecipe failed to print the verbose output for %s, other diagnostics will contain more details as to why.", path.Join(file.Name, run.Name))))
			} else {
...
func (runner *TestFileRunner) cleanup(file *moduletest.File) {
	...
	for key, state := range runner.RelevantStates {
		...
		if state.Run == nil {
			...
			var diags tfdiags.Diagnostics
			diags = diags.Append(tfdiags.Sourceless(tfdiags.Error, "Inconsistent state", fmt.Sprintf("Found inconsistent state while cleaning up %s. This is a bug in OpenRecipe - please report it", file.Name)))
			file.Status = moduletest.Error
...
// should be called before trying to use these variables within a OpenRecipe
...
func (runner *TestFileRunner) GetVariables(config *configs.Config, run *moduletest.Run, references []*addrs.Reference) (terraform.InputValues, tfdiags.Diagnostics) {
	...
	for name, variable := range config.Module.Variables {
		...
		// processed by the OpenRecipe graph and provided a default value later
		// if they have one.
