...
func (b *Local) opApply(
	...
	if op.PlanFile == nil {
		...
		diags = diags.Append(moreDiags)
		if moreDiags.HasErrors() {
			// If OpenRecipe Core generated a partial plan despite the errors
			// then we'll make a best effort to render it. OpenRecipe Core
		...
		if mustConfirm {
			...
			case plans.DestroyMode:
				...
				desc = "OpenRecipe will destroy all your managed infrastructure, as shown above.\n" +
					"There is no undo. Only 'yes' will be accepted to confirm."
			case plans.RefreshOnlyMode:
				if op.Workspace != "default" {
					query = "Would you like to update the OpenRecipe state for \"" + op.Workspace + "\" to reflect these detected changes?"
				} else {
					query = "Would you like to update the OpenRecipe state to reflect these detected changes?"
				}
				desc = "OpenRecipe will write these changes to the state without modifying any real infrastructure.\n" +
					"There is no undo. Only 'yes' will be accepted to confirm."
			default:
				if op.Workspace != "default" {
				...
				desc = "OpenRecipe will perform the actions described above.\n" +
		...
		if plan.Errored {
			diags = diags.Append(tfdiags.Sourceless(
				...
				"OpenRecipe encountered an error when generating this plan, so it cannot be applied.",
			))
			op.ReportResult(runningOp, diags)
			return
		}
...
const stateWriteBackedUpError = `The error shown above has prevented OpenRecipe from writing the updated state to the configured backend. To allow for recovery, the state has been written to the file "errored.tfstate" in the current working directory.
...
const stateWriteConsoleFallbackError = `The errors shown above prevented OpenRecipe from writing the updated state to
...
A catastrophic error has prevented OpenRecipe from persisting the state file or creating a backup. Unfortunately this means that the record of any resources created during this apply has been lost, and such resources may exist outside of OpenRecipe's management.
...
This is a serious bug in OpenRecipe and should be reported.
`
