...
type IntermediateStatePersistInfo struct {
	...
	// persisted, or the time of the first report for OpenRecipe Core if there
	// hasn't yet been a persisted snapshot.
	LastPersist time.Time

	// ForcePersist is true when OpenRecipe CLI has receieved an interrupt
...
func (h *StateHook) Stopping() {
	...
	// If OpenRecipe has been asked to stop then that might mean that a hard
	// kill signal will follow shortly in case OpenRecipe doesn't stop
	...
	// do if they _do_ subsequently hard-kill OpenRecipe during an apply.
	...
	if mgrPersist, ok := h.StateMgr.(statemgr.Persister); ok && h.Schemas != nil {
		...
		// OpenRecipe Core won't start any new operations after it's been
		...
		if h.shouldPersist() {
			err := mgrPersist.PersistState(h.Schemas)
			if err != nil {
				// This hook can't affect OpenRecipe Core's ongoing behavior,
...
// whether to create intermediate state snapshots when OpenRecipe Core emits
...
type IntermediateStateConditionalPersister interface {
	// ShouldPersistIntermediateState will be called each time OpenRecipe Core
