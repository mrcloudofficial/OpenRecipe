...
func TestLocal_planTainted(t *testing.T) {
	...
	expectedOutput := `OpenRecipe used the selected providers to generate the following execution
...
func TestLocal_planDeposedOnly(t *testing.T) {
	...
	// is not common: it should arise only if OpenRecipe fails during
	...
	// as "current".
	//
	// However, that situation was more common in some earlier OpenRecipe
...
func TestLocal_planDestroy_withDataSources(t *testing.T) {
	...
	expectedOutput := `OpenRecipe will perform the following actions:
