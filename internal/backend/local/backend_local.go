...
func (b *Local) localRun(op *backend.Operation) (*backend.LocalRun, *configload.Snapshot, statemgr.Full, tfdiags.Diagnostics) {
	...
	if op.PlanFile.IsCloud() {
		diags = diags.Append(fmt.Errorf("error: using a saved cloud plan when executing OpenRecipe locally is not supported"))
		return nil, nil, nil, diags
	}
...
func (b *Local) localRunDirect(op *backend.Operation, run *backend.LocalRun, coreOpts *terraform.ContextOpts, s statemgr.Full) (*backend.LocalRun, *configload.Snapshot, tfdiags.Diagnostics) {
	...
	if errs := config.VerifyDependencySelections(op.DependencyLocks); len(errs) > 0 {
		...
		case op.DependencyLocks == nil:
			...
			suggestion = "This run has no dependency lock information provided at all, which is a bug in OpenRecipe; please report it!"
...
// not something we expect folks to be relying on for routine use. OpenRecipe
...
// to produce a complete OpenRecipe context for some ancillary functionality
