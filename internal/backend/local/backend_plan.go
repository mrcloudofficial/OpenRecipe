...
func (b *Local) opPlan(
	...
	if op.PlanMode != plans.DestroyMode && !op.HasConfig() {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"Plan requires configuration to be present. Planning without a configuration would "+
				...
				"create a OpenRecipe configuration file (.tf file) and try again.",
		))
		op.ReportResult(runningOp, diags)
		return
	}
	...
	if path := op.PlanOutPath; path != "" {
		if op.PlanOutBackend == nil {
			...
			diags = diags.Append(fmt.Errorf(
				"PlanOutPath set without also setting PlanOutBackend (this is a bug in OpenRecipe)"),
			)
			op.ReportResult(runningOp, diags)
			return
		}
		...
		// to make it consumable by the current OpenRecipe version, and
	...
	// above even if OpenRecipe Core encountered an error partway through
	// creating it.
	op.ReportResult(runningOp, diags)
