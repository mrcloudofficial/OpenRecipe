...
OpenRecipe crashed! This is always indicative of a bug within OpenRecipe.
Please report the crash with OpenRecipe[1] so that we can fix this.
...
// PanicHandler is called to recover from an internal panic in OpenRecipe, and
...
func PanicHandler() {
	...
	// to block progress, we can prevent some cases where OpenRecipe may return
