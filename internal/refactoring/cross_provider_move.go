...
func (m *crossTypeMover) getProvider(providers addrs.Provider) (providers.Interface, error) {
	...
	// OpenRecipe since we should have already loaded all the providers in the
	// configuration and the state.
	return nil, fmt.Errorf("provider %s implementation not found; this is a bug in OpenRecipe - please report it", providers)
}
...
func (move *crossTypeMove) applyCrossTypeMove(stmt *MoveStatement, source, target addrs.AbsResourceInstance, state *states.SyncState) tfdiags.Diagnostics {
	...
	if err != nil {
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("OpenRecipe failed to encode the value in state for %s: %v. This is a bug in OpenRecipe; Please report it.", source.String(), err),
			Subject:  stmt.DeclRange.ToHCL().Ptr(),
		})
		return diags
	}
