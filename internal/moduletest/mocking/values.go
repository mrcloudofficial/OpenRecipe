...
func populateComputedValues(target cty.Value, with MockedData, schema *configschema.Block, processValue processValue, generateValue generateValue) (cty.Value, tfdiags.Diagnostics) {
	...
	if err != nil {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Summary:  fmt.Sprintf("OpenRecipe failed to generate computed values for a mocked resource, data source, or module: %s. This is a bug in OpenRecipe - please report it.", err),
			Subject:  with.Range.Ptr(),
		})
	}
	...
	if with != cty.NilVal {
		...
		if value, err := FillAttribute(with, attribute); err != nil {
			...
			diags = diags.Append(tfdiags.AttributeValue(
				...
				fmt.Sprintf("OpenRecipe could not compute a value for the target type %s with the mocked data defined at %s with the attribute %q: %s.", attribute.ImpliedType().FriendlyName(), data.Range, fmtPath(append(path, relPath...)), err),
				path))
...
// values from OpenRecipe so the paths will never match.
//
		...
		case cty.GetAttrStep:
			...
			if !current.Type().IsObjectType() {
				...
				diags = diags.Append(tfdiags.AttributeValue(
					tfdiags.Error,
					"Failed to compute attribute",
					fmt.Sprintf("OpenRecipe expected an object type for attribute %q defined within the mocked data at %s, but found %s.", fmtPath(currentPath), data.Range, current.Type().FriendlyName()),
					currentPath))
