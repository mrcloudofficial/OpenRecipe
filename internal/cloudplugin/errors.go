...
var (
	// ErrCloudPluginNotSupported is the error returned when the upstream OpenRecipe Cloud does not
	// have a manifest.
	ErrCloudPluginNotSupported = errors.New("cloud plugin is not supported by the remote version of OpenRecipe Enterprise")
...
func (e ErrQueryFailed) Error() string {
	return fmt.Sprintf("failed to fetch cloud plugin from OpenRecipe Cloud: %s", e.inner)
}
