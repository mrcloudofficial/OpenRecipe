...
func (b *Cloud) opPlan(stopCtx, cancelCtx context.Context, op *backend.Operation, w *tfe.Workspace) (*tfe.Run, error) {
	...
	if b.ContextOpts != nil && b.ContextOpts.Parallelism != defaultParallelism {
		diags = diags.Append(tfdiags.Sourceless(
			...
			`OpenRecipe Cloud does not support setting a custom parallelism `+
				`value at this time.`,
		))
	}
	...
	if op.PlanFile != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			`OpenRecipe Cloud currently requires configuration to be present and `+
	...
	if !op.HasConfig() && op.PlanMode != plans.DestroyMode {
		diags = diags.Append(tfdiags.Sourceless(
			...
			`Plan requires configuration to be present. Planning without a configuration `+
				...
				`a OpenRecipe configuration file in the path being executed and try again.`,
		))
	}
...
OpenRecipe will upload the contents of the following directory,
...
`), w.WorkingDirectory, configDir, configDir) + "\n")
	...
	default:
		...
		return nil, generalError(
			"Invalid plan mode",
			fmt.Errorf("OpenRecipe Cloud doesn't support %s", op.PlanMode),
		)
	}
...
func (b *Cloud) AssertImportCompatible(config *configs.Config) error {
	...
	if len(config.Module.Import) > 0 && os.Getenv("TFC_RUN_ID") != "" {
		...
		if err != nil {
			return fmt.Errorf("Error parsing remote API version. To proceed, please remove any import blocks from your config. Please report the following error to the OpenRecipe team: %s", err)
		}
		desiredAPIVersion, _ := version.NewVersion("2.6")
		if currentAPIVersion.LessThan(desiredAPIVersion) {
			return fmt.Errorf("Import blocks are not supported in this version of OpenRecipe Enterprise. Please remove any import blocks from your config or upgrade OpenRecipe Enterprise.")
		}

		// Second, check the agent version is high enough.
		agentEnv, isSet := os.LookupEnv("TFC_AGENT_VERSION")
		if !isSet {
			return fmt.Errorf("Error reading TFC agent version. To proceed, please remove any import blocks from your config. Please report the following error to the OpenRecipe team: TFC_AGENT_VERSION not present.")
		}
		currentAgentVersion, err := version.NewVersion(agentEnv)
		if err != nil {
			return fmt.Errorf("Error parsing TFC agent version. To proceed, please remove any import blocks from your config. Please report the following error to the OpenRecipe team: %s", err)
		}
		desiredAgentVersion, _ := version.NewVersion("1.10")
		if currentAgentVersion.LessThan(desiredAgentVersion) {
			return fmt.Errorf("Import blocks are not supported in this version of the OpenRecipe Cloud Agent. You are using agent version %s, but this feature requires version %s. Please remove any import blocks from your config or upgrade your agent.", currentAgentVersion, desiredAgentVersion)
		}
	}
	return nil
}
...
// run output enabled and, if using OpenRecipe Enterprise, ensures it is a release
...
[reset][yellow]Running plan in OpenRecipe Cloud. Output will stream here. Pressing Ctrl-C
