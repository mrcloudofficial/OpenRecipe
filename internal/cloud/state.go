...
type State struct {
	...
	// The server can optionally return an X-OpenRecipe-Snapshot-Interval header
	...
	// If the header X-OpenRecipe-Snapshot-Interval is present then
...
// PersistState uploads a snapshot of the latest state as a StateVersion to OpenRecipe Cloud
...
func (s *State) uploadState(lineage string, serial uint64, isForcePush bool, state, jsonState, jsonStateOutputs []byte) error {
	...
	if errors.Is(err, tfe.ErrStateVersionUploadNotSupported) {
		// Create the new state with content included in the request (OpenRecipe Enterprise v202306-1 and below)
...
func (s *State) Lock(info *statemgr.LockInfo) (string, error) {
	...
	_, err := s.tfeClient.Workspaces.Lock(ctx, s.workspace.ID, tfe.WorkspaceLockOptions{
		Reason: tfe.String("Locked by OpenRecipe"),
	})
	if err != nil {
...
// GetRootOutputValues fetches output values from OpenRecipe Cloud
