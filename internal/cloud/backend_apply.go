...
func (b *Cloud) opApply(stopCtx, cancelCtx context.Context, op *backend.Operation, w *tfe.Workspace) (*tfe.Run, error) {
	...
	if op.PlanFile.IsLocal() {
		diags = diags.Append(tfdiags.Sourceless(
			...
			`OpenRecipe Cloud can apply a saved cloud plan, or create a new plan when `+
	...
	if cp, ok := op.PlanFile.Cloud(); ok {
		...
		if cp.Hostname != b.Hostname {
			diags = diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf("The given saved plan refers to a run on %s, but the currently configured OpenRecipe Cloud or OpenRecipe Enterprise instance is %s.", cp.Hostname, b.Hostname),
			))
			return r, diags.Err()
		}
		...
		if mustConfirm && b.input {
			...
			if op.PlanMode == plans.DestroyMode {
				...
				opts.Description = "OpenRecipe will destroy all your managed infrastructure, as shown above.\n" +
					"There is no undo. Only 'yes' will be accepted to confirm."
			} else {
				opts.Query = "\nDo you want to perform these actions in workspace \"" + op.Workspace + "\"?"
				opts.Description = "OpenRecipe will perform the actions described above.\n" +
...
func (b *Cloud) renderApplyLogs(ctx context.Context, run *tfe.Run) error {
	...
	if b.CLI != nil {
		...
		for next := true; next; {
			...
			// Apply logs show the same OpenRecipe info logs as shown in the plan logs
...
func unusableSavedPlanError(status tfe.RunStatus, url string) error {
	...
	case tfe.RunCanceled:
		...
		reason = "The given plan file can no longer be applied because the run was canceled via the OpenRecipe Cloud UI or API."
	case tfe.RunDiscarded:
		summary = "Saved plan is discarded"
		reason = "The given plan file can no longer be applied; either another run was applied first, or a user discarded it via the OpenRecipe Cloud UI or API."
	case tfe.RunErrored:
	...
	default:
		...
		reason = "OpenRecipe Cloud cannot apply the given plan file. This may mean the plan and checks have not yet completed, or may indicate another problem."
	}
...
[reset][yellow]Running apply in OpenRecipe Cloud. Output will stream here. Pressing Ctrl-C
