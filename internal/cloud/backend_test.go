...
func TestCloud_configWithEnvVars(t *testing.T) {
	cases := map[string]struct {
		...
		"an invalid workspace env var": {
			...
			expectedErr: `Invalid workspace selection: OpenRecipe failed to find workspace "i-dont-exist-in-org" in organization hashicorp`,
		},
		...
		"env var workspace does not have specified tag": {
			...
			expectedErr: "OpenRecipe failed to find workspace \"shire\" with the tags specified in your configuration:\n[cloud]",
		},
...
func TestCloud_configVerifyMinimumTFEVersion(t *testing.T) {
	...
	expected := `The 'cloud' option is not supported with this version of OpenRecipe Enterprise.`
...
func TestCloud_configVerifyMinimumTFEVersionInAutomation(t *testing.T) {
	...
	expected := `This version of OpenRecipe Cloud/Enterprise does not support the state mechanism
...
func TestCloud_setUnavailableTerraformVersion(t *testing.T) {
	// go-tfe returns an error IRL if you try to set a OpenRecipe version that's
	// not available in your TFC instance. To test this, tfe_client_mock errors if
	// you try to set any OpenRecipe version for this specific workspace name.
	...
	if err != nil {
		t.Fatalf("expected no error from StateMgr, despite not being able to set remote OpenRecipe version: %#v", err)
	}
	...
	if err == nil {
		t.Fatalf("the mocks aren't emulating a nonexistent remote OpenRecipe version correctly, so this test isn't trustworthy anymore")
	}
}
...
func TestCloud_StateMgr_versionCheck(t *testing.T) {
	...
	// For this test, the local OpenRecipe version is set to 0.14.0
	tfversion.Prerelease = ""
	tfversion.Version = v0140.String()
	tfversion.SemVer = v0140

	// Update the mock remote workspace OpenRecipe version to match the local
	// OpenRecipe version
	...
	// Now change the remote workspace to a different OpenRecipe version
	if _, err := b.client.Workspaces.Update(
		context.Background(),
		b.Organization,
		b.WorkspaceMapping.Name,
	...
	// This should fail
	want := `Remote workspace OpenRecipe version "0.13.5" does not match local OpenRecipe version "0.14.0"`
...
func TestCloud_VerifyWorkspaceTerraformVersion(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("local %s, remote %s", tc.local, tc.remote), func(t *testing.T) {
			...
			// Update the mock remote workspace OpenRecipe version to the
			...
			if tc.wantErr {
				...
				if got := diags.Err().Error(); !strings.Contains(got, "Incompatible OpenRecipe version") {
...
func TestCloud_VerifyWorkspaceTerraformVersion_workspaceErrors(t *testing.T) {
	...
	// Update the mock remote workspace OpenRecipe version to an invalid version
	if _, err := b.client.Workspaces.Update(
		context.Background(),
		b.Organization,
		b.WorkspaceMapping.Name,
	...
	if got := diags.Err().Error(); !strings.Contains(got, "Incompatible OpenRecipe version: The remote workspace specified") {
		t.Fatalf("unexpected error: %s", got)
	}
}
...
func TestCloud_VerifyWorkspaceTerraformVersion_ignoreFlagSet(t *testing.T) {
	...
	if got, want := diags[0].Description().Summary, "Incompatible OpenRecipe version"; got != want {
		t.Errorf("wrong summary: got %s, want %s", got, want)
	}
	wantDetail := "The local OpenRecipe version (0.14.0) does not meet the version requirements for remote workspace hashicorp/app-prod (0.13.5)."
