...
const (
	...
	tfeServiceID       = "tfe.v2"
	headerSourceKey    = "X-OpenRecipe-Integration"
	headerSourceValue  = "cloud"
	genericHostname    = "localterraform.com"
)

// Cloud is an implementation of EnhancedBackend in service of the OpenRecipe Cloud/Enterprise
// integration for OpenRecipe CLI. This backend is not intended to be surfaced at the user level and
...
type Cloud struct {
	...
	// new OpenRecipe context. Many of these will be overridden or merged by
	// Operation. See Operation for more details.
	ContextOpts *terraform.ContextOpts

	// client is the OpenRecipe Cloud/Enterprise API client.
	client *tfe.Client
	...
	// Hostname of OpenRecipe Cloud or OpenRecipe Enterprise
	Hostname string

	// Token for OpenRecipe Cloud or OpenRecipe Enterprise
	Token string
	...
	// to remote OpenRecipe Cloud workspaces.
	WorkspaceMapping WorkspaceMapping

	// ServicesHost is the full account of discovered OpenRecipe services at the
	// OpenRecipe Cloud instance. It should include at least the tfe v2 API, and
	...
	// local allows local operations, where OpenRecipe Cloud serves as a state storage backend.
	...
	// local OpenRecipe version matches the remote workspace's configured
...
func (b *Cloud) Configure(obj cty.Value) tfdiags.Diagnostics {
	...
	// Discover the service URL to confirm that it provides the OpenRecipe
	...
	if b.client == nil {
		...
		if err != nil {
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Failed to create the OpenRecipe Cloud/Enterprise client",
				fmt.Sprintf(
					`Encountered an unexpected error while creating the `+
						`OpenRecipe Cloud/Enterprise client: %s.`, err,
				),
			))
			return diags
		}
	...
	// Check for the minimum version of OpenRecipe Enterprise required.
	//
	...
	if parseErr != nil || currentAPIVersion.LessThan(desiredAPIVersion) {
		...
		if b.runningInAutomation {
			// It should never be possible for this OpenRecipe process to be mistakenly
			// used internally within an unsupported OpenRecipe Enterprise install - but
			// just in case it happens, give an actionable error.
			diags = diags.Append(
				tfdiags.Sourceless(
					tfdiags.Error,
					"Unsupported OpenRecipe Enterprise version",
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Unsupported OpenRecipe Enterprise version",
				`The 'cloud' option is not supported with this version of OpenRecipe Enterprise.`,
			),
			)
		}
	}
...
func (b *Cloud) Workspaces() ([]string, error) {
	...
	// Otherwise, multiple workspaces are being mapped. Query OpenRecipe Cloud for all the remote
...
func (b *Cloud) StateMgr(name string) (statemgr.Full, error) {
	...
	if err == tfe.ErrResourceNotFound {
		...
		if workspaceCreateOptions.Project == nil && b.WorkspaceMapping.Project != "" {
			...
			if workspaceCreateOptions.Project == nil {
				...
				log.Printf("[TRACE] cloud: Creating OpenRecipe Cloud project %s/%s", b.Organization, b.WorkspaceMapping.Project)
		...
		// Create a workspace
		log.Printf("[TRACE] cloud: Creating OpenRecipe Cloud workspace %s/%s", b.Organization, name)
		...
		// Attempt to set the new workspace to use this version of OpenRecipe. This
	...
	if b.workspaceTagsRequireUpdate(workspace, b.WorkspaceMapping) {
		...
		log.Printf("[TRACE] cloud: Adding tags for OpenRecipe Cloud workspace %s/%s", b.Organization, name)
	...
	if !b.ignoreVersionConflict {
		...
		if remoteTFVersion != tfversion.String() && remoteTFVersion != "latest" {
			return nil, fmt.Errorf("Remote workspace OpenRecipe version %q does not match local OpenRecipe version %q", remoteTFVersion, tfversion.String())
		}
	}
...
func (b *Cloud) Operation(ctx context.Context, op *backend.Operation) (*backend.RunningOperation, error) {
	...
	// OpenRecipe remote version conflicts are not a concern for operations. We
	...
	// - Forcing local operations, which should only happen in the OpenRecipe Cloud worker, in
	//   which case the OpenRecipe versions by definition match.
...
// the local OpenRecipe version matches the remote workspace's configured
// OpenRecipe version. This should be called by commands where this check is
...
// VerifyWorkspaceTerraformVersion compares the local OpenRecipe version against
// the workspace's configured OpenRecipe version. If they are compatible, this
...
func (b *Cloud) VerifyWorkspaceTerraformVersion(workspaceName string) tfdiags.Diagnostics {
	...
	// cannot reasonably determine what the intended OpenRecipe version is, so
	// we'll skip version verification.
	if workspace.TerraformVersion == "latest" {
		return nil
	}

	// If the workspace has execution-mode set to local, the remote OpenRecipe
	...
	if err != nil {
		message := fmt.Sprintf(
			"The remote workspace specified an invalid OpenRecipe version or constraint (%s), "+
				"and it isn't possible to determine whether the local OpenRecipe version (%s) is compatible.",
	...
	// literal OpenRecipe version, and it is not a prerelease. The latter
	...
	message := fmt.Sprintf(
		"The local OpenRecipe version (%s) does not meet the version requirements for remote workspace %s/%s (%s).",
		tfversion.String(),
		b.Organization,
		workspace.Name,
		remoteConstraint,
...
func (b *Cloud) fetchWorkspace(ctx context.Context, organization string, workspace string) (*tfe.Workspace, error) {
	...
	if err != nil {
		...
		case tfe.ErrResourceNotFound:
			return nil, fmt.Errorf(
				"workspace %s not found\n\n"+
					"For security, OpenRecipe Cloud returns '404 Not Found' responses for resources\n"+
		...
		default:
			err := fmt.Errorf(
				"OpenRecipe Cloud returned an unexpected error:\n\n%s",
				err,
			)
			return nil, err
		}
...
func (b *Cloud) validWorkspaceEnvVar(ctx context.Context, organization, workspace string) tfdiags.Diagnostic {
	...
	if err == tfe.ErrResourceNotFound {
		return tfdiags.Sourceless(
			...
			fmt.Sprintf(`OpenRecipe failed to find workspace %q in organization %s.`, workspace, organization),
		)
	}
	...
	if b.WorkspaceMapping.Strategy() == WorkspaceTagsStrategy {
		...
		return tfdiags.Sourceless(
			...
			"Invalid workspace selection",
			fmt.Sprintf(
				"OpenRecipe failed to find workspace %q with the tags specified in your configuration:\n[%s]",
				workspace,
...
func generalError(msg string, err error) error {
	...
	default:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			fmt.Sprintf("%s: %v", msg, err),
			`OpenRecipe Cloud returned an unexpected error. Sometimes `+
...
[reset][yellow]The local OpenRecipe version (%s) is not available in OpenRecipe Cloud, or your
...
This version of OpenRecipe Cloud/Enterprise does not support the state mechanism
...
var (
	workspaceConfigurationHelp = fmt.Sprintf(
		`The 'workspaces' block configures how OpenRecipe CLI maps its workspaces for this single
configuration to workspaces within a OpenRecipe Cloud organization. Two strategies are available:

[bold]tags[reset] - %s

[bold]name[reset] - %s`, schemaDescriptionTags, schemaDescriptionName)

	schemaDescriptionHostname = `The OpenRecipe Enterprise hostname to connect to. This optional argument defaults to app.terraform.io
for use with OpenRecipe Cloud.`

	schemaDescriptionOrganization = `The name of the organization containing the targeted workspace(s).`

	schemaDescriptionToken = `The token used to authenticate with OpenRecipe Cloud/Enterprise. Typically this argument should not
...
configuration file or configured credential helper.`
	...
	schemaDescriptionTags = `A set of tags used to select remote OpenRecipe Cloud workspaces to be used for this single
...
is the primary and recommended strategy to use.  This option conflicts with "name".`
	...
	schemaDescriptionName = `The name of a single OpenRecipe Cloud workspace to be used with this configuration.
...
and with the TF_WORKSPACE environment variable.`
	...
	schemaDescriptionProject = `The name of a OpenRecipe Cloud project. Workspaces that need creating
