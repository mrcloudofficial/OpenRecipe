...
// TestCommand represents the OpenRecipe a given run block will execute, plan
// or apply. Defaults to apply.
type TestCommand rune

// TestMode represents the plan mode that OpenRecipe will use for a given run
// block, normal or refresh-only. Defaults to normal.
type TestMode rune

const (
	// ApplyTestCommand causes the run block to execute a OpenRecipe apply
	// operation.
	ApplyTestCommand TestCommand = 0

	// PlanTestCommand causes the run block to execute a OpenRecipe plan
	// operation.
...
//
// Each run block represents a single OpenRecipe command to be executed and a set
// of validations to run after the command.
type TestRun struct {
	Name string

	// Command is the OpenRecipe command to execute.
	//
	// One of ['apply', 'plan'].
	Command TestCommand
...
type TestRunOptions struct {
	...
	// Refresh is analogous to the -refresh=false OpenRecipe plan option.
	Refresh bool

	// Replace is analogous to the -replace=ADDRESS OpenRecipe plan option.
	Replace []hcl.Traversal

	// Target is analogous to the -target=ADDRESS OpenRecipe plan option.
