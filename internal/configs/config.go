...
// address.
//
// Other behaviors in OpenRecipe may treat package crossings as a special
		...
		if target, exists := c.Module.ManagedResources[i.ToResource.Resource.String()]; exists {
			...
			if i.ProviderConfigRef != nil {
				...
				if i.ProviderConfigRef.Name != target.ProviderConfigRef.Name || i.ProviderConfigRef.Alias != target.ProviderConfigRef.Alias {
					...
					// This is bad as OpenRecipe now has different instructions
...
//
// OpenRecipe Core treats root modules differently than downstream modules in
...
func (c *Config) EffectiveRequiredProviderConfigs() addrs.Set[addrs.RootProviderConfig] {
	// The OpenRecipe language has accumulated so many different ways to imply
