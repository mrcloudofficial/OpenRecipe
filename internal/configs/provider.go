...
func decodeProviderBlock(block *hcl.Block) (*Provider, hcl.Diagnostics) {
	...
	if attr, exists := content.Attributes["version"]; exists {
		diags = append(diags, &hcl.Diagnostic{
			...
			Detail:   "OpenRecipe 0.13 and earlier allowed provider version constraints inside the provider configuration block, but that is now deprecated and will be removed in a future version of OpenRecipe. To silence this warning, move the provider version constraint into the required_providers block.",
	...
	for _, name := range []string{"count", "depends_on", "for_each", "source"} {
		if attr, exists := content.Attributes[name]; exists {
			diags = append(diags, &hcl.Diagnostic{
				...
				Detail:   fmt.Sprintf("The provider argument name %q is reserved for use by OpenRecipe in a future version.", name),
	...
	for _, block := range content.Blocks {
		...
		default:
			...
			diags = append(diags, &hcl.Diagnostic{
				...
				Detail:   fmt.Sprintf("The block type name %q is reserved for use by OpenRecipe in a future version.", block.Type),
