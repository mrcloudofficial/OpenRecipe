...
func (m *Module) appendFile(file *File) hcl.Diagnostics {
	...
	for _, c := range file.CloudConfigs {
		if m.CloudConfig != nil {
			diags = append(diags, &hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Duplicate OpenRecipe Cloud configurations",
				Detail:   fmt.Sprintf("A module may have only one 'cloud' block configuring OpenRecipe Cloud. OpenRecipe Cloud was previously configured at %s.", m.CloudConfig.DeclRange),
				Subject:  &c.DeclRange,
			})
			continue
		}
	...
	if m.Backend != nil && m.CloudConfig != nil {
		diags = append(diags, &hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "Both a backend and OpenRecipe Cloud configuration are present",
			Detail:   fmt.Sprintf("A module may declare either one 'cloud' block configuring OpenRecipe Cloud OR one 'backend' block configuring a state backend. OpenRecipe Cloud is configured at %s; a backend is configured at %s. Remove the backend block to configure OpenRecipe Cloud.", m.CloudConfig.DeclRange, m.Backend.DeclRange),
	...
	if len(file.CloudConfigs) != 0 {
		...
		default:
			...
			diags = append(diags, &hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Duplicate OpenRecipe Cloud configurations",
				Detail:   fmt.Sprintf("A module may have only one 'cloud' block configuring OpenRecipe Cloud. OpenRecipe Cloud was previously configured at %s.", file.CloudConfigs[0].DeclRange),
		...
		if !constraint.Required.Check(tfversion.SemVer) {
			...
			case len(path) == 0:
				diags = diags.Append(&hcl.Diagnostic{
					Severity: hcl.DiagError,
					Summary:  "Unsupported OpenRecipe Core version",
					Detail: fmt.Sprintf(
						"This configuration does not support OpenRecipe version %s. To proceed, either choose another supported OpenRecipe version or update this version constraint. Version constraints are normally set for good reason, so updating the constraint may lead to other errors or unexpected behavior.",
						tfversion.String(),
					),
					Subject: constraint.DeclRange.Ptr(),
				})
			default:
				diags = diags.Append(&hcl.Diagnostic{
					...
					Summary:  "Unsupported OpenRecipe Core version",
					Detail: fmt.Sprintf(
						"Module %s (from %s) does not support OpenRecipe version %s. To proceed, either choose another supported OpenRecipe version or update this version constraint. Version constraints are normally set for good reason, so updating the constraint may lead to other errors or unexpected behavior.",
