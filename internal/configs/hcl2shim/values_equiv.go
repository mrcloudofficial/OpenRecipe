...
// provided by OpenRecipe Core with the "planned new state" produced by the
...
// because they contain only the subset of data that OpenRecipe Core itself is
// able to determine.
...
// cty (and thus OpenRecipe Core) uses big.Float, and so we expect to lose
