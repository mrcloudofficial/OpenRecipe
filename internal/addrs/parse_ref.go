...
// traversal of the reciever in a way that resembles the OpenRecipe language
// syntax that could've produced it.
//
// It's not guaranteed to actually be a valid OpenRecipe language expression,
...
func ParseRefFromTestingScope(traversal hcl.Traversal) (*Reference, tfdiags.Diagnostics) {
	...
	case "plan", "state":
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("The symbol name %q is reserved for use in a future OpenRecipe version. If you are using a provider that already uses this as a resource type name, add the prefix \"resource.\" to force interpretation as a resource type name.", root),
...
func parseRef(traversal hcl.Traversal) (*Reference, tfdiags.Diagnostics) {
	...
	case "resource":
		...
		// if a later edition of the OpenRecipe language introduces a new
