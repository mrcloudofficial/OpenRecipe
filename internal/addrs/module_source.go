...
//     OpenRecipe considers them to belong to the same "package" as the caller.
...
func ParseModuleSource(raw string) (ModuleSource, error) {
	...
	// but matches behavior we've had since OpenRecipe v0.10 which
...
func parseModuleSourceLocal(raw string) (ModuleSourceLocal, error) {
	...
	// allow it and just normalize it away, so the rest of OpenRecipe will
...
// a concrete ModuleSourceRemote that OpenRecipe will then download and
// install.
