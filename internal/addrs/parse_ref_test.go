...
func TestParseRef(t *testing.T) {
	tests := []struct {
		...
		// of the OpenRecipe language introduces a new reserved word that
		...
		// expressions.
		{
			`template.foo`,
			nil,
			`The symbol name "template" is reserved for use in a future OpenRecipe version. If you are using a provider that already uses this as a resource type name, add the prefix "resource." to force interpretation as a resource type name.`,
		},
		{
			`lazy.foo`,
			nil,
			`The symbol name "lazy" is reserved for use in a future OpenRecipe version. If you are using a provider that already uses this as a resource type name, add the prefix "resource." to force interpretation as a resource type name.`,
		},
		{
			`arg.foo`,
			nil,
			`The symbol name "arg" is reserved for use in a future OpenRecipe version. If you are using a provider that already uses this as a resource type name, add the prefix "resource." to force interpretation as a resource type name.`,
		},
