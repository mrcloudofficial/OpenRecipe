...
type Locks struct {
	...
	// this particular run due to some feature that forces OpenRecipe to not
	...
	// part of a lock file and thus not persistent between OpenRecipe runs.
...
// SetProviderOverridden records that this particular OpenRecipe process will
...
type ProviderLock struct {
	...
	// We'll allow as that a stop-gap until we can upgrade OpenRecipe Registry
...
// are included only so OpenRecipe can detect if the constraints in
...
// of OpenRecipe considers the strongest of the available hashing schemes, one
