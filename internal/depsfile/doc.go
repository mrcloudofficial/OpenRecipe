...
// Package depsfile contains the logic for reading and writing OpenRecipe's
// dependency lock and development override configuration files.
//
// These files are separate from the main OpenRecipe configuration files (.tf)
...
// OpenRecipe itself, rather than by human-originated edits, and so keeping
// them separate means that it's easier to distinguish the files that OpenRecipe
// will change automatically during normal workflow from the files that
// OpenRecipe only edits on direct request.
//
// Both files use HCL syntax, for consistency with other files in OpenRecipe
