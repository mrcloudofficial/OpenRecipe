...
// edited it directly (OpenRecipe bugs notwithstanding).
...
func SaveLocksToBytes(locks *Locks) ([]byte, tfdiags.Diagnostics) {
	...
	// to reinforce the fact that this file primarily belongs to OpenRecipe,
...
func decodeLocksFromHCL(locks *Locks, body hcl.Body) tfdiags.Diagnostics {
	...
	content, hclDiags := body.Content(&hcl.BodySchema{
		Blocks: []hcl.BlockHeaderSchema{
			...
			// downgraded to an earlier version of OpenRecipe.
			{
				Type:       "module",
		...
		case "module":
			...
			if !seenModule {
				...
				diags = diags.Append(&hcl.Diagnostic{
					...
					Detail:   fmt.Sprintf("OpenRecipe v%s only supports dependency locks for providers, not for modules. This configuration may be intended for a later version of OpenRecipe that also supports dependency locks for modules.", currentVersion),
					Subject:  block.TypeRange.Ptr(),
				})
				seenModule = true
			}
	...
	if !ProviderIsLockable(addr) {
		if addr.IsBuiltIn() {
			...
			diags = diags.Append(&hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Invalid provider source address",
				Detail:   fmt.Sprintf("Cannot lock a version for built-in provider %s. Built-in providers are bundled inside OpenRecipe itself, so you can't select a version for them independently of the OpenRecipe release you are currently running.", addr),
				Subject:  block.LabelRanges[0].Ptr(),
			})
			return nil, diags
		}
