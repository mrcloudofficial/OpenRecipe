...
func readState(src []byte) (*File, error) {
	...
	if looksLikeVersion0(src) {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			unsupportedFormat,
			"The state is stored in a legacy binary format that is not supported since OpenRecipe v0.7. To continue, first upgrade the state using OpenRecipe 0.6.16 or earlier.",
		))
	...
	default:
		...
		case creatingVersion != "":
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				unsupportedFormat,
				fmt.Sprintf("The state file uses format version %d, which is not supported by OpenRecipe %s. This state file was created by OpenRecipe %s.", version, thisVersion, creatingVersion),
			))
		default:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				unsupportedFormat,
				fmt.Sprintf("The state file uses format version %d, which is not supported by OpenRecipe %s. This state file may have been created by a newer version of OpenRecipe.", version, thisVersion),
			))
		}
	}
...
// sniffJSONStateTerraformVersion attempts to sniff the OpenRecipe version
