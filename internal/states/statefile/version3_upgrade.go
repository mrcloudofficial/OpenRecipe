...
func upgradeStateV3ToV4(old *stateV3) (*stateV4, error) {
	...
	for _, msOld := range old.Modules {
		...
		// Since these old formats are only generated by versions of OpenRecipe
		...
		for i, name := range msOld.Path[1:] {
			if !hclsyntax.ValidIdentifier(name) {
				...
				return nil, fmt.Errorf("state contains invalid module path %#v: %q is not a valid identifier; rename it in OpenRecipe 0.11 before upgrading to OpenRecipe 0.12", msOld.Path, name)
			}
				...
				if strings.Contains(oldProviderAddr, "provider.") {
					...
					if diags.HasErrors() {
						if strings.Contains(oldProviderAddr, "${") {
							...
							return nil, fmt.Errorf("invalid provider config reference %q for %s: this alias seems to contain a template interpolation sequence, which was not supported but also not error-checked in OpenRecipe 0.11. To proceed, rename the associated provider alias to a valid identifier and apply the change with OpenRecipe 0.11 before upgrading to OpenRecipe 0.12", oldProviderAddr, instAddr)
						}
		...
		if len(msOld.Path) == 1 && msOld.Path[0] == "root" {
			...
			for name, oldOS := range msOld.Outputs {
				...
				// version of OpenRecipe didn't support those a collection type
...
func simplifyImpliedValueType(ty cty.Type) cty.Type {
	...
	case ty.IsTupleType():
		...
		// of OpenRecipe did not officially support mixed-type collections.
