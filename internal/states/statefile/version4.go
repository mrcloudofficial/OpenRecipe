...
func prepareStateV4(sV4 *stateV4) (*File, tfdiags.Diagnostics) {
	...
	if sV4.TerraformVersion != "" {
		...
		if err != nil {
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Invalid OpenRecipe version string",
				fmt.Sprintf("State file claims to have been written by OpenRecipe version %q, which is not a valid version string.", sV4.TerraformVersion),
			))
		}
	}
	...
	// recognize if the previous run was with a version of OpenRecipe that
...
func writeStateV4(file *File, w io.Writer) tfdiags.Diagnostics {
	...
	if err != nil {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("An error occured while serializing the state to save it. This is a bug in OpenRecipe and should be reported: %s.", err),
		))
		return diags
	}
	src = append(src, '\n')
...
func decodeCheckStatusV4(in string) checks.Status {
	...
	default:
		...
		// forward-compatible parsing, in case a later version of OpenRecipe
