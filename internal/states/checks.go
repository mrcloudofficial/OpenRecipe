...
// declared in configuration, updated after each OpenRecipe Core run that
...
type CheckResultAggregate struct {
	...
	// OpenRecipe Core from even determining the individual checkable objects
	...
	//
	// That's different than OpenRecipe Core explicitly reporting that there are
...
//
// This should be called only after a OpenRecipe Core run has completed and
