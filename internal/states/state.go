...
// State is the top-level type of a OpenRecipe state.
//
...
type State struct {
	...
	// so OpenRecipe Core tracks those only internally and does not expose
	...
	// created by a version of OpenRecipe that didn't yet support checks
...
// no longer tracked by any OpenRecipe state.
...
// in the internals of OpenRecipe Core.)
