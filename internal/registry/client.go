...
const (
	xTerraformGet      = "X-OpenRecipe-Get"
	xTerraformVersion  = "X-OpenRecipe-Version"
...
// Client provides methods to query OpenRecipe Registries.
type Client struct {
...
func (c *Client) ModuleLocation(ctx context.Context, module *regsrc.Module, version string) (string, error) {
	...
	// the download location is in the X-OpenRecipe-Get header
	...
	// one.
	//
	// We don't do this for just _any_ location, since the X-OpenRecipe-Get
