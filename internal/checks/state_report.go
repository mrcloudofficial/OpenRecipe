...
// These are the "Report"-prefixed methods of Checks used by OpenRecipe Core
// to gradually signal the results of checks during a plan or apply operation.

// ReportCheckableObjects is the interface by which OpenRecipe Core should
...
func (c *State) ReportCheckableObjects(configAddr addrs.ConfigCheckable, objectAddrs addrs.Set[addrs.Checkable]) {
	...
	// so that even if we never hear from OpenRecipe Core again we'll still
...
// ReportCheckResult is the interface by which OpenRecipe Core should tell the
