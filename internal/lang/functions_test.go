...
func TestFunctions(t *testing.T) {
	...
	tests := map[string][]struct {
		...
		"can": {
			...
			},
			{
				...
				// OpenRecipe understands it as a reference to an attribute
		...
		"jsonencode": {
			...
			// to preserve backwards compatibility with OpenRecipe 0.11
			{
		...
		"try": {
			{
				...
				// structures that OpenRecipe can't statically validate by
				// definition.
				`try({}.baz, "fallback")`,
	...
	// OpenRecipe core based on declarations in that module, so here
