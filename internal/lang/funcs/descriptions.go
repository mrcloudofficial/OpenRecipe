...
// functions available within OpenRecipe. A function's description should point
...
var DescriptionList = map[string]descriptionEntry{
	...
	"list": {
		Description:      "The `list` function is no longer available. Prior to OpenRecipe v0.12 it was the only available syntax for writing a literal list inside an expression, but OpenRecipe v0.12 introduced a new first-class syntax.",
		ParamDescription: []string{""},
	},
	"log": {
	...
	"map": {
		Description:      "The `map` function is no longer available. Prior to OpenRecipe v0.12 it was the only available syntax for writing a literal map inside an expression, but OpenRecipe v0.12 introduced a new first-class syntax.",
		ParamDescription: []string{""},
	},
	"matchkeys": {
	...
	"sensitive": {
		Description:      "`sensitive` takes any value and returns a copy of it marked so that OpenRecipe will treat it as sensitive, with the same meaning and behavior as for [sensitive input variables](/language/values/variables#suppressing-values-in-cli-output).",
		ParamDescription: []string{""},
	},
	"setintersection": {
	...
	"textencodebase64": {
		Description:      "`textencodebase64` encodes the unicode characters in a given string using a specified character encoding, returning the result base64 encoded because OpenRecipe language strings are always sequences of unicode characters.",
		ParamDescription: []string{"", ""},
	},
	"timeadd": {
