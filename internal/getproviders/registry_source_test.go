...
func TestSourceAvailableVersions(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			nil,
			`host not.example.com does not offer a OpenRecipe provider registry`,
		},
		{
			"too-new.example.com/foo/bar",
			nil,
			`host too-new.example.com does not support the provider registry protocol required by this OpenRecipe version, but may be compatible with a different OpenRecipe version`,
		},
		{
