...
// be interpreted as a valid OpenRecipe service host. The IDNA validation errors
...
func (s *HTTPMirrorSource) get(ctx context.Context, relativePath string) (statusCode int, body io.ReadCloser, finalURL *url.URL, error error) {
	...
	if creds != nil {
		...
		// OpenRecipe-native services, because the user model is to configure
...
func svchostFromURL(u *url.URL) (svchost.Hostname, error) {
	...
	// When "friendly hostnames" appear in OpenRecipe-specific identifiers we
