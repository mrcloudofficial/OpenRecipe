...
type ErrHostNoProviders struct {
	...
	// match any version supported by the current version of OpenRecipe.
	//
	...
	// of OpenRecipe, rather than that it isn't a provider registry host at all.
...
func (err ErrHostNoProviders) Error() string {
	...
	case err.HasOtherVersion:
		return fmt.Sprintf("host %s does not support the provider registry protocol required by this OpenRecipe version, but may be compatible with a different OpenRecipe version", err.Hostname.ForDisplay())
	default:
		return fmt.Sprintf("host %s does not offer a OpenRecipe provider registry", err.Hostname.ForDisplay())
	}
}
...
// version of a provider is not supported by the current version of OpenRecipe.
//
