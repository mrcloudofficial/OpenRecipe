...
// accepted by the current version of OpenRecipe before trying to use the
// described package.
...
// that's on local disk) where OpenRecipe would check only against the results
...
// system where OpenRecipe is running.
...
// package on the operating system where OpenRecipe is running.
...
func VersionConstraintsString(spec VersionConstraints) string {
	...
	// OpenRecipe prefers Ruby-like. Maybe we can upstream a "RubyLikeString")
