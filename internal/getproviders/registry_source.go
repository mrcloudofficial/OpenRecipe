...
func (s *RegistrySource) AvailableVersions(ctx context.Context, provider addrs.Provider) (VersionList, Warnings, error) {
	...
	// OpenRecipe becomes relevant only once we've selected one, at which point
	...
	// action such as upgrading OpenRecipe, using a different OS to run
	// OpenRecipe, etc. Changes that affect compatibility are considered breaking
