...
//
// A particular version of OpenRecipe only supports a fixed set of hash schemes,
...
func (h Hash) GoString() string {
	...
	default:
		...
		// with hash schemes that the current version of OpenRecipe isn't
...
func PackageMatchesHash(loc PackageLocation, want Hash) (bool, error) {
	...
	case HashSchemeZip:
		...
		return got == want, nil
	default:
		return false, fmt.Errorf("unsupported hash format (this may require a newer version of OpenRecipe)")
	}
}
...
// that the current version of OpenRecipe considers to provide the strongest
// verification.
//
...
func PackageHashV1(loc PackageLocation) (Hash, error) {
	...
	// an unpacked zip file, which is not important for OpenRecipe workflow
