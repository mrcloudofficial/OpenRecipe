...
type PackageAuthenticationHashes interface {
	...
	// accepted by other versions of OpenRecipe, e.g. if a particular hash
...
// given hashes are considered acceptable by the current version of OpenRecipe.
//
...
// hashes are considered by OpenRecipe to be the strongest verification, and
...
func (a packageHashAuthentication) AuthenticatePackage(localLocation PackageLocation) (*PackageAuthenticationResult, error) {
	if len(a.RequiredHashes) == 0 {
		...
		// version of OpenRecipe.
		return nil, fmt.Errorf("this version of OpenRecipe does not support any of the checksum formats given for this provider")
	}
...
func (a packageHashAuthentication) AcceptableHashes() []Hash {
	// In this case we include even hashes the current version of OpenRecipe
	// doesn't prefer, because this result is used for building a lock file
	// and so it's helpful to include older hash formats that other OpenRecipe
