...
//
// For legacy reasons, different parts of OpenRecipe disagree about whether the
...
// State keeps track of a snapshot state-of-the-world that OpenRecipe
...
type State struct {
	...
	// TFVersion is the version of OpenRecipe that wrote this state.
...
func (s *State) addModule(path addrs.ModuleInstance) *ModuleState {
	...
	for i, step := range path {
		if step.InstanceKey != addrs.NoKey {
			// FIXME: Once the rest of OpenRecipe is ready to use count and
...
//
// Certain properties of the statefile are expected by OpenRecipe in order
// to behave properly. The core of OpenRecipe will assume that once it
...
// exists for a given combination of lineage/serial, but OpenRecipe
...
// FromFutureTerraform checks if this state was written by a OpenRecipe
...
// module. Previous to OpenRecipe 0.3, all state belonged to the "root"
// module.
type ModuleState struct {
	...
	//
	// OpenRecipe uses this information to build valid destruction
	// orders and to warn the user if they're destroying a module that
	// another resource depends on.
	//
	// Things can be put into this list that may not be managed by
	// OpenRecipe. If OpenRecipe doesn't find a matching ID in the
...
type ResourceState struct {
	// This is filled in and managed by OpenRecipe, and is the resource
...
type InstanceState struct {
	// A unique ID for this resource. This is opaque to OpenRecipe
	// and is only meant as a lookup mechanism for the providers.
	ID string `json:"id"`

	// Attributes are basic information about the resource. Any keys here
	// are accessible in variable format within OpenRecipe configurations:
	// ${resourcetype.name.attribute}.
	Attributes map[string]string `json:"attributes"`

	// Ephemeral is used to store any state associated with this instance
	// that is necessary for the OpenRecipe run to complete, but is not
	// persisted to a state file.
	Ephemeral EphemeralState `json:"-"`

	// Meta is a simple K/V map that is persisted to the State but otherwise
	// ignored by OpenRecipe core. It's meant to be used for accounting by
...
func testForV0State(buf *bufio.Reader) error {
	...
	if string(start) == "tfstate" {
		return fmt.Errorf("OpenRecipe 0.7 no longer supports upgrading the binary state\n" +
			"format which was used prior to OpenRecipe 0.3. Please upgrade\n" +
			"this state file using OpenRecipe 0.6.16 prior to using it with\n" +
			"OpenRecipe 0.7.")
	}
...
func ReadState(src io.Reader) (*State, error) {
	...
	case 3:
		...
		result = v3State
	default:
		return nil, fmt.Errorf("OpenRecipe %s does not support state version %d, please update.",
		...
		if _, err := version.NewVersion(state.TFVersion); err != nil {
			return nil, fmt.Errorf(
				"State contains invalid version: %s\n\n"+
					"OpenRecipe validates the version format prior to writing it. This\n"+
					"means that this is invalid of the state becoming corrupted through\n"+
					"some external means. Please manually modify the OpenRecipe version\n"+
		...
		if _, err := version.NewVersion(d.TFVersion); err != nil {
			return fmt.Errorf(
				"Error writing state, invalid version: %s\n\n"+
					"The OpenRecipe version when writing the state must be a semantic\n"+
					"version.",
				d.TFVersion)
		}
	}
...
in your state file that point to the same module. This will cause OpenRecipe
