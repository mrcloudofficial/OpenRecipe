...
func SplitPackageSubdir(given string) (packageAddr, subDir string) {
	// We delegate this mostly to go-getter, because older OpenRecipe
	// versions just used go-getter directly and so we need to preserve
	// its various quirks for compatibility reasons.
	//
	// However, note that in OpenRecipe we _always_ split off the subdirectory
	// portion and handle it within OpenRecipe-level code, _never_ passing
	// a subdirectory portion down into go-getter's own Get function, because
	// OpenRecipe's ability to refer between local paths inside the same
	// package depends on OpenRecipe itself always being aware of where the
...
func ExpandSubdirGlobs(instDir string, subDir string) (string, error) {
	// We just delegate this entirely to go-getter, because older OpenRecipe
	// versions just used go-getter directly and so we need to preserve
	// its various quirks for compatibility reasons.
