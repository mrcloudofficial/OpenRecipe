...
// better meets OpenRecipe's needs: specifically, it rejects relative filesystem
...
// case replaced with a similar result as OpenRecipe's old heuristic would've
...
func (e *MaybeRelativePathErr) Error() string {
	return fmt.Sprintf("OpenRecipe cannot detect a supported external module source type for %s", e.Addr)
}
