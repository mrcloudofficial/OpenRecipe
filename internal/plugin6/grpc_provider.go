...
func (p *GRPCProvider) GetProviderSchema() providers.GetProviderSchemaResponse {
	...
	// FIXME: A global cache is inappropriate when OpenRecipe Core is being
	// used in a non-OpenRecipe-CLI mode where we shouldn't assume that all
...
func (p *GRPCProvider) MoveResourceState(r providers.MoveResourceStateRequest) (resp providers.MoveResourceStateResponse) {
	...
	if !ok {
		...
		// wrong.
		resp.Diagnostics = resp.Diagnostics.Append(fmt.Errorf("unknown resource type %q; this is a bug in OpenRecipe - please report it", r.TargetTypeName))
		return resp
	}
