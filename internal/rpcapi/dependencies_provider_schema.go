...
func unconfiguredBuiltinProviderInstance(addr addrs.Provider) (providers.Interface, error) {
	...
	if !ok {
		return nil, fmt.Errorf("this version of OpenRecipe does not support provider %s", addr)
	}
	return factory(), nil
}
...
func providerSchemaToProto(schemaResp providers.GetProviderSchemaResponse) *terraform1.ProviderSchema {
	...
	// different terminology than the user-facing terminology for OpenRecipe
	// Core and the OpenRecipe language, and so part of our job here is to
	// map between the two so that rpcapi uses OpenRecipe Core's words
	...
	// will probably follow different evolutionary paths. For example, OpenRecipe
