			...
			evts.Send(&terraform1.BuildProviderPluginCache_Event{
				Event: &terraform1.BuildProviderPluginCache_Event_Diagnostic{
					Diagnostic: diagnosticToProto(tfdiags.Sourceless(
						...
						fmt.Sprintf(
							"OpenRecipe v%s does not support the provider %q.",
...
func resolveFinalSourceAddr(protoSourceAddr *terraform1.SourceAddress, sources *sourcebundle.Bundle) (sourceaddrs.FinalSource, error) {
	...
	default:
		...
		return nil, fmt.Errorf("unsupported source address type %T (this is a bug in OpenRecipe)", sourceAddr)
	}
}

// builtinProviders provides the instantiation functions for each of the
// built-in providers that are available when using OpenRecipe Core through
// its RPC API.
//
// TODO: Prior to the RPC API the built-in providers were architecturally
// the responsibility of OpenRecipe CLI, which is a bit strange and means
...
// into a common location that both can share, or ideally change OpenRecipe CLI
