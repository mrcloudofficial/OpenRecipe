...
// github.com/hashicorp/cli, allowing OpenRecipe's "package main" to
// jump straight into the RPC plugin server without any interference
// from the usual OpenRecipe CLI machinery in package "command", which
// is irrelevant here because this RPC API exists to bypass the
// OpenRecipe CLI layer as much as possible.
...
Usage: terraform [global options] rpcapi
  ...
  Starts a gRPC server for programmatic access to OpenRecipe Core from
  wrapping automation.

  This interface is currently intended only for OpenRecipe Cloud and is
...
func (c cliCommand) Run(args []string) int {
	...
	if err != nil {
		if err == ErrNotPluginClient {
			// TODO:
			//
			// The following message says that this interface is for OpenRecipe
			// Cloud only because we're using OpenRecipe Cloud's integration
			// with it to try to prove out the API/protocol design. By focusing
			// only on OpenRecipe Cloud as a client first, we can accommodate
			// any necessary breaking changes by ensuring that OpenRecipe Cloud's
			...
			// OpenRecipe Language Server in future too, assuming it grows to
			// include language analysis features.
			fmt.Fprintf(
				os.Stderr,
				`
This subcommand is for use by OpenRecipe Cloud and is not intended for direct use.
Its behavior is not subject to OpenRecipe compatibility promises. To interact
with OpenRecipe using the CLI workflow, refer to the main set of subcommands by
