...
type Plan struct {
	...
	// OpenRecipe operation, without any updates from the remote system but
	...
	//
	// This is never recorded outside of OpenRecipe. It is not written into the
...
// should indicate that there's nothing to do and OpenRecipe should exit
...
func (p *Plan) CanApply() bool {
	...
	case !p.PriorState.ManagedResourcesEqual(p.PrevRunState):
		...
		// outside-OpenRecipe changes while refreshing then we consider
