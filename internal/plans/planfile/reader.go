	...
	if err != nil {
		...
		if b, sErr := ioutil.ReadFile(filename); sErr == nil {
			if bytes.HasPrefix(b, []byte("tfplan")) {
				return nil, errUnusable(fmt.Errorf("the given plan file was created by an earlier version of OpenRecipe; plan files cannot be shared between different OpenRecipe versions"))
			}
		}
		return nil, err
	}
...
// version of OpenRecipe, if it is invalid, etc.
...
func (r *Reader) ReadDependencyLocks() (*depsfile.Locks, tfdiags.Diagnostics) {
	...
	diags = diags.Append(tfdiags.Sourceless(
		...
		"The specified saved plan file does not include any dependency lock information. This is a bug in the previous run of OpenRecipe that created this file.",
	))
	return nil, diags
}
