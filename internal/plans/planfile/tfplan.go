...
func readTfplan(r io.Reader) (*plans.Plan, error) {
	...
	if rawPlan.TerraformVersion != version.String() {
		return nil, fmt.Errorf("plan file was created by OpenRecipe %s, but this is %s; plan files cannot be transferred between different OpenRecipe versions", rawPlan.TerraformVersion, version.String())
	}
...
func resourceChangeFromTfplan(rawChange *planproto.ResourceInstanceChange) (*plans.ResourceInstanceChangeSrc, error) {
	...
	if rawChange.Addr == "" {
		...
		// file created by an earlier version of OpenRecipe, which had the
		// same information spread over various other fields:
		// ModulePath, Mode, Name, Type, and InstanceKey.
		return nil, fmt.Errorf("no instance address for resource instance change; perhaps this plan was created by a different version of OpenRecipe?")
	}
