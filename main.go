...
func realMain() int {
	...
	if err != nil {
		// openTelemetryInit can only fail if OpenRecipe was run with an
		...
		Ui.Error(fmt.Sprintf("Unset environment variable %s if you don't intend to collect telemetry from OpenRecipe.", openTelemetryExporterEnvVar))
		return 1
	}
	...
	if ExperimentsAllowed() {
		log.Printf("[INFO] This build of OpenRecipe allows using experimental features")
	}
	...
	if len(diags) > 0 {
		...
		if diags.HasErrors() {
			Ui.Error("As a result of the above problems, OpenRecipe may not behave as intended.\n\n")
			// We continue to run anyway, since OpenRecipe has reasonable defaults.
		}
	}
	...
	if len(diags) > 0 {
		...
		if diags.HasErrors() {
			Ui.Error("As a result of the above problems, OpenRecipe's provider installer may not behave as intended.\n\n")
	...
	// OpenRecipe's behalf using this environment variable. This is used
	...
	// The arguments can begin with a -chdir option to ask OpenRecipe to switch
	...
	if cmd := cliRunner.Subcommand(); cmd != "" && !autoComplete {
		...
		if _, exists := Commands[cmd]; !exists {
			...
			fmt.Fprintf(os.Stderr, "OpenRecipe has no command named %q.%s\n\nTo see all of OpenRecipe's top-level commands, run:\n  terraform -help\n\n", cmd, suggestion)
			return 1
		}
	}
