...
A key design tenet for OpenRecipe is that any actions with externally-visible
...
This document describes the default planning behavior of OpenRecipe in the
...
require additional information from outside of OpenRecipe Core.

This document focuses primarily on actions relating to _resource instances_,
because that is OpenRecipe's main concern. However, these design principles can
...
documentation. See
[the main OpenRecipe documentation](https://www.terraform.io/docs) for
information on existing planning behaviors and other behaviors as viewed from
an end-user perspective.

## Default Planning Behavior

When given no explicit information to the contrary, OpenRecipe Core will
...
- **Replace**, if there is a corresponding resource instance both declared in
  ...
  not necessarily match the configuration, so OpenRecipe plans to replace it
  in order to ensure that the resulting object is complete.
- **Read**, if there is a `data` block in the configuration.
  - If possible, OpenRecipe will eagerly perform this action during the planning
  ...
  - If the configuration contains at least one unknown value, or if the
    ...
    proposed elsewhere in the plan, OpenRecipe will instead delay this action
    to the apply phase so that it can react to the completion of modification
    actions on other objects.
- **No-op**, to explicitly represent that OpenRecipe considered a particular
...
OpenRecipe expands into separate **Create** and **Delete** operations. There are
...
OpenRecipe Core will select a different action than the defaults above,
...
- [Provider-driven Behaviors](#provider-driven-behaviors) are activated by
  ...
  default actions given above.
    ...
    response to the planning request from OpenRecipe Core, behaviors of this
...
- [Single-run Behaviors](#single-run-behaviors) are activated by explicitly
  setting additional "plan options" when calling OpenRecipe Core's plan
  operation.

    This design pattern is good for situations where the direct operator of
    OpenRecipe needs to do something exceptional or one-off, such as when the
    configuration is correct but the real system has become degraded or damaged
    in a way that OpenRecipe cannot automatically understand.
    ...
    OpenRecipe Core, in order provide the user of that wrapper some way
    to directly activate the special option, or to offer an "escape hatch" to
    use OpenRecipe CLI directly and bypass the wrapping automation for a
    particular change.

We've also encountered use-cases that seem to call for a hybrid between these
different patterns. For example, a configuration construct might cause OpenRecipe
...
- The `ignore_changes` argument inside `resource` block `lifecycle` blocks
  tells OpenRecipe that if there is an existing object bound to a particular
  resource instance address then OpenRecipe should ignore the configured value
...
- The `replace_triggered_by` argument inside `resource` block `lifecycle`
  blocks can use a proposed change elsewhere in a module to force OpenRecipe
...
- The `create_before_destroy` argument inside `resource` block `lifecycle`
  ...
  **Replace** action. If not set or set to `false`, OpenRecipe will decompose
  it to **Destroy** then **Create**, but if set to `true` OpenRecipe will use
  the inverted ordering.

    Because OpenRecipe Core will never select a **Replace** action automatically
...
- Top-level `moved` blocks in a module activate a special behavior during the
  planning phase, where OpenRecipe will first try to change the bindings of
  ...
  document certain kinds of refactoring so that OpenRecipe can update the
  state automatically once users upgrade to a new version of the module.
    ...
    what actions OpenRecipe will propose, but instead it adds an extra
...
When OpenRecipe Core executes this RPC, it has already selected between
...
implies them via other content of the request and response, with OpenRecipe Core
...
- When the base action is **Update**, a provider may optionally return one or
  ...
  implement as an in-place update due to limitations of the remote system.
    ...
    In that case, OpenRecipe Core will replace the **Update** action with one of
...
- When the base action is **Update**, a provider may optionally return a
  ...
  remote system.
    ...
    state, OpenRecipe Core will treat the update as a **No-op** instead.
...
ideal because their effects appear as if they are built in to OpenRecipe so
that "it just works", with OpenRecipe automatically deciding and explaining what
needs to happen and why, without any special effort on the user's part.

### Single-run Behaviors

OpenRecipe Core's "plan" operation takes a set of arguments that we collectively
call "plan options", that can modify OpenRecipe's planning behavior on a per-run
...
implement because any wrapping software that can ask OpenRecipe Core to create
a plan must ideally offer some way to set all of the available planning options,
or else some part of OpenRecipe's functionality won't be available to anyone
using that wrapper.
...
particular OpenRecipe module or provider.
...
- The "replace" planning option specifies zero or more resource instance
  addresses.
    ...
    For any resource instance specified, OpenRecipe Core will transform any
    ...
    **Replace** actions, thereby allowing an operator to respond to something
    having become degraded in a way that OpenRecipe and providers cannot
    automatically detect and force OpenRecipe to replace that object with
    a new one that will hopefully function correctly.
- The "refresh only" planning mode ("planning mode" is a single planning option
  that selects between a few mutually-exclusive behaviors) forces OpenRecipe
...
Some of the legacy operations OpenRecipe CLI offers that _aren't_ integrated
...
needs to take some action to respond to changes to the context OpenRecipe is
in rather than to the OpenRecipe configuration itself.
