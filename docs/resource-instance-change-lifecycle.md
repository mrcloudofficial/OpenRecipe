# OpenRecipe Resource Instance Change Lifecycle

This document describes the relationships between the different operations
called on a OpenRecipe Provider to handle a change to a resource instance.
...
* **Proposed New State**: OpenRecipe Core uses some built-in logic to perform
  ...
  which a provider may use as a starting point for its planning operation.
    ...
    OpenRecipe expects the value to only be chosen by the provider and it will
    ...
    OpenRecipe Core therefore constructs the proposed new state by taking the
...
* **Initial Planned State** and **Final Planned State** are both descriptions
  ...
  the planned action.
    ...
    the initial one is created during OpenRecipe Core's planning phase based
...
* **Previous Run State** is the same object as the **New State** from
  the previous run of OpenRecipe. This is exactly what the provider most
  recently returned, and so it will not take into account any changes that
  may have been made outside of OpenRecipe in the meantime, and it may conform
...
* **Upgraded State** is derived from **Previous Run State** by using some
  ...
  been made outside of OpenRecipe.

* The **Import ID** and **Import Stub State** are both details of the special
  process of importing pre-existing objects into a OpenRecipe state, and so
...
called to plan and apply a change, including the expectations OpenRecipe Core
enforces for each.

For historical reasons, the original OpenRecipe SDK is exempt from error
messages produced when certain assumptions are violated, but violating them
will often cause downstream errors nonetheless, because OpenRecipe's workflow
...
OpenRecipe Core will call this function multiple times at different phases
...
a subsequent apply operation, allowing OpenRecipe to render the plan for the
...
The first call is during the planning phase, before OpenRecipe prints out a
...
If the user accepts the plan, OpenRecipe will call `PlanResourceChange` a
...
OpenRecipe Core compares the final with the initial planned state, enforcing
...
a single OpenRecipe run is complete. OpenRecipe Core saves the **New State**
in a state snapshot for the entire configuration, so it'll be preserved for
use on the next run.

When the user subsequently runs OpenRecipe again, the **New State** becomes
...
saved state snapshot from one run to the next, OpenRecipe Core must deal with
...
OpenRecipe Core therefore begins by calling `UpgradeResourceState` and passing
the **Previous Run State** in a _raw_ form, which in current protocol versions
is the raw JSON data structure as was stored in the state snapshot. OpenRecipe
...
OpenRecipe Core has no way to enforce it, a provider should only change the
...
changes made to the corresponding remote object outside of OpenRecipe.
...
OpenRecipe Core to interact with the data fully for subsequent steps.

### ReadResource

Although OpenRecipe typically expects to have exclusive control over any remote
object that is bound to a resource instance, in practice users may make changes
to those objects outside of OpenRecipe, causing OpenRecipe's records of the
object to become stale.

The `ReadResource` function asks the provider to make a best effort to detect
any such external changes and describe them so that OpenRecipe Core can use
...
OpenRecipe Core expects a provider to carefully distinguish between the
...
* **Drift**: the remote API returned data that is materially different from
  ...
  requested.
    ...
    provider does this, OpenRecipe _may_ report it to the user as a change
    made outside of OpenRecipe, if OpenRecipe Core determined that the detected
...
the planned new state and new state, or OpenRecipe Core will raise an error.
...
entire lifecycle is driven through OpenRecipe, including the initial creation
of the object.

As an aid to those who are adopting OpenRecipe as a replacement for existing
processes or software, OpenRecipe also supports adopting pre-existing objects
to bring them under OpenRecipe's management without needing to recreate them
first.
...
**Previous Run State** pretending as if a previous OpenRecipe run is what had
created the object.
...
OpenRecipe Core will always pass the returned **Import Stub State** to the
...
changes outside of OpenRecipe, a provider may not be able to fully support
...
* Return an error explaining why importing isn't possible.
    ...
    to bring the existing object under OpenRecipe's management. However, if a
    ...
    as part of adopting OpenRecipe, rather than to perform an import that will
    leave the object in a situation where OpenRecipe cannot meaningfully manage
    it.
