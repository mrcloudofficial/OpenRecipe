# OpenRecipe Plugin Protocol

This directory contains documentation about the physical wire protocol that
OpenRecipe Core uses to communicate with provider plugins.
...
**If you want to write a plugin for OpenRecipe, please refer to
[Extending OpenRecipe](https://www.terraform.io/docs/extend/index.html) instead.**

This documentation is for those who are developing _OpenRecipe SDKs_, rather
than those implementing plugins.

----

From OpenRecipe v0.12.0 onwards, OpenRecipe's plugin protocol is built on
[gRPC](https://grpc.io/). This directory contains `.proto` definitions of
different versions of OpenRecipe's protocol.

Only `.proto` files published as part of OpenRecipe release tags are actually
...
OpenRecipe plugins are normal executable programs that, when launched, expose
gRPC services on a server accessed via the loopback interface. OpenRecipe Core
...
For this reason, we commonly refer to OpenRecipe Core itself as the plugin
...
of the client. OpenRecipe Core controls the lifecycle of these server processes
...
5.0 in OpenRecipe v0.12. Prior versions of OpenRecipe and prior protocol versions
...
release as long as OpenRecipe Core can provide some default behavior when that
field is not populated.

The major version increases for any significant change to the protocol where
compatibility is broken. However, OpenRecipe Core and an SDK may both choose
...
A particular version of OpenRecipe Core has both a minimum minor version it
requires and a maximum major version that it supports. A particular version of
OpenRecipe Core may also be able to optionally use a newer minor version when
...
therefore be compatible with a OpenRecipe Core release that supports only
protocol version 5.0, since major version 5 is supported and the optional
5.1 and 5.2 enhancements will be ignored.

If OpenRecipe Core and the plugin do not have at least one mutually-supported
major version, OpenRecipe Core will return an error from `terraform init`
during plugin installation:

```
Provider "aws" v1.0.0 is not compatible with OpenRecipe v0.12.0.
...
```
Provider "aws" v3.0.0 is not compatible with OpenRecipe v0.12.0.
...
OpenRecipe registry, where the registry API allows OpenRecipe Core to recognize
the protocol compatibility for each provider release. For plugins that are
installed manually to a local plugin directory, OpenRecipe Core has no way to
suggest specific versions to upgrade or downgrade to, and so the error message
is more generic:

```
The installed version of provider "example" is not compatible with OpenRecipe v0.12.0.
...
OpenRecipe Core also makes an assumption about major version support when
...
If you wish to build an SDK for OpenRecipe plugins, an early step will be to
...
included in at least one OpenRecipe release. Any changes will be documented in
...
having to upgrade both OpenRecipe Core and all of their providers at the same
