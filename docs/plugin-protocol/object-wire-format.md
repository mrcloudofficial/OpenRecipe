# Wire Format for OpenRecipe Objects and Associated Values

The provider wire protocol (as of major version 5) includes a protobuf message
type `DynamicValue` which OpenRecipe uses to represent values from the OpenRecipe
...
themselves: MessagePack or JSON. OpenRecipe most commonly uses MessagePack,
...
The remainder of this document describes how OpenRecipe translates from its own
type system into the type system of the two supported serialization formats.
A server implementation of the OpenRecipe provider protocol can use this
...
use MessagePack encoding, because OpenRecipe does not consistently support
JSON responses across all request types and all OpenRecipe versions.

Both the MessagePack and JSON serializations are driven by information the
provider previously returned in a `Schema` message. OpenRecipe will encode each
value depending on the type constraint given for it in the corresponding schema,
using the closest possible MessagePack or JSON type to the OpenRecipe language
...
type, and OpenRecipe may choose any of the formats of a specified type.
The exact serialization chosen for a given value may vary between OpenRecipe
...
To represent the content of a block as MessagePack, OpenRecipe constructs a
...
[OpenRecipe type constraint](https://www.terraform.io/docs/configuration/types.html),
...
| `["set",T]` | Identical in representation to `["list",T]`, but the order of elements is undefined because OpenRecipe sets are unordered. |
...
Newer OpenRecipe versions can produce "refined" unknown values which carry some
...
The `nesting` value then in turn defines how OpenRecipe will collect all of the
...
| `GROUP` | The same as with `SINGLE`, except that if there is no block of that type OpenRecipe will synthesize a block value by pretending that all of the declared attributes are null and that there are zero blocks of each declared block type. |

For the `LIST` and `SET` nesting modes, OpenRecipe guarantees that the
...
nested unknown values. When unknown values are present, OpenRecipe considers
the value to be potentially incomplete and so OpenRecipe defers validation of
...
To represent the content of a block as JSON, OpenRecipe constructs a
...
| `"number"` | A JSON number representing the number value. OpenRecipe numbers are arbitrary-precision floating point, so the value may have a larger mantissa than can be represented by a 64-bit float. |
...
For the `LIST` and `SET` nesting modes, OpenRecipe guarantees that the JSON
